...
1. **SPEC** - Enter plan mode yourself if the operator hasn't, interview, and write
   `.project/specs/<date>-<slug>.md`: as short as truth allows, trim rationale before executables
   - goal, files, out-of-scope, verification commands, progress log, no person's name. Verify every LOAD-BEARING claim you cite: an inherited claim is
...
5. **REVIEW** - Run `/code-review`; add `/security-review` on auth, serving, PII, or deletes.
   ...
   REQUIRED when the chunk touches `src/`, on request otherwise. `/simplify` is optional and
   runs BEFORE `/code-review` (its fixes need reviewing too). Docs-only chunk: PII gates alone.
   A review covers EVERY change since the chunk's base, committed or not - name that range and
   verify the reviewer received it.
...
9. **HANDOFF** - Name every discovered thing and WHERE IT WENT - stub, `IDEAS.md` line, vision
   ...
   a COMPLETE also names `acceptance: run` or `owed at <chunk>`, and moves to
...
`game-summaries` returns a stable `event_id`. `root_team_id` is TWO namespaces: in
`/teams/{id}/opponents` a local registry key, never a `gc_uuid`; in `/organizations/{id}/teams`
it IS the `gc_uuid`; never store either in the wrong column. A present
`progenitor_team_id` means the coach linked via team lookup, a reliable dedup signal. Resolve
`public_id` to `gc_uuid` via `POST /search` filtered by `public_id`.
