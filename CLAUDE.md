...
- **Enriched stat columns** (schema-ready, not yet populated): `game_stream_id` on games (links to boxscore/plays endpoints), `bats`/`throws` on players, home/away and vs-LHP/RHP split columns on season batting/pitching, `pitches`/`total_strikes` on game pitching, `batting_order`/`positions_played`/`is_primary` on game batting, `stat_completeness` tracking on all stat tables, `spray_charts` table for ball-in-play coordinate data.
