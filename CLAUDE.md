...
- **FPS% (first pitch strike %)**: Pitching stat computed from plays data. Flag records actual first-pitch result for ALL PAs; exclusions (HBP, Intentional Walk) applied at query time only (`WHERE outcome NOT IN ('Hit By Pitch', 'Intentional Walk')`). **Coaching priority**: FPS% is the first stat coaches look at when scouting a pitching staff -- always surface it prominently.
...
- **Game ordering convention**: Every `ORDER BY game_date` MUST include `start_time` as a same-direction tiebreaker with `NULLS LAST` (e.g., `ORDER BY game_date DESC, start_time DESC NULLS LAST`). Doubleheaders, rescheduled games, and multi-site days all produce same-date collisions that `game_date` alone cannot resolve.
