...
**Skill trigger phrases live in each skill's `description` frontmatter** (`.claude/skills/*/SKILL.md`) and are deliberately not restated here -- one source, not three. Six skills are user-triggered: **plan**, **implement**, **ingest-endpoint**, **codex-spec-review**, **codex-review**, and **workflow-help**, which prints the full phrase list on request. Four more are agent-loaded rather than user-triggered: **agent-standards**, **context-fundamentals**, **filesystem-context**, **multi-agent-patterns**.

**One workflow is not a skill and is therefore stated here in full**, because there is no `description` to carry it:
