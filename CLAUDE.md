...
- **Scouting pipeline parity**: `run_scouting_sync` (web) and `bb data scout` (CLI) MUST produce equivalent data artifacts — all stages of the scouting pipeline in both paths.
...
- **Canonical plays-stage orchestration**: `run_plays_stage()` in `src/gamechanger/pipelines/plays_stage.py` — single entry point for plays crawl + load + reconcile orchestration. Any new plays-stage orchestration site (CLI, web, report, future caller) MUST invoke this helper. Callers MUST NOT instantiate `PlaysLoader` directly to orchestrate plays loading, MUST NOT call the plays endpoint (`GET /game-stream-processing/{event_id}/plays`) directly, and MUST NOT call `reconcile_game(...)` directly as part of plays-stage orchestration. Exceptions: the standalone `bb data load --loader plays` operator surface and the standalone `bb data reconcile` CLI are NOT plays-stage orchestration paths and may bypass.
