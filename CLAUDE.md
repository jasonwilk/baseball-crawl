...
- **Opponent resolution**: `/admin/opponents/{link_id}/resolve` provides a GC-search-powered workflow for connecting unresolved opponents to real GC teams. Includes duplicate detection at confirm time (warns if selected team's `public_id` already exists). "No match" dismissal sets `is_hidden=1` (reversible via "Unhide").
