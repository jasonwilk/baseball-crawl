...
> **Current strategic frame (2026-06-12)**: The product as actually used is reports-first -- generate a one-off scouting report for a GameChanger `public_id` and share the link. Member-team sync, the dashboard, and tracked-opponent management are unused and headed for quarantine-then-removal. The forward feature is morning-of-game scheduled reports. Explicit non-goals: cross-team player identity, multi-season rollups, longitudinal tracking. See `docs/ROADMAP.md` for the full reframe, protected core, and epic sequence (A-E). The sections below still describe the original multi-surface vision; `docs/ROADMAP.md` is authoritative on scope until a "curate the vision" session reconciles `docs/VISION.md`. See `.claude/rules/quarantine.md` for which surfaces are quarantined (dashboard, member-sync, opponent discovery) and what quarantine means (deprecated, unmaintained, parity-excluded, closed to new feature work -- not deletion).
...
- **Canonical team deletion**: `cascade_delete_team()` and `cleanup_orphan_teams()` in `src/reports/generator.py` — consolidated deletion paths. New team-deletion paths MUST use these functions. Callers (e.g., `src/api/routes/admin.py::_delete_team_cascade`) are thin wrappers that delegate cleanup to the canonical helper and add only path-specific concerns (HTTP response shaping, informed-consent gates, audit logging, flash messages). Callers MUST NOT duplicate cleanup logic in-place.
...
- **Canonical admin predicate**: `_user_is_admin(conn, user)` (connection-injected, for middleware/`_get_permitted_teams`) and its own-connection wrapper `user_is_admin(user)` in `src/api/auth.py` — single source of the admin check. Admin = `ADMIN_EMAIL` env matches user email OR `users.role = 'admin'`. Route modules (`dashboard.py::_is_admin_user`, `admin.py::_require_admin`) delegate to it. New code MUST NOT add a third copy of the admin check — delegate to the auth.py predicate.
- **Dashboard team access (admin-sees-all)**: `_get_permitted_teams()` in `src/api/auth.py` resolves admins to ALL `teams` rows; non-admins resolve to their explicit `user_team_access` grants. Applies in dev and production. Security-relevant invariant: admins are never gated by `user_team_access`; non-admins always are. Do not reintroduce membership-based auto-grant as the dashboard-access mechanism for admins.
...
- **Scouting pipeline parity** (FROZEN -- both sides quarantined): `run_scouting_sync` (web) and `bb data scout` (CLI) historically had to produce equivalent data artifacts (all five stages in both paths). Both sides are now quarantined (see `.claude/rules/quarantine.md`), so this is an inert/frozen invariant for the surviving code, NOT a forward constraint -- do not add new stages or extend this parity. New scouting work targets the reports flow.
...
- **Shared query functions**: Query logic shared across surfaces lives in a shared function in `src/api/db.py`. This pattern is PRESERVED for the protected-core seams that still serve both surfaces (e.g. `get_pitching_workload` / `get_pitching_history` / `build_pitcher_profiles`). The dashboard is now quarantined (see `.claude/rules/quarantine.md`), so NEW cross-surface data needs target the reports flow only -- they are not a forward parity obligation to keep the dashboard in step.
...
See `.claude/rules/scouting-data-flows.md` for opponent flow vs. reports flow comparison and naming conventions. Reports is the SOLE forward scouting/delivery surface; the dashboard/opponent-flow surfaces are quarantined and their feature parity is frozen -- new work does NOT need to keep both surfaces in parity.
