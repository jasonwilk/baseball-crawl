...
@app.command(name="backfill-team-names")
def backfill_team_names(
    dry_run: bool = typer.Option(
        False,
        "--dry-run",
        help="Show how many name mappings exist on disk without writing to the database.",
    ),
) -> None:
    """Backfill UUID-stub opponent team names from on-disk data.

    Scans data/raw/ for opponents.json, schedule.json, and scouting games.json
    files to build a UUID → name mapping, then updates any team rows where
    name == gc_uuid (UUID-stubs from prior loads without name resolution).

    Safe to re-run -- rows with non-UUID names are never touched.

    Examples:
        bb data backfill-team-names
        bb data backfill-team-names --dry-run
    """
    from src.gamechanger.loaders.backfill import (
        backfill_team_names as _backfill,
    ...
    if dry_run:
        ...
        typer.echo(
            f"Dry run -- found {len(lookup)} name mapping(s) in on-disk data. "
            "No changes written."
        )
        raise SystemExit(0)
    ...
    with closing(sqlite3.connect(str(db_path))) as conn:
        conn.execute("PRAGMA journal_mode=WAL;")
        conn.execute("PRAGMA foreign_keys=ON;")
        updated = _backfill(conn, data_root)

    typer.echo(f"Backfill complete: {updated} team name(s) updated.")
    raise SystemExit(0)
