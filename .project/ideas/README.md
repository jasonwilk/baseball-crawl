...
| [IDEA-113](IDEA-113-season-aggregates-module-misnomer.md) | Rename/fold `src/db/season_aggregates.py` post-E-259 — after the cutover it no longer computes aggregates (only holds the SUM projection a reader consumes); the name is a misnomer. Fold the builders + `*_RECOMPUTE_KEYS` into `src/api/db.py` next to their sole consumer, or rename. Cosmetic/orientation. Surfaced by DE in E-259 holistic. Domain: DE | CANDIDATE | 2026-10-07 |
| [IDEA-114](IDEA-114-clarify-morning-run-dryrun-summary-prose.md) | Clarify CLAUDE.md's morning-run `--dry-run` prose — it reads as if dry-run sends the always-sent summary email, but `src/cli/report.py:581`/`:666` exempt dry-run. This exact ambiguity misled the CR spec-audit (finding 6, refuted by code) during E-256/E-259 planning. One-line fix; NOT folded into either epic (scope creep). Domain: CA | CANDIDATE | 2026-10-07 |
...
| [IDEA-117](IDEA-117-scope-multi-agent-patterns-never-summarize.md) | Scope `multi-agent-patterns/SKILL.md:24`'s bare "Never summarize" to the dispatch context block (its `:26` already scopes it; the standalone `:24` reads as a blanket verbatim-relay mandate — the over-broad reading E-260 removed elsewhere). Mirror E-260-04's `:203` scoping. Retained-class, ruled OUT of E-260-01 scope. Domain: CA | CANDIDATE | 2026-10-10 |
| [IDEA-118](IDEA-118-refresh-context-fundamentals-ambient-figures.md) | Refresh the stale ambient per-session figures in `context-fundamentals/SKILL.md` (`:28` "614-886 lines" + `:185`/`:193` example budget) that E-260-04's `:74-85` table re-derivation left behind — non-contradicting (ambient SUBSET, not whole-layer) but stale; drop the post-E-213 provenance. Domain: CA | CANDIDATE | 2026-10-10 |
