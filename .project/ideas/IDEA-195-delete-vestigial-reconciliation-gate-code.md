...
**⚠️ FOOTGUN -- there are TWO ratchets in this repo and they share vocabulary.** ⚰ **UPDATED 2026-08-02 at E-280 closure: BOTH GATES ARE NOW RETIRED.** The line below read *"Only one is retired"* and that is no longer true.

- **Retired 2026-07-26**: the *reconciliation* ratchet -- this one. Baseline `.project/baselines/reconciliation-scoreboard.json`, gate code in `recon_scoreboard.py`, flag `--update-baseline` on `bb report reconcile-scoreboard`.
- **Retired 2026-08-02 (E-280)**: the *context-layer* ratchet -- trigger 7 of `.claude/rules/context-layer-assessment.md`, hook `.claude/hooks/context-ratchet.sh`, baseline `.project/baselines/context-layer-ratchet.json`, also carrying an `--update-baseline` flag. ~~**This one still gates every epic closure and is not in scope here.**~~ **The operator retired the GATE outright** (*"I don't think gating and ratchets are working. I think we just need periodic refinement."*); the script survives as an on-demand diagnostic feeding a periodic refinement pass, and only the operator runs `--update-baseline`. **Still not in scope here** -- the reason changed, the scoping did not.

**⚠️ THE FOOTGUN IS NOT DISSOLVED BY THE SECOND RETIREMENT -- it is INVERTED, and the new direction is the more dangerous one.** While one gate was live, the hazard was a vocabulary-driven sweep **deleting a live gate**. Now that neither gates, the hazard is a reader concluding that *both instruments* are disposable and deleting the context-layer **measurement**, which is explicitly preserved and is the input to the periodic refinement pass. **Retiring a gate is not deleting its instrument** -- that is the identical distinction this idea already draws for the reconciliation half (*"Delete the gate half; keep the measurement half"*), now applying to both.

A grep for `--update-baseline`, `baseline`, or "ratchet" still hits both, and the two baseline JSONs still sit **in the same directory** with adjacent names. Scope this deletion by the *file* (`recon_scoreboard.py`, its tests, its own baseline JSON), never by the shared vocabulary. Related: [[IDEA-123]], whose ratchet-false-FAIL failure mode was dissolved by the same retirement.
