...
- Does this belong as a skill at all, or as a short procedure inside the existing `ingest-endpoint` skill? E-072 assumed a whole new skill; that assumption is what carried the context-ratchet cost that helped kill it. ⚰ **The COST is gone as of 2026-08-02 — E-280 retired the context-layer size gate on the operator's ruling**, so ~~trigger 7 gates `.claude/skills/**` line growth against a committed baseline, and a large new SKILL.md would need an operator-signed exception on top of an already-stale baseline~~ no longer applies: there is no baseline to clear and no exception to sign, and a new SKILL.md is now priced only at the periodic refinement pass. ⚠️ **The QUESTION survives the removal of its cost, and should not be read as settled in favour of a new skill** — "is a whole skill the right shape for this?" was always a simple-first design question, and the ratchet was only ever an argument reinforcing it. The E-072 sentence above stays as the historical record of why that epic died; only the forward-looking constraint is retired.
