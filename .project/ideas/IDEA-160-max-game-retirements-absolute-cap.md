...
1. **The cap must fire where the floor would NOT.** Size the fixture so the floor ratio PASSES and only the cap refuses (the roster equivalent used 14 prior / 9 fresh / 5 absent: `9 >= 7` clears the floor, `5 > 2` trips the cap). A fixture that also fails the floor proves nothing about the cap.
