...
- [ ] **AC-6 (primary-source confirmation)**: The model doc's NSAA and Legion/NRBL tables are confirmed to match the operator-verified primary sources (per Technical Notes TN-10) — coach confirmed NO table change is needed (Sub-Varsity is already flat 1/2/3/4 year-round; Legion already flat 105 no date split). NRBL carries its OWN `nrbl.net` source line ("NRBL adopts standard ALB pitching regulations") alongside the existing ALB citation (doc line 52) — it does NOT silently inherit ALB's citation. The doc's NSAA source attribution must agree with pitch-rules.md's NSAA cite (E-272-04) — same source, no drift.
