...
- 2026-03-06: Post-archive codex review. 4 findings triaged, 3 fixed, 1 skipped. Fixed: (1) `scripts/check_credentials.py` -- swapped exception handler order so `ForbiddenError` caught before `CredentialExpiredError` (was dead code due to subclass relationship). (2) `docs/admin/bootstrap-guide.md` -- corrected base URL from `api.sportninja.com` to `api.team-manager.gc.com`. (3) `tests/test_check_credentials.py` -- tightened 403 assertion to require "denied" only (was accepting "expired" as alternative). Skipped: `crawl.run(profile=...)` assertion in orchestrator tests -- low value, tests mock wiring not real behavior. All 10 credential check tests pass.
