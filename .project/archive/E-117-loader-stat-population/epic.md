...
**Routing note:** The routing table maps "ETL" to data-engineer, and these stories touch loader upsert SQL. However, the work is Python dict-mapping and SQL column-list expansion in existing loader files — no schema design, no migration authoring, no query optimization. SE is the correct agent per the "Python implementation, crawlers, parsers" row. DE was consulted during planning for column inventories and architectural decisions (stat_completeness, NULL handling).
