...
- **Top-level keys are classified by IDENTITY, not shape** (corrected 2026-08-02, narrowed 2026-08-03): each key is a `public_id` slug or a UUID, and the form does NOT mark which side it is. BOTH keys are slugs on a minority of payloads. **What decides the form is unestablished** -- "slug iff the team has a public GC presence" was refuted 2026-08-03 (opponents with their own public_id came back UUID-keyed). Match a key against our own `public_id`, then our `gc_uuid`; the remaining key is the opponent's. A UUID regex must NOT decide which side is which -- shape is a last-resort tiebreak only, and shape-based detection silently discarded the opponent's entire envelope on all-slug payloads.
...
## `games.game_date` Derivation -- Four Traps (read 2026-07-27, E-278 planning)

⚠️ **RENAMED to `GameSummaryEntry.date_source_instant` in E-278-05 (2026-07-28). `last_scoring_update` no longer exists as an attribute -- a grep for it returns ZERO hits in `src/` and `tests/`, so do not go looking for it.** The GameChanger WIRE field of that name is untouched and still real: see `docs/api/endpoints/get-teams-team_id-game-summaries.md`. The observation that forced the rename is below and still holds of the renamed field.

**That field does NOT carry a last-scoring-update instant on any live path.** `ScoutingLoader._build_games_index_from_data` (navigate by SYMBOL -- this cited `:774` and had already rotted) is the ONLY construction site in `src/` and sets `last_scoring_update=str(start_ts)` where `start_ts = game.get("start_ts") or game.get("end_ts") or ""`. The real field belongs to the authenticated `game-summaries` endpoint, whose file-reading loader entry points died in E-256. The name is vestigial and actively misleading -- it misled a live-data audit into filing "a scorekeeper edited the book the next morning" as a candidate cause. ⚰ `_derive_game_date`'s docstring USED to inherit the same error; E-278-05 fixed it, and the `GameSummaryEntry` attribute block now states what each instant field holds and how the two differ. **The basis is already the start instant.** Do not "fix" it by re-basing.

⚰ **REPAIRED in E-278-04 (2026-07-28). The paragraph below describes the OLD behavior and is retained because its MECHANISM is the durable lesson; do not read it as current.** What changed: `derive_local_date` now returns `None` on a present-but-unresolvable zone, `_derive_game_date` resolves the zone FIRST (via the new `resolve_timezone` predicate) and stores the `1900-01-01` sentinel rather than a date, `tzdata` is a declared runtime dependency so the alias namespace resolves at all, and an unknown-date row is no longer a dedup candidate. The ABSENT-timezone sentence still holds exactly as written -- absent and unresolvable are different cases and only the second changed.

**The durable half, which no fix retires: the fail-open output and the `or` fallback were identical BY CONSTRUCTION, so making the seam return `None` was a NO-OP at the site that produced the defect.** When `astimezone` raises, `dt` keeps the tzinfo it was PARSED with, and `.date()` on an aware datetime returns its own written wall-clock date -- exactly what `[:10]` slices off the front of an ISO string. That holds for ANY ISO-8601 string whose first ten characters are the date, not merely for our data. So "route the failure into the caller's existing fallback" is not automatically a fix: **read what the caller's fallback actually returns before believing a fail-closed change does anything.**

**(OLD, superseded)** An unknown timezone yields the UTC date through the SUCCESS return, not the `or` fallback. `derive_local_date`'s `except (ZoneInfoNotFoundError, ValueError)` logged a WARNING and fell through to `return dt.date().isoformat()` with `dt` still UTC -- it did NOT return `None`. For a `...Z` instant that string is byte-identical to the unparseable-instant fallback `instant[:10]`, so the two paths were indistinguishable from the stored date; only the log discriminated. An ABSENT timezone is different again: `tz_name = summary.timezone or get_operating_timezone().key` routes it to the operating zone (default `America/Chicago`), never to UTC.

**Two writers of `games.game_date`, on different bases.** Forward: `_derive_game_date` from `date_source_instant` (= `start_ts or end_ts`; named `last_scoring_update` before E-278-05). Backfill: `src/db/backfill_game_dates.py` from **`start_time`** + stored `games.timezone`, wired to a live operator command (`src/cli/data.py:570`), and it SKIPS rows with NULL `start_time`. Since `start_time = game.get("start_ts")` has no `end_ts` fallback while `date_source_instant` does, a row whose payload lacked `start_ts` is skipped by the backfill while a sibling row is re-derived -- splitting their dates with the API payloads in perfect agreement. Its `dry_run=True` mode is a ready-made read-only stored-vs-re-derived audit across all rows.

**`_upsert_game`'s ON CONFLICT sets `game_date = excluded.game_date` unconditionally** (`game_loader.py:1576`) -- no COALESCE, no `preserve_scores` gating, unlike the orientation tuple / `game_stream_id` / `start_time` / `timezone` on the adjacent lines. A date can move on any re-scout, so date divergence is a standing instability, not a one-time ingest event.

**⚠️ FIXED IN DEV/CI 2026-07-28, NOT YET IN PRODUCTION.** E-278-04 declared and installed `tzdata` (`requirements.in` / `requirements.txt` / `pyproject.toml`), taking `available_timezones()` from 498 to 599; `US/Central`, `US/Pacific` and every other backward link now resolve here. **The shipping image is a separate question**: `python:3.13-slim` still omits the links and the Dockerfile apt-installs only `curl` and `sqlite3`, so production keeps deriving wrong dates until `docker compose up -d --build app` rebuilds against the new lockfile. A green suite is not evidence production is fixed. The paragraph below records the pre-fix measurement.

**⚠️ The legacy `US/*` zone names DO NOT RESOLVE in this image, and that is a live `game_date` corruption.** EXECUTED 2026-07-27: `ZoneInfo("US/Central")` and `ZoneInfo("US/Pacific")` both raise `ZoneInfoNotFoundError`; `America/Chicago` / `America/Denver` / `America/Phoenix` / `America/New_York` all resolve. `available_timezones()` = 498, `/usr/share/zoneinfo/US/Central` absent, `tzdata` pip package NOT installed. The live DB stores `US/Central` on 20 rows and `US/Pacific` on 4, so those rows take branch C above and silently get the **UTC** date. Reproduced end-to-end: for `2026-06-20T02:00:00.000Z`, `_derive_game_date` returns `2026-06-19` under `America/Chicago` and `2026-06-20` under `US/Central` — **two rows naming the same real zone split their `game_date`**. de-epicA measured 24 unresolvable-tz rows, all 24 holding a UTC-slice date, 9 genuinely wrong.

**Do NOT reason about this from tzdata knowledge — RUN it.** Both of us got it wrong from memory first: the intuition "`US/Central` is just an alias for `America/Chicago`" is true of full tzdata and false of this container. The near-midnight intuition fails here too: branch C compares local against **UTC** (a 5-6 hour gap for Central), not against another US zone (1 hour), so *every* game starting at/after ~19:00 CDT lands on the wrong date — not just near-midnight ones.

Consequence for dedup work: `_find_duplicate_game`'s `WHERE game_date = ?` is a hard candidate gate, so any of the above splits a cross-perspective twin permanently. The derivation itself is PURE in `(instant, tz)` plus the `OPERATING_TIMEZONE` env (read per call, not cached) -- nothing is perspective-keyed.
