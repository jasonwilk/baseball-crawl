...
- Migrations are append-only. Never edit an applied migration. **But know what that rule rests on before asserting it at anyone** — see [migration-immutability-basis.md](migration-immutability-basis.md): it is MY agent definition only, absent from `.claude/rules/migrations.md` (never present in any revision), and mechanically unenforced.
...
- [etl-patterns.md](etl-patterns.md) -- Programmatic token refresh (`token_manager.py` + `signing.py`, auto-refresh before expiry), idempotent ingestion, pagination patterns (cursor-based, x-next-page), project file paths for migrations/DB/API spec/stat glossary.
...
- [plays_boxscore_reconciliation_baseline.md](plays_boxscore_reconciliation_baseline.md) -- E-245 north-star baseline: plays→boxscore fidelity is 98-100% (COVERAGE is the gap, not fidelity); correct grain = player-level (game+persp+player_id), NOT team-level; self-game (home==away) integrity bug = 23 games, breaks team attribution — **that population is REPAIRED (0 self-games live, verified 2026-07-27); the 23 is a baseline record, not a live hazard.** Full inventory in `.project/research/E-245-*`.
...
- [health_gate_prior_set_must_be_temporal.md](health_gate_prior_set_must_be_temporal.md) -- A "same population on both sides" ratio-gate invariant is satisfiable by a POLLUTED set; the load-bearing clause is WHEN `prior` was captured (before this run's writes). Capture belongs to the caller as a required param; a cap sitting over a broken gate MASKS it and is not evidence it works (E-276 planning).
- [reclamation-schema-lessons.md](reclamation-schema-lessons.md) -- Cascade-target and reachability-root are INDEPENDENT axes (a table can be both; migration 005 answers only the first, which is how an audit derived a contradiction that wasn't there); keep a pin entry after a keep-root makes it unreachable (FK-safety net; removal without the root = `IntegrityError` rolling back the WHOLE sweep) and comment WHY; any chunked-`IN` test is VACUOUS unless it lowers `SQLITE_LIMIT_VARIABLE_NUMBER` (250,000 on this build, not 999). (E-277.)
- [migration-immutability-basis.md](migration-immutability-basis.md) -- The append-only rule binds only the DE agent definition; it is absent from `.claude/rules/migrations.md` in EVERY revision and mechanically unenforced (filename tracking, no checksum). Know this before asserting it at anyone.
- [game_duplicate_class_taxonomy.md](game_duplicate_class_taxonomy.md) -- FOUR duplicate-`games` classes (not one defect); 2 are already fixable by `bb data merge-duplicate-games` (unrun, not a gap); `last_scoring_update` is VESTIGIAL (= `start_ts or end_ts`, so the input IS DB-recoverable); **`US/Central`/`US/Pacific` DO NOT RESOLVE in this image and `derive_local_date` silently returns the UTC date -> 9 wrong `game_date`s, and this CREATES date-split twins**; "identical scores + play ratio >=0.85" beats a start-time window (0 false positives/season); same-perspective merge is a new primitive, not a loosened refusal. (E-278 planning.)
