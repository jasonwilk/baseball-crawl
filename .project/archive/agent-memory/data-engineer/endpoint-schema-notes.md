...
- **Top-level keys, classified by IDENTITY not shape** (corrected 2026-08-02, narrowed 2026-08-03): the response is a JSON object with exactly 2 keys (one per team). Each key is a `public_id` slug or a UUID, and the form does NOT mark which side it is -- both keys are slugs on a minority of payloads, and what decides the form is unestablished ("slug iff the team has a public GC presence" was refuted 2026-08-03). The parser matches a key against our own `public_id`, then our `gc_uuid`; the remaining key is the opponent's. A dashes/no-dashes regex must NOT decide the split -- doing so reported "no opponent key" on all-slug payloads and discarded the opponent's whole envelope. **A UUID-form opponent key is a per-account local `root_team_id`, NOT the opponent's canonical `gc_uuid`** -- never persist it into `teams.gc_uuid` (the E-211 contamination path).
...
- **Position history**: `player_text` in lineup group encodes positions played (e.g., `"(SS, P)"`, `"(2B, P, 2B)"`). In pitching group, encodes decision (`"(W)"`, `"(L)"`, `"(SV)"`, `""`).
...
- **Same team key format as boxscore**: `team_players` top-level keys are per-team slug-or-UUID, not a side marker. Reuse the boxscore parser's identity-based detection, never a shape check.
...
- **Player entity mapping**: Each roster record maps directly to a `Player` entity row. The roster endpoint is the authoritative source for player-to-team membership (the `team_rosters` junction, keyed by season_id). Ingesting a roster = upsert `Player` + insert `team_rosters`.
