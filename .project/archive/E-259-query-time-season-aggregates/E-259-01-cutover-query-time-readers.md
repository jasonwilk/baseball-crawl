...
- [ ] **AC-3** (AMENDED 2026-07-12 — user-approved; the literal zero-diff premise was FALSE on the current fixture — see the ruling recorded in Technical Notes §1): Given the ORDER BY clauses, when this story is complete, then they reproduce the prior ordering semantics over the new per-game-SUM projection (batting: PA-proxy `(ab+bb+hbp+shf) DESC, last_name ASC`; pitching: `ip_outs DESC, last_name ASC`), now as expressions over the SUM projection rather than stored columns (Technical Notes §8). `tests/test_report_golden.py` passes with the golden's **`batting` and `pitching` season arrays regenerated ONCE via `scripts/regen_report_golden.py`** — a sanctioned, git-diff-visible, review-gated correction of a pre-existing fixture inconsistency (`tests/fixtures/seed.sql`'s stored `player_season_*` rows are NOT the SUM of its `player_game_*` rows; the committed golden encoded the stale stored values while its own `pitcher_profiles`/`pitching_workload`/`pitching_history` sections already read per-game). **Bounded + verified:**
  - The regen diff MUST be **confined to the top-level `batting` and `pitching` season arrays.** Any change to ANY other golden section (`pitcher_profiles`, `pitching_workload`, `pitching_history`, `plays_*`, `recent_games`, `record`, `roster`, `spray_charts`, `team_info`, `freshness`, `runs_avg`) is a **MUST FIX** — those sections do not read the stored season tables and the cutover must not perturb them.
  - Each regenerated season value MUST equal the **per-game SUM** for that player (perspective-filtered), **independently hand-reproducible from `seed.sql`** — NOT an arbitrary regenerated number (worked proof: PLAYER_VARSITY_01 pitching `3/54/22` stored → `4/69/26` summed, `18+21+15+15` outs across GAME_001/003/005/007). Any batting **row-order** shift MUST reflect the summed PA-proxy `DESC` (§8), not an ORDER BY reproduction bug.
  - **No import edits, no test-LOGIC edits** — the §1 "may-not-touch-the-golden-TEST" invariant is intact; only the golden DATA is regenerated via the path the test itself documents (`test_report_golden.py:31`). The epic-invariant "may not touch the golden to make itself pass" is **SATISFIED-BY-CORRECTION**, not violated: the golden is corrected to the TRUE season values through the sanctioned review-gated path, retiring a stale stored-vs-summed disagreement — not an expectation hand-edited to launder a behavior change.
