...
## Description
After this story is complete, the opponent-discovery vertical and the remaining dead admin-support code are gone: `crawlers/opponent_resolver.py`, `loaders/opponent_seeder.py`, `src/db/merge.py`, and the now-dead opponent helpers in `src/api/db.py` are deleted. Additionally, `src/gamechanger/config.py` and `config/teams.yaml` — whose deletion was gated and DEFERRED from E-239-04 (that story's AC-3 importer gate fired because `crawlers/opponent_resolver.py` imports `config.py` at module level, and `config.py` is the sole reader of `config/teams.yaml`) — are deleted here, after AC-1 removes their last importer (`opponent_resolver.py`). The Epic-E-reserved `src/gamechanger/resolvers/gc_uuid_resolver.py` and (by default) `team_resolver.py` are explicitly preserved despite having zero importers after D2.
...
- [ ] **AC-1c** (config carry-forward from E-239-04 AC-3 gate): `src/gamechanger/config.py` and `config/teams.yaml` are deleted in this story, AFTER AC-1 removes `crawlers/opponent_resolver.py` (config.py's last surviving importer — the `config.py:63`-equivalent module-level `from src.gamechanger.config import CrawlConfig, TeamEntry`). Re-run the AC-3-style grep before deleting: confirm `config.py` has zero remaining importer across `src/`, `scripts/`, `tests/` once `opponent_resolver.py` is gone, then delete both `config.py` and its sole data file `config/teams.yaml` together. If any unexpected surviving importer remains, do NOT delete and flag to PM.
...
- DELETE `src/gamechanger/config.py` and `config/teams.yaml` (carried forward from E-239-04 AC-3 gate; only after AC-1 removes `opponent_resolver.py`)
...
- DELETE `tests/test_config.py` (carried forward from E-239-04 AC-5; deleted once config.py is gone in AC-1c)
