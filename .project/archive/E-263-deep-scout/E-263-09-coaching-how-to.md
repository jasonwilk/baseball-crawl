...
- [ ] **AC-3**: The doc explains the probable-starter conditioning in plain terms — that the game plan is built around the arm expected to pitch (and what the "committee" state means), and that the catching card is plays-derived (a scouting estimate, distinct from official box-score stats) per Technical Notes TN-7. It also explains, in plain coaching terms, that pitcher availability depends on the competition level selected when the report is generated (High School Varsity / High School JV–Reserve–Freshman / American Legion), that the season phase is figured automatically, and that when the level is left unset the availability calls fall back to a general youth-arm guideline and "may be off by a day at the margins" (per Technical Notes TN-11) — so a coach reads the exact-vs-guideline accuracy sentence correctly.
