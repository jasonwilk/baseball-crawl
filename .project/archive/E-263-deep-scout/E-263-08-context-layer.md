...
- [ ] **AC-3**: `CLAUDE.md` Commands section documents that `bb report generate` now emits the Deep Scout sections and gains the operator-selected competition-level input (the eligibility-gate flag + admin dropdown, per Technical Notes TN-11 / E-263-02c) that drives SIG-001's rest-rule table; the report stays opponent-only (no `--vs`/`--date` matchup seam — a v2 item), consistent with what actually shipped.
...
## Technical Approach
claude-architect designs the exact wording and placement (per the context-layer-epic convention: CA leads design in their domain, PM frames ACs). Read the shipped modules (`src/reports/deep_scout/`), the shipped command surface (opponent-only, plus the new operator-selected level flag + admin dropdown from E-263-02c per Technical Notes TN-11), and the design doctrine (`.project/research/deep-scout-design-2026-07-12.md` §5 ethics, §6 fact-sheet). Model the new `scouting-ethics.md` on the existing safety-rule shape (`.claude/rules/pii-safety.md`). Verify against the actual shipped code, not the plan — the schema filename is whatever E-263-02a landed.
