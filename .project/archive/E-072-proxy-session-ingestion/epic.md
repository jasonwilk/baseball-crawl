...
**Abandonment rationale**: The epic's premises all survive -- `proxy/addons/*.py`, `src/cli/proxy.py`, the `ingest-endpoint` skill, and exactly 120 files in `docs/api/endpoints/` (matching this epic's own "119 endpoints + `web-routes-not-api.md`" count) are all present and unchanged. Nothing was deleted out from under it. It is abandoned on **value and cost**, not on invalidated premises:

1. **No roadmap slice.** This is a nice-to-have automation, not a `docs/ROADMAP.md` §5 slice. The reports-first reframe neither killed it nor called for it.
2. **No pull in ~4.5 months.** New-endpoint discovery volume has not justified the skill since 2026-03-13.
3. **An unpriced cost that did not exist when this was planned.** The context-layer assessment's trigger 7 now gates `.claude/skills/**` line growth against a committed baseline (`.project/baselines/context-layer-ratchet.json`), and that baseline is already carrying inherited drift. E-072-01 is a very large single SKILL.md, so it would arrive needing an operator-signed ratchet exception on top of an already-stale baseline -- a real new cost against unchanged value.

**Extraction before abandonment**: the reusable fragments -- the path-normalization ruleset and the data-first insight (the richer endpoint-log carries `response_body`, so most endpoints need no curl recapture) -- were captured as **IDEA-192** on 2026-07-26. IDEA-192 also supersedes E-072 as the unblock pointer in IDEA-014, which listed this epic as a precondition; IDEA-014 was annotated in the same pass.

**Superseded stale-READY triage note (2026-07-08, retained as history)**: This epic sat READY ~99–122 days with no dispatch pull. Its premise (a skill to bulk-process a mitmproxy session into documented endpoints + raw captures) is NOT invalidated by the reports-first reframe — the proxy/ingest tooling still exists — but it is a nice-to-have automation, not a ROADMAP slice, and new-endpoint-discovery volume has not justified pulling it in 3-4 months. Demoted to DRAFT per the stale-READY rule (READY >60 days → re-confirm against ROADMAP or demote). Re-promote to READY if proxy-session ingestion volume grows enough to justify the skill; otherwise it can be discarded at a future review. Content preserved; no re-refinement done here.
...
## History
- **2026-07-26: ABANDONED** by operator ruling on a PM triage of the three oldest epics in `/epics/`. Not implemented; no story was ever dispatched, so the epic produced zero changes to `src/`, `tests/`, `migrations/`, `docs/`, or the context layer. Rationale in the Status section above (no roadmap slice; no pull in ~4.5 months; a large new SKILL.md would need an operator-signed context-ratchet exception). Reusable fragments extracted to IDEA-192; IDEA-014's dangling E-072 unblock pointer annotated in the same pass.

  **Documentation assessment**: No documentation impact. The epic shipped nothing, so no update trigger in `.claude/rules/documentation.md` fires. (Its own Non-Goals already excluded all application and HTTP-layer code.)

  **Context-layer assessment** (eight triggers per `.claude/rules/context-layer-assessment.md`, evaluated individually rather than blanket-waived; every verdict is **no** for the same structural reason -- an abandoned epic that produced no artifact cannot have established, discovered, or changed anything):
  1. New convention, pattern, or constraint established -- **no**. None; nothing was built.
  2. Architectural decision with ongoing implications -- **no**. The "new skill vs. evolve `ingest-endpoint`" decision recorded in Technical Notes was never acted on, and IDEA-192 deliberately reopens it as an open question rather than inheriting it.
  3. Footgun, failure mode, or boundary discovered -- **no**. The two security boundaries this epic documented (web access tokens are never persisted to `.env`, so a web curl template would require displaying a live `gc-token`; never save `request_headers` from the richer endpoint-log) were pre-existing facts it restated, not discoveries. Both are carried forward in IDEA-192 so they are not lost with the archive.
  4. Change to agent behavior, routing, or coordination -- **no**.
  5. Domain knowledge that should influence future agent decisions -- **no**. The endpoint-log format details and normalization rules are API-workflow reference material, correctly homed in IDEA-192 rather than promoted to a load target.
  6. New CLI command, workflow, or operational procedure introduced -- **no**. The `ingest-session` skill was never created; `CLAUDE.md`'s Workflows section and the `/workflow-help` cheat sheet were never touched and need no reconciliation.
  7. Net context-layer growth ratchet -- **no growth**. The epic adds nothing to the four gated subtrees. Recorded for the record because this trigger is the abandonment's third rationale in reverse: the growth it *would* have caused is part of why it is being abandoned rather than dispatched. **This is not a ratchet offset and must not be cited as one** -- declining to grow the layer is not the same as reducing it, and the inherited baseline drift remains an open operator obligation, untouched by this closure.
  8. Reusable behavioral lesson surfaced -- **no**. One process observation (a spec can be fully refined, review-clean across four passes, and still not be worth building) is a triage judgment, not a promotable behavioral lesson, and it cites no defect it demonstrably caught. Trigger 8's promotion gate is deliberately not met.

  **Deletion-side eviction**: E-072 deleted and renamed nothing and retired no claim, so there is no reference surface to sweep. The one live cross-reference *into* this epic (IDEA-014's unblock pointer) was reconciled rather than struck, per reconcile-not-strike.
