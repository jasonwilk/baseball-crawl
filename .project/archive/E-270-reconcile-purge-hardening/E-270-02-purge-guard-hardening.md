...
## Status
`DONE`
<!-- DONE 2026-07-24. PM AC verdict 8/8, code-reviewer APPROVED 8/8, no MUST FIX.
     AC-5 was AMENDED mid-review (the resolved-path requirement) and then
     re-verified against the amended text, not the text it originally passed.
     Four findings closed: PM's fabricated test citation, plus three cr SHOULD
     FIXes. UNBLOCKS E-270-06, whose AC-4 must verify against the MERGED code
     (`src/cli/db.py`, `src/db/purge_scouting.py`) rather than this story's
     prose — several shipped details are not in the story text. -->
<!-- was: IN_PROGRESS -->
...
- [ ] **AC-5**: Given a real (post-confirmation) purge, when it runs, then `backup_database` is invoked FIRST and, if it raises for any reason, the purge is ABORTED with a non-zero exit and a clear message before any DELETE (the library's `BEGIN IMMEDIATE` is never reached). **The argument MUST be the RESOLVED path (`preview.resolved_path`), not the raw `--db-path` value** (amended 2026-07-24 during dispatch; this AC originally said `backup_database(db_path=db_path)`, which is the weaker and actually unsafe form). The two functions resolve differently: `backup_database` uses its argument AS-IS when non-None (`src/db/backup.py:39-43`), while `purge_scouting_data` routes through `resolve_db_path(db_path)`. So on a RELATIVE `--db-path` the raw form would snapshot a cwd-relative file while the purge emptied the repo-root-relative one — a backup of the wrong database, on the most destructive command in the system, discovered only when someone tried to restore. Passing the resolved path makes "the backup and the purge targeted the same file" true by construction, and it is the same path the operator was shown at the AC-2 prompt. A test must pin the argument (not merely that a backup happened). Sequence and rationale per TN-5(d) (backup after confirm, fail-closed). NOTE for the test author (data-engineer): `backup_database`'s DESTINATION is hardwired to `<repo_root>/data/backups/` via `parents[2]`, independent of `db_path` — so the AC-5 test MUST monkeypatch `backup_database` (both to assert call-ordering / that a raise aborts before the purge AND to prevent a CliRunner test writing a real snapshot into the repo's `data/backups/`).
...
## Notes
No migration. `validate_app_env()` (`src/api/helpers.py`) raises `RuntimeError`; `backup_database(db_path=None) -> Path` (`src/db/backup.py`) uses the SQLite online backup API and raises fail-closed on an absent/locked DB. That `None` is the function's SIGNATURE default, not the argument this story passes — AC-5 requires the RESOLVED path, because `backup_database` uses a non-None argument as-is while `purge_scouting_data` re-resolves it. This story folds audit item 5(c); E-270-05 carries only items 5(a)/5(b) to keep `src/cli/db.py` edited by one story.
