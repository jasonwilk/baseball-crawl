...
## Status
`DONE`
<!-- DONE 2026-07-24. PM AC verdict 4/4. Context-layer-only story (CLAUDE.md,
     no Python), so PM verified alone per the dispatch process; code-reviewer
     picks it up at the unconditional Closure CR Integration Review over the
     combined diff, where the doc-sweep rule auto-loads for context-layer
     changes. PM scope ruling: the non-overridable `validate_app_env()` typo
     guard is KEPT in the paragraph though AC-4's enumerated list does not name
     it -- omitting it would document `--force` as more permissive than it is,
     reintroducing an AC-1-class understatement one sentence away. -->
<!-- was: IN_PROGRESS -->
...
## Dependencies
- **Blocked by**: E-270-02 — **SATISFIED 2026-07-24** (E-270-02 is DONE; this story is now dispatchable). AC-4 reconciles the paragraph to the purge behavior E-270-02 shipped, so the doc must describe what actually shipped. Verify against the MERGED code (`src/cli/db.py`, `src/db/purge_scouting.py`), NOT E-270-02's story prose — several shipped details are absent from the story text, including `preview_purge`'s fail-closed raise on a missing database and the fact that the auto-backup receives the RESOLVED path.
- **Blocks**: None
