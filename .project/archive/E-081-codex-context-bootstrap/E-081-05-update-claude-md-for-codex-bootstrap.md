...
## Acceptance Criteria
- [x] **AC-1**: `CLAUDE.md` contains a concise section describing the checked-in Codex layer (`AGENTS.md`, `.codex/config.toml`, `.agents/skills/`) and the gitignored runtime layer (`CODEX_HOME`, local auth/session/history/cache artifacts).
- [x] **AC-2**: `CLAUDE.md` states that the default Codex workflow uses a project-local `CODEX_HOME` inside the workspace and that host `~/.codex` mapping is optional, not required.
- [x] **AC-3**: `CLAUDE.md` points readers to `docs/admin/codex-guide.md` for the full operator-facing setup and smoke-check instructions.
- [x] **AC-4**: `CLAUDE.md` does not describe spawned agents, `multi_agent`, or other experimental Codex features as part of the baseline Codex lane.
- [x] **AC-5**: The new Codex guidance in `CLAUDE.md` reflects implemented file paths and behavior only; it does not speculate about future Codex hardening work.
...
- [x] `CLAUDE.md` reflects implemented Codex reality only
- [x] The Codex guidance is concise and points to the operator guide for detail
