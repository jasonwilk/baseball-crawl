...
## Acceptance Criteria
- [ ] **AC-1**: A function in `src/reports/positioning_card.py` (module placement locked per CR M3) takes `(opponent_id, position, conn)` and returns an SVG string (or appends to a template render context) for a single position's field diagram. **SVG aspect ratio + dimensions + all stroke/font specifics are consumed from the locked-constants artifact at `/.project/research/E-229-locked-layout-constants.md` §B (produced by E-229-2b, amended to v1.2 with the field-anchored `Coordinate-space rescaling (normative)` subsection)**. This story does NOT name specific numeric values for those constants — they come from the artifact at impl time. The function queries `team_position_aggregate` and `spray_charts` directly per epic TN-7 JOIN patterns. **Consumes §B v1.2's field-anchored mapping**: engine output `team_position_aggregate.(star_x, star_y)` lives in `src/charts/spray.py::_raw_to_svg`'s 320×480 GC canonical coord space with home plate at engine (160, 295); the per-card 200×320 viewBox places the field outline in the inner rectangle (10, 80) — (190, 305) (top 80 y-units = header zone; bottom 15 y-units = legend zone). The generator MUST apply the artifact's field-anchored formula (`x_card = 10 + (x_engine / 320) * 180; y_card = 80 + (y_engine / 295) * 225`) at the read-then-place boundary so that engine field-coordinates align to the card's painted field outline. Sign rules carry through unchanged (TN-15 y=0-at-deep-CF convention holds in both spaces). The engine is NOT modified — rescaling is a consumer responsibility per the §B normative subsection.
