...
|----|-------|--------|-------------|----------|
| E-229-01 | Migration 002 v2 — schema rewrite in place | DONE | None | data-engineer |
| E-229-02 | Engine rewrite — team-aggregate centroid + per-batter deviation + atomicity | DONE | E-229-01 | software-engineer |
| E-229-2b | Quarter-letter layout feasibility prototype | DONE | E-229-02 | ux-designer |
| E-229-03 | Card field SVG generator — star, textbook dot, compass letter ring, density background | DONE | E-229-02, E-229-2b | software-engineer |
| E-229-04 | Outlier batter pills — jersey lookup + collision jitter | DONE | E-229-03, E-229-2b | software-engineer |
| E-229-05 | Compact card template — quarter-letter 4-up geometry + coach design review | DONE | E-229-04, E-229-2b | software-engineer |
| E-229-06 | Coach prep page — full-field overlay, all 6 positions | DONE | E-229-03, E-229-04, E-229-2b | software-engineer |
| E-229-07 | Coach call sheet — jersey × position matrix, alphabetical sort | DONE | E-229-02, E-229-2b | software-engineer |
| E-229-08 | Bundle generation — 4-page mixed-orientation PDF + coverage-cue snapshot + LLM render-time threading | DONE | E-229-05, E-229-06, E-229-07, E-229-09 | software-engineer |
| E-229-09 | Tier 2 LLM input contract — render-time threading, no DB persistence | DONE | E-229-02 | software-engineer |
| E-229-10 | Pipeline wiring — standalone path + scouting path + dashboard link | DONE | E-229-08, E-229-09 | software-engineer |
...
> **Amendment (E-229-2b, 2026-05-17 — LOCKED v1 supersedes PROVISIONAL v0).** `COMPASS_LEGEND_SHORT` above replaces the original PROVISIONAL text `★ team default · ○ textbook · A–H = outlier zones (see right)`. At 6.5 pt (the size UXD self-iterated to in order to fit the original text on one line) the legend failed coach AC-12 — below TN-16's 7 pt dugout-glance floor, and 70% grey ink in dugout shade compounds the legibility cost. Coach pre-approved the shortened Option 1 form, which fits one line at the 7 pt floor without compressing typography or wrapping into the body zone. Full path (PROVISIONAL → 6.5 pt → coach FAIL → Option 1) recorded in `/.project/research/E-229-locked-layout-constants.md` §F amendment note + Decisions Log entry "Legend text + typography". `COMPASS_LEGEND_LONG` is unchanged by this amendment. Implementations MUST consume from the locked-constants artifact, not from any prior TN-3 prose snapshot.
...
The calibration pass also has a **third job**: validate that the LOCKED v1 §B NULL-jersey fallback rendering (`(L. init)` form — e.g., `(R. init)` for a Ramirez whose roster jersey is NULL) survives real coach review when an opponent surfaces a jersey-NULL roster row. The PROVISIONAL → LOCKED v1 coach-AC-12 review used a fictional opponent (Eastlake Bears) whose 5 outlier batters all had jerseys assigned, so this fallback path was NOT exercised at print review. The artifact's rationale ("the literal token `init` signals to the coach that this is the initial-only form") is defensible but unverified at coach-eye level. If coach feedback during calibration flags the literal-token rendering as confusing, the fix lives in the LOCKED v1 §B fallback spec + the `_PillData.text()` helper in `src/reports/positioning_card.py` — no story-AC re-litigation is needed because E-229-04 AC-2 was retro-fitted to cite the artifact rather than name the format inline.
...
- 2026-05-17: **Prior dispatch attempt — abandoned mid-flight due to environment loss (epic worktree filesystem deleted before closure commit).** Implementation work lost; planning-artifact corrections salvaged and baked into the spec before re-dispatch. Status held at READY. Lessons preserved in the spec:
  - **E-229-02 AC-10(f) extended** to name three inline fixture-builder locations and their column-set update requirements: `_make_positioning_row` at `tests/test_report_renderer.py:1480`, `_make_result` at `tests/test_positioning_llm.py:43`, and bare-SQL INSERTs at `tests/test_report_generator.py:~2940/~2956`. Files-to-Modify extended to cover them. Adds an explicit "scoping rule for assertion content" (update / xfail-mark / delete) and a hard constraint that the suite must collect cleanly post-E-229-02.
  - **E-229-2b LOCKED v1 artifacts committed** at `/.project/research/E-229-locked-layout-constants.md` (v0 → v1) and `/.project/research/E-229-2b-quarter-letter-prototype.html`. **TN-3 amendment applied**: `COMPASS_LEGEND_SHORT` changed from `★ team default · ○ textbook · A–H = outlier zones (see right)` to `★ default · ○ textbook · A-H outliers`. Flip path: UXD drafted PROVISIONAL constants → self-iterated legend to 6.5 pt to fit one line at original text length → AC-11 self-validation flagged "below TN-16 7 pt floor; may not survive coach review" → coach AC-12 verdict FAIL on the 6.5 pt legend (other elements PASS) → coach pre-approved Option 1 (shortened text + 7 pt) → UXD applied the bump to the prototype HTML + constants spec + flipped frontmatter to LOCKED v1. Three callsite updates applied: epic TN-3 (canonical), E-229-03 AC-6 (legend rendering), E-229-03 Technical Approach code block (constant definition). E-229-2b story stays TODO — re-dispatch UXD validates against the pre-existing artifact rather than rebuilding from scratch; coach review pattern AC-11 + AC-12 record on the story file stands as evidence of prior convergence and may be cited by the re-dispatch coach review or re-run.
  - **E-229-04 AC-2 NULL-jersey fallback** example replaced with citation to LOCKED v1 §B "Outlier pill — NULL-jersey fallback" (canonical source supersedes any inline example).
  - **Gap items added to story files** (not in prior dispatch's PM edits; captured here for re-dispatch): E-229-02 AC scope expanded to delete superseded `tests/test_positioning.py`; E-229-05 scope clarified to (a) own deletion of the `POSITIONING_CALL_WORDS` vocabulary block in `src/reports/renderer.py` lines 67-109 (deferred from E-229-02 per AC-10(f)), (b) clear xfail markers citing `E-229-05` in test_report_renderer.py and test_report_generator.py, (c) AC-8 coach review must evaluate print-ready output, not browser preview.
...
- 2026-05-17: **Status → ACTIVE. Dispatch begun (second attempt).** Re-dispatch on fresh worktree at `/tmp/.worktrees/baseball-crawl-E-229/` branching from `epic/E-228-defensive-positioning-cards` HEAD per TN-1 mandatory branch implementation. Epic worktree filesystem rebuilt; no in-flight state from the abandoned prior dispatch (session `72e8fdeb`) persists. All planning-artifact learnings from the prior attempt are already baked into the spec (per the 2026-05-17 "Prior dispatch attempt — abandoned" entry above). First story IN_PROGRESS: E-229-01 (Migration 002 v2 — schema rewrite in place), assigned to data-engineer. Closure target remains `epic/E-228-defensive-positioning-cards`, NOT main; merge-to-main is a separate, user-gated decision after local dev validation on the combined E-228+E-229 branch HEAD ("MIGHT merge to main only after validation" — user verbatim at READY transition).

- 2026-05-18: **E-229-08 R2 voluntary front-loading of v1 cleanup acknowledged.** During E-229-08's R2 remediation pass (the integration-into-`generate_report()` work), SE voluntarily took the orphaned v1 call-site cleanup that was originally deferred to E-229-10's scope per the E-229-09 deferral pattern: deleted `from src.reports.positioning_llm import enrich_positioning` and the v1 call-site loop body at `src/reports/generator.py:1217-1244` (the loop that accessed retired `result.team_state_call`). Also cleared the `TestTier2LLMWiringInGenerateReport` xfail class from `tests/test_report_generator.py` (no longer needed; v1 wiring is gone). This is why E-229-10's AC-6 grep finds zero hits without E-229-10 having to do the cleanup work — it was done one story earlier, in the same git touch as the integration. SE's reasoning: the cleanup was geographically owned by `generate_report()` (which SE was already modifying for AC-3), so doing it in the same touch was cheaper than splitting it across stories. Recorded here so future readers can trace why E-229-10's scope ended up lighter than the original spec implied.

- 2026-05-18: **Pre-closure codex review (chained from /implement with review modifier) — 3 findings, all FIXED.**
  - **F1 (P1)**: Cards sheet overflowed printable area by 0.5" — `@page positioning-cards` margin set to `0` per LOCKED v1 constants §A.
  - **F2 (P1)**: Page-4 slots 3 & 4 (compass-key + opponent-context-card) rendered empty because bundle never threaded `compass_key_svg` / `opponent_context_*` kwargs. Added `render_compass_key_svg()` to `positioning_card.py` + `_build_opponent_context()` + `_TIER_COPY` + 7 small per-stat query helpers to `positioning_bundle.py`; threaded 4 kwargs into renderer. Slot-fill lock from Phase 4 iteration 2 P2.4 now actually realized in render output.
  - **F3 (P2)**: `_query_cards_positioning_rows` hard-filtered `perspective_team_id = team_id` while three sibling renderers used prefer-then-fallback — caused cards section to disappear for opponents with foreign-perspective-only data. Extracted shared `_choose_perspective_team_id()` helper; cards query AND opponent-context payload both consume the same chosen perspective for consistency.
  - **Test discipline lesson**: pre-existing bundle tests asserted slot HEADERS but not slot CONTENTS — that's why F2 escaped. SE added 16 new tests asserting body content (compass-key SVG with all 8 letters + 4 axis labels, 4 stat rows with exact computed values, tier line with all three classification branches, coverage-line degradation cases). Codified for future bundle-style stories: slot-fill ACs must require a content-level assertion, not just a header-presence assertion.
  - **CR APPROVED** 5/5 per-finding (F1, F2a, F2b, F2c, F3); 8/8 specific checks VERIFIED; targeted set 312 → 328 (+16); full suite 4733 → 4749 (+16); regressions held flat.

- 2026-05-18: **Documentation impact assessment complete** (per `.claude/rules/documentation.md`): 3 of 5 triggers fire (new feature, schema changes, system-behavior change). docs-writer dispatched and completed updates to:
  - `docs/coaching/scouting-reports.md` — Defensive Positioning section fully rewritten for E-229 compass-zone model (4-page bundle, star/outlier concept, A-H zones, call sheet/prep page/player card reading, coverage tiers, compass key + opponent context cards). All E-228 categorical vocabulary (STRAIGHT UP, SHADE LEFT/RIGHT, MIXED, L/R Sh/Dp symbols) retired.
  - `docs/admin/architecture.md` — schema entry updated to migration 002 v2 (deviation-based columns, zone_id); Defensive Positioning Engine section rewritten for team-aggregate star model with sign-rule table.
  - `docs/admin/operations.md` — `bb data scout` step count updated 6→7; Defensive Positioning Bundle section rewritten with 4-page contents table, regenerate_positioning_bundle helper documented, LLM ephemerality noted, calibration constants updated (E-228 categorical constants removed; POSITION_RANGE_SCALE added).
  - Categorical vocabulary retired across all three docs (call_state, team_state_call, direction_shade, depth_shade, zone_concentration, MIXED, POSITIONING_CALL_WORDS, POSITION_RESPONSIBILITY_SECTORS, MIXED_NON_ADJACENT_PAIRS_THRESHOLD, BIP_DEPTH_THRESHOLD, etc.). One open Q for calibration pass: verify `POSITION_RANGE_SCALE` constant name matches the implementation in `src/reports/positioning.py` (docs-writer cited it conceptually per the epic spec; exact identifier may differ).

- 2026-05-18: **Context-layer impact assessment complete** (per `.claude/rules/context-layer-assessment.md`): six triggers evaluated, four fire. **T1 YES** — bundle_snapshot.json sidecar pattern, prefer-then-fallback perspective query shape, and slot-fill content-assertion test discipline codified to `architecture-subsystems.md`, `perspective-provenance.md`, and `testing.md` respectively. **T2 YES** — atomic dual-table write invariant (TN-6) and LLM render-time threading (CR B1) codified to `architecture-subsystems.md`; field-anchored SVG coord mapping (TN-15) codified to the new `positioning-vocabulary.md` rule (numeric constants remain canonical in `.project/research/E-229-locked-layout-constants.md` v1.2). **T3 YES** — bundle-test-content gap (F2 escape) and coverage-cue degradation recurrence (E-228 Phase 4b → E-229 snapshot fix) codified to `testing.md` and `architecture-subsystems.md`. **T4 NO** — no agent ecosystem, routing, or closure-sequence changes. **T5 YES** — 8-zone compass vocabulary + sign table, SVG coord convention (y=0 at deep CF), coverage tier semantics, and "in/deep" axis lock codified to the new `.claude/rules/positioning-vocabulary.md` (scoped to positioning files) and to baseball-coach memory's new `positioning-domain.md` topic file (coaching-facing knowledge). **T6 NO** — `bb data scout` stage 7 was added in E-228 Phase 4b; E-229 only modified what gets rendered, not the pipeline shape. Zero lines added to CLAUDE.md.

- 2026-05-18: **Status → COMPLETED.** All 11 stories DONE (E-229-01, E-229-02, E-229-2b, E-229-03, E-229-04, E-229-05, E-229-06, E-229-07, E-229-08, E-229-09, E-229-10). Code-reviewer APPROVED all gated stories across multiple rounds; staging boundary advanced cleanly story-by-story per the implement skill's per-story `git add -A` protocol. Cumulative diff at closure: **39 files changed, 11104 insertions(+), 3937 deletions(-)**. The combined E-228 + E-229 branch HEAD now carries the full team-aggregate defensive positioning system: v2 schema (migration 002 rewritten in place), team-aggregate centroid engine, locked layout constants artifact (v1.2 field-anchored Coordinate-space rescaling), per-position field SVG generator, outlier batter pills with collision jitter, compact card template (quarter-letter 4-up portrait), coach prep page (landscape full-field overlay), coach call sheet (landscape jersey × position matrix), Tier 2 LLM rationale contract (render-time threading, no DB persistence), 4-page mixed-orientation bundle with coverage-cue snapshot, and the three pipeline surfaces (standalone CLI, scouting auto-bundle, dashboard link). Closure merge target: `epic/E-228-defensive-positioning-cards` per TN-1 mandatory branch implementation; NOT main. Merge-to-main is a separate, user-gated decision after local dev validation on the combined E-228+E-229 branch HEAD per the READY-transition lock ("MIGHT merge to main only after validation"). Documentation + context-layer impact assessments to follow per `.claude/rules/documentation.md` + `.claude/rules/context-layer-assessment.md` before archive.
