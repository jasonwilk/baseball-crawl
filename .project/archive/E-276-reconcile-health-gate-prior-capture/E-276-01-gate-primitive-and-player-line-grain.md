...
## Status
`DONE` — 2026-07-26. **Both gates cleared**: code-reviewer APPROVED at round 2, and all **15** acceptance criteria verified by PM. Two review rounds plus a prose pass; unblocks 02, 03, 04 and 05.

**⛔ AC-12's justification defect is CLOSED — do not re-open it as outstanding.** SE's completion report named it *"the only open item, PM's to rule on"*, which was true when written and was overtaken the same day: PM ruled and **corrected it at source**. AC-12's false sentence (*"an unconditional vacuous-permit would make an empty roster payload read as authoritative"*) is replaced with the true weaker reason, and **AC-6 gained a pointer SE never saw**, because it grounds the opt-in partly on the same now-inert roster exposure. **The requirement never changed and was satisfied throughout; only the stated reason was false.**

**Three spec corrections landed during this story, all in the artifacts rather than in review threads** — AC-13's `18 of 18` illustration (unreachable figure, and it named a branch that input cannot reach), two TN-9 rows plus a fourth-site fold, and the AC-12/AC-6 justification above. **A later reader should treat this file as current and any relayed summary of it as stale.**

*(Historical, kept because it records a real pre-dispatch correction: AC-15's operative sentence was corrected at the dispatch flip — it had mandated the cross-run accumulation signature the R1 disposition withdrew.)*
...
- [ ] **AC-2 (which-mechanism-refused — binding, per TN-11)**: The refusal assertion MUST identify which mechanism refused, asserted on a **structural record carried by the result dataclass rather than on WARN prose** — a test that greps log text passes when someone rewords the message. Assert `refused_by == "gate"`, `gate_permitted is False`, and that **the gate's prior count equals the pre-run population (9), not the post-upsert population (18)**. That count is the numeric tell; a surviving-row count alone can pass post-fix for a wrong reason.
      ...
      **The generalization worth carrying, because it is this epic's own subject pointed at its own fix**: the corrected gate's failure mode is **quieter than the bug it replaces.** The original defect at least left a checkable `9-of-18` in the log — a denominator an operator could compare against the roster; a fail-open empty snapshot leaves a plausible `0-of-0` and permits. *(This read "an implausible `18-of-18`" until 2026-07-26 — see AC-13's correction note. The contrast survives the fix and in fact sharpens: `9 of 18` is not implausible on its face, which is why the defect went unnoticed in production logs — the tell is the DENOMINATOR being twice the roster, not the ratio looking odd.)* **A gate that fails open is worse than the defect it corrects, because it looks like a gate.**
...
- [ ] **AC-6**: The shared authority check implements the vacuous-permit rule of Technical Notes TN-1(c) — an empty protected population yields the fetch-ok value rather than a refusal — and **this grain's gate is the corrected gate ALONE**, computing the floor ratio over the **pre-upsert snapshot** population, per TN-1(b). The legacy live-population gate is **replaced, not conjoined**. Both the vacuous-permit rule and the corrected gate are covered by direct tests at the primitive level.
      ...
      **⚠️ Read "protects the roster grain's shared fetch-ok use" at its true strength — see AC-12's correction note** *(pointer added 2026-07-26)*. The sharing is real and the opt-in requirement stands, but the protection is against a **future** edit, not a live hazard: an unconditional rule would **not** make an empty roster payload authoritative today, because `fetch_ok` is checked first and roster passes `fetch_ok=bool(fresh)` (executed). **The load-bearing half of the reason above is the pinned assertion, not the roster exposure.** This pointer exists because a reader who tests the roster half and finds it inert may conclude the whole requirement is unfounded — which is exactly the failure AC-12's note records.
...
- [ ] **AC-12 (suite green — scoped to THIS STORY, which is what previously went unstated)**: `python -m pytest tests/` reports 0 failed. **As of this story's completion**, the **72 tests in the three grain files** keep every assertion unchanged, with mechanical keyword-argument churn at the 9 direct call sites listed in TN-13.
      ...
      **The sibling is the load-bearing half, and it is the one an implementer will skip** as redundant with the flipped test. It is the only executable guard on the property AC-6 exists to protect: `crawl_is_authoritative` is **shared**, so a grain that never asked for the rule must not silently acquire it. If the default-off assertion is lost, the next edit that "simplifies" the opt-in away lands unnoticed and every test still passes. Same reasoning as AC-7: the property this protects is the one a future edit restores by accident, which is why it needs a check that fails rather than a note that was true once.

      **⛔ THE JUSTIFICATION ABOVE USED TO BE STRONGER AND IT WAS FALSE. Corrected 2026-07-26 — do NOT restore it.** The struck sentence read: *"An unconditional vacuous-permit would make an empty roster payload read as authoritative there."* **It would not** [EXECUTED by SE at story 01 round 2; PM re-verified at the call site]. `fetch_ok` is checked **first** and returns `False` before the rule is ever reached, and the roster grain passes **`fetch_ok=bool(fresh)`** — so an empty roster payload refuses on that conjunct with or without the parameter. The rule changes the answer on **exactly one input**: `fetch_ok=True` with `prior_count == 0`, which no grain but player-line reaches today, and player-line reaches it **by design**.

      **The true reason is weaker, and it is enough**: default-off protects the shared callers from a *future* edit, not from today's code. **Keep the sibling test on that ground.**

      **⚠️ WHY THIS MATTERED MORE THAN A WRONG SENTENCE USUALLY DOES.** The requirement was correct and satisfied throughout; only the reason was false. But **this particular reason is the only thing standing between the sibling test and an implementer who finds it redundant** — and a false justification is *refutable*. A reader who checks it, as SE did, correctly concludes the stated danger is imaginary, and the natural next step is deleting the test the sentence exists to protect. **A false reason for a correct requirement is worse than no reason at all.**

      **⛔ AND THE EPIC ALREADY CONTAINED THE TRUE VERSION, 45 LINES ABOVE, IN AC-6.** Its calibration paragraph says applying the rule unconditionally *"would not widen the gate in production, because all three helpers early-return on an empty live prior."* **Accurate.** So this story carried the correct calibration and a stronger false claim simultaneously, and **no consistency sweep caught it because both sentences support the same conclusion** — opt-in is required. A sweep looks for contradictions between claims and verdicts; these two agreed on the verdict and disagreed on the fact. *(Second two-homes-one-claim instance in two rounds, and SE produced it one round after fixing the first. Awareness confers no immunity; the detector was SE executing its own sentence rather than restating it.)*
...
- [ ] **AC-13 (PRESERVE the operator-facing which-refuser discrimination)**: When this grain refuses, the WARN it emits MUST name the refusing mechanism and carry **that mechanism's own counts**, rendered from `refused_by` and the gate-outcome record.
      ...
      **This is a preservation requirement, not a new capability** [PM-VERIFIED, clean read of `retire_absent_player_lines`]. The refusal path already emits a `fresh_comparable_count` / `prior_count` / `floor_ratio` triple. **The reason it needs naming is not the conjunction** — that is superseded — **it is that several mechanisms each produce "0 retired" on this path**, and an unlabelled triple cannot say which one is reporting.
      ...
      Concretely: an unlabelled triple tells an operator nothing about **which** population was measured. On the 9-vs-9 churn the polluted gate reads **`9 of 18`** — 9 stale lines plus the 9 this run just wrote — clears the floor at exact equality, **PERMITS, and hard-deletes all nine while looking healthy.** The corrected gate reads `0 of 9` on the same input and refuses. A message carrying the reporting mechanism's own pair is the difference between a diagnosable log line and the one that hid this bug.

      **⛔ THIS ILLUSTRATION WAS WRONG IN TWO WAYS UNTIL 2026-07-26, and BOTH are corrected above** *(found by SE implementing this AC; PM-verified against the code, not accepted on SE's arithmetic)*. It read: *"a refusal reading 'not authoritative, comparable 18 of 18'."*
      1. **`18 of 18` is unreachable.** `comparable = gate_prior & fresh` is bounded by `|fresh| = 9`. The true figure is `9 of 18`; only the **numerator** was ever wrong, and the denominator-18 claim elsewhere is correct.
      2. **Worse, it named a branch this input cannot reach.** `9 >= 0.5 · 18` holds, so on the very input this epic exists to fix the polluted gate **permits** — it emits a *retire* line, not a refusal. **The illustration for a refusal message was an input that never refuses.**

      **The REQUIREMENT is untouched and was correct throughout** — the WARN names its mechanism and carries that mechanism's counts. Only the illustration was false. *(Recorded rather than silently fixed: this is the epic's own thesis landing inside the epic — a figure reasoned to rather than run, carried through six review passes because the conclusion it supported was right.)*
...
- [ ] **AC-15 (THE ADOPTED R1 OUTCOME — a DIAGNOSTIC on a PERMITTED retire. No gate, no cap, no `extra_guard`)** *(added at the R1 disposition, 2026-07-26)*: On a retire this grain **PERMITS**, when a victim id name-matches or jersey-matches a **surviving fresh** id, emit **one** WARN naming the count, the ids, and **`bb data dedup-players`** as the instrument. Additionally record **that same single-invocation outcome** — the matched-victim finding, computable from this call alone — in the gate-outcome record (TN-11 carries the field set).

      **⛔ THIS SENTENCE PREVIOUSLY ENDED *"record the accumulation signature in the gate-outcome record"* — the exact instruction this AC's own **"THIS AC IS SINGLE-RUN AND MUST STAY SINGLE-RUN"** banner (below) records as WITHDRAWN.** Corrected 2026-07-26 at the dispatch flip. **The withdrawal had been written into this AC's body, into TN-11 and into the History, and the AC's own operative sentence was the one site nobody swept** — an implementer reads a mandate before its retraction, and this AC is the first story dispatched. *(Same shape as the 10th mechanism: a verified repair verifies what it was pointed at. Three sites were repaired; the headline that generated them was not one of them.)*
...
- `src/db/player_dedup.py` — **PROSE ONLY, one docstring.** *(Added 2026-07-26 with the TN-9 row it serves.)* `_fold_name`'s docstring enumerates the fold as *"shared by detection … and the planner's terminal-name test … so the two never diverge"*; this story's `_dedup_candidate_victims` imports it and is a **third** consumer. **Correct the enumeration; do NOT weaken the single-fold guarantee** — the count is what changed, not the property. **No behaviour change in this file** — the import already exists.

      ⚠️ **This entry exists because a TN-9 row without a file-list entry is an UNREACHABLE obligation.** The row assigns the fix to story 01; had the file stayed unlisted, an implementer working the list would never open it and story 05's sweep does not cover `src/`. **This is the R4 finding's exact shape, recurring one round later on a row added to fix a different incompleteness** — closing an inventory gap opened a file-list gap, and the second is invisible from the artifact that fixed the first.
