...
## Status
`DONE`
<!-- DONE 2026-08-02. Both gates: code-reviewer APPROVED (round 2), PM AC verdict PASS on AC-1 through AC-9 including AC-5a. No finding deferred — every one reached FIXED or WITHDRAWN within the story. CR-measured finals: rule file 29,251 → 23,172 B (−6,079, −20.8%); always-on load 92,419 → 86,340 B (−6.6%); `.claude/rules` 2,649 → 2,642 lines (−7). Story findings for the closure record are in epic TN-20. -->
...
## Acceptance Criteria
- [x] **AC-1**: An enumeration worklist artifact exists with one row per rule in the pre-change file, where a "rule" is any numbered rule or bolded rule-sentence. Every row quotes its pre-change literal text. **RED**: a row whose quoted text does not appear verbatim in the pre-change blob, or a rule present in the pre-change blob with no row.
- [x] **AC-2**: The enumeration records the pre-change file's line count and byte size as read from disk, and those two figures match an independent measurement taken at review time. **RED**: either figure disagrees with the reviewer's own measurement.
- [x] **AC-3**: Every row carries a written verdict naming where that rule lives after the change — including the verdict `carried unchanged`. **RED**: any row with no verdict, or a verdict naming a location that does not contain the rule.
- [x] **AC-4**: No rule is dropped. **RED**: any row whose verdict is a removal, or whose named successor location does not contain the rule when read.
- [x] **AC-5**: The rule file is strictly smaller in bytes after the change, and both figures are recorded. **RED**: after ≥ before. (**Bytes are this story's primary measure**, and they are measured directly — `wc -c` on the file — not inferred from any subtree tool. Per epic TN-4(b), a line count is the wrong instrument for long-paragraph prose: this file runs ~300 bytes per line, so halving its bytes moves few lines.)
- [x] **AC-5a**: The **always-on load** is measured before and after, and both figures recorded. The pre-change reading is **92,419 bytes ≈ 23,000 tokens** (`CLAUDE.md` plus the seven `paths: "**"` rules; this file is 29,251 B of it, 32%). **RED**: after ≥ before, or either figure absent. The design owner's one-command measurement, offered as a starting point rather than a required invocation:
      ```
...
- [x] The enumeration worklist is committed as an artifact, not summarized in a message — `.project/research/2026-08-02-E-280-01-rule-inventory.md`, 77 rows (1-76 pre-registered, 77 added post-hoc and labelled as such)
- [x] `.claude/rules/doc-sweep.md` three-step discipline applied to the moved concepts
