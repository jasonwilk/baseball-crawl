...
## Status
`ABANDONED` (2026-07-26, operator ruling on PM triage; previously DRAFT, demoted from READY 2026-07-08 by the E-255-06 stale-READY triage)

**Abandonment rationale**: All three cited failure modes were re-verified as **still present** in current code on 2026-07-26 -- `_resolve_web_token_key` (`src/gamechanger/credential_parser.py:282`) still discards any token whose type is non-null and not `"user"`; `import_creds` (`src/cli/creds.py:171`) still calls `parse_credentials` and nothing else, so response-body tokens remain unreachable; the misleading warning is verbatim at lines 290-297. Every symbol this spec names still resolves (line references have drifted -- TN-7's "line 245" is now 265, TN-8's "~270" is now 282 -- which is why they should have been symbol citations).

It is abandoned on **value**, not on a false premise: this fixes a **fallback** credential path. `bb creds setup web` is the documented primary and performs the full login programmatically, and two other import shapes already work (paste the `POST /auth` JSON response body, which `_parse_json_credentials()` already handles; or copy a GET curl carrying a refresh token). Two stories of curl-replay machinery -- five body types, a logout-rejection guard, a full HTTP error matrix -- is disproportionate surface for a path with three working alternatives that has blocked nobody in months.

**⛔ Correction to this epic's own demotion reason (the verdict stands; the reason was false).** The 2026-07-08 stale-READY note demoted this epic partly on the ground that *"the auth/credential code has since changed materially (notably E-254's auth hardening — magic-link GET/POST split, passkey/login changes)."* **That is wrong.** E-254 hardened *app-internal* authentication in `src/api/` -- magic links, passkeys, production detection -- which is a different subsystem from GameChanger credential import in `src/gamechanger/credential_parser.py`. It does not touch this surface, and all three defects survived it untouched. The demotion was still correct (the epic was stale on age and on value), but a future reader inheriting the stated reason would conclude the code had moved when it had not. Recorded here rather than silently overwritten because a verdict's stated reason rots independently of the verdict, and a correct conclusion shields its false premise from review.

**Extraction before abandonment**: two captures on 2026-07-26. **IDEA-193** carries TN-8's warning-message fix -- live today, actively misdirecting, a handful of lines, independent of everything else here, and explicitly flagged so that TN-8's forward-looking clause ("copy a POST /auth curl -- tokens will be extracted automatically") is *not* carried over, since it promises the capability this epic will now never ship. **IDEA-194** carries the full execution design with an explicit promotion trigger (the operator hits this and all three workarounds fail) and the three constraints that must survive any future attempt: never execute a logout curl, do not route through `create_session()`, never log or store the password body.
...
## History
- **2026-07-26: ABANDONED** by operator ruling on a PM triage of the three oldest epics in `/epics/`. Not implemented; neither story was ever dispatched, so the epic produced zero changes to `src/`, `tests/`, `migrations/`, `docs/`, or the context layer. Rationale and the demotion-reason correction are in the Status section above. Extractions: IDEA-193 (TN-8 warning fix, the cheap independent half) and IDEA-194 (the full execution design, with an explicit promotion trigger).

  **Documentation assessment**: No documentation impact. The epic shipped nothing, so no update trigger in `.claude/rules/documentation.md` fires. Worth noting for whoever picks up IDEA-193 or IDEA-194: the operator-facing docs that describe `bb creds import` (`docs/admin/bootstrap-guide.md`, `docs/admin/operations.md`, `docs/admin/getting-started.md`, `docs/admin/credential-refresh.md`) are **accurate as they stand** -- they document the import shapes that actually work and correctly present `bb creds setup web` as primary. They describe today's behavior, not this epic's, so they need no change now and would need one only if IDEA-194 ever ships.

  **Context-layer assessment** (eight triggers per `.claude/rules/context-layer-assessment.md`, evaluated individually; every verdict is **no**, because an abandoned epic that produced no artifact cannot have established, discovered, or changed anything):
  1. New convention, pattern, or constraint established -- **no**. Nothing was built.
  2. Architectural decision with ongoing implications -- **no**. TN-1's two-layer split (parser stays pure; the CLI layer owns HTTP execution) was never acted on and is preserved in IDEA-194 as a proposal, not a standing decision.
  3. Footgun, failure mode, or boundary discovered -- **no**, with one deliberate carry-forward. The three genuine hazards this spec captured (executing a logout curl destroys the operator's GC session; `create_session()`'s default headers can invalidate a captured `gc-signature`; the password body must never be logged or stored) are real, but they are hazards *of the unbuilt feature* -- they bind only whoever attempts it. They are recorded in IDEA-194 rather than promoted to a rule, because promoting a constraint for code that does not exist would grow the layer for a hypothetical. The existing `.claude/rules/http-discipline.md` and the CLAUDE.md credential prohibitions already cover the live surface.
  4. Change to agent behavior, routing, or coordination -- **no**.
  5. Domain knowledge that should influence future agent decisions -- **no**. The api-scout consultation findings (refresh-type curls fire every browser session load and every ~51 minutes; `gc-signature` staleness rejected at ~6.2 hours) are endpoint-behavior reference material. `docs/api/endpoints/post-auth.md` already exists and is api-scout's to maintain; this epic discovered nothing that file lacks.
  6. New CLI command, workflow, or operational procedure introduced -- **no**. No new `bb` subcommand; `bb creds import` already existed and is unchanged.
  7. Net context-layer growth ratchet -- **no growth**. The epic adds nothing to the four gated subtrees. **Not a ratchet offset**; the inherited baseline drift remains an open operator obligation, untouched by this closure.
  8. Reusable behavioral lesson surfaced -- **no** for promotion, but one observation is worth stating because it *did* catch something concrete here: this epic's demotion reason cited a subsystem change that never touched its surface, and the false reason survived 18 days under a correct verdict because nobody re-checked a premise supporting a conclusion they agreed with. That shape is **already codified** in `.claude/rules/tool-output-integrity.md` ("A claim you RELAY is a claim you AUTHOR" -- a verdict's stated reason rots independently of the verdict). Trigger 8 requires promotion to a *load target*; this needs no promotion because its load target already carries it. Recording the fresh instance rather than re-promoting the lesson.

  **Deletion-side eviction**: E-175 deleted and renamed nothing. One claim **is** retired here -- the demotion note's "the auth/credential code has since changed materially (notably E-254's auth hardening)". Swept for it: the claim appears in this epic file (corrected above, retained as history rather than struck) and in one line of PM's own `MEMORY.md` active-epics list, which names the epic without repeating the false reason and was updated at this closure. No `.claude/rules/`, `.claude/agents/`, `CLAUDE.md`, or other agent's memory restates it. Per the sweep's own discipline the check was not a token grep for "E-254" alone but a look for *judgments that rested on the claim* -- e.g. a priority or urgency rating justified by "the code has moved underneath it"; none found outside the epic file.
