...
|---|---|---|
| `Anytown High` (+ `Reserve`/`Reserves`/`Sophomore`/`Sophomores`) | one real HS program and its sub-varsity squads | a school name carrying no level word, plus the tier token where one is present |
...
**The one shared-bytes rule, because "share no bytes with the real name" is unsatisfiable here.** The tier tokens (`Reserve`, `Reserves`, `Sophomore`, `Sophomores`) DO appear in the real names, and they are kept deliberately: they are the literal regex inputs every argument in TN-4 and TN-5 turns on, so removing them would delete the evidence rather than redact it. The standard applied is therefore **no shared *identifying* bytes** — the city/sponsor/mascot, i.e. everything that could re-identify a program, is fully replaced. Nothing here is a truncation, an abbreviation, or a prefix of a real name. An earlier pass of this redaction retained the real school's directional suffix (`…East`); that was removed, because unlike the tier tokens it carried no argument and did narrow the candidate set within a one-state, one-season population.
...
- **OQ-8 (NEW 2026-07-25, and it may be a COST this epic does not currently carry — gates the operator's build/shrink/shelve call):** A relayed measurement over the 198 already-probed teams reports that **E-274 in isolation would newly route 7 sponsor-named 15U/16U teams to `legion` where the bracket says `nrbl`.** Stated at the right strength: this is a **wrong league LABEL expanding a wrong-classification population, NOT a wrong pitch count** — the LEGION and NRBL curves are byte-identical today, so no rest number moves. Do **not** upgrade it to a safety claim; an inherited safety overstatement is the exact defect class E-272 shipped and this epic has logged five population-scope instances of.

  **PM could not verify it, and there is a mechanism tension that must be resolved before it is either accepted or dismissed.** Verifying requires executing the classifier over that population, which PM does not do. The tension: E-274's branch fires only on **school-family** `age_group` values, while a `15U`/`16U` bracket is the **travel** family — and TN-2 states the travel branch is UNCHANGED, with E-272's ladder already mapping 15U–16U → `nrbl`. So a team whose `age_group` is a bracket should resolve `nrbl` today and E-274 should not touch it. Two readings survive, and **they differ in severity by a lot**:
  1. The bracket is in the team **NAME** while `age_group` carries a school value. Then TN-2's conflict rule (`age_group` beats a name bracket) is exactly what produces the wrong label — a real and predictable consequence of a ruling already in this epic. But it also means these teams resolve **`nrbl` today, not `unknown`**, which makes this a **regression** of 7 correct classifications, contradicting the Success Criterion "the other 70 of 73 are unchanged" and the "zero regressions" goal.
  2. They genuinely are `unknown` today, in which case neither the name nor `age_group` carries a bracket, and the "15U/16U bracket rung" attribution needs re-deriving before the count means anything.

  Reading 1 is the one that changes the epic. **Resolve before READY**, and if it holds, TN-2's conflict rule needs a bracket carve-out (or E-274 needs to yield to a mapped bracket) — which is a design change, not a note. Owner: software-engineer or api-scout (needs a classifier run over the probed population). Cross-reference: this is the same ladder [[IDEA-178]] attacks from the `ngb` side, and its relayed sizing (14 affected / 9 correctly served) is recorded there under the same unverified marker.
