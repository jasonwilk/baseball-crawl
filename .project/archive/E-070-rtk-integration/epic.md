...
- 2026-03-08: Post-dev codex code review (uncommitted mode, covered E-070 + E-077 changes). 6 findings, all triaged by SE+CA+PM team. All 6 fixed: (1) credential token confusion -- JWT type check prevents access tokens from being saved as refresh tokens, (2) RTK exit status masking -- changed `;` to `&&` before RTK subshell so prior failures propagate, (3) mobile force_refresh crash -- added no-key guard raising AuthSigningError, (4) 401 retry error suppression -- removed error-swallowing except clauses so refresh failures propagate, (5) missing regression tests -- 6 new tests added, (6) trust_env=True leak -- added trust_env=False to token_manager httpx.Client. 79/79 targeted tests pass.
