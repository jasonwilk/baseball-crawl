...
**Date:** 2026-08-04 · **Status:** PARKED — ruled by the operator 2026-08-08: rule RELAXED, scrub CANCELLED. Real team/org/game ID prefixes are acceptable in `-REDACTED` placeholders (operator team-ID policy); PERSON-scoped identifiers (player/user ids) remain synthetic-only. The `api-docs.md` rule edit rides the PII-docs chunk; the measurement below stays as the record. Nothing is owed here
