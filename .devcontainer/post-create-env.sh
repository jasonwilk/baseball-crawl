...
alias tbb='d=$(tmux ls -F "#{session_name} #{?session_attached,attached,detached}" 2>/dev/null | grep "^baseball" | grep detached | head -1 | cut -d" " -f1); if [ -n "$d" ]; then tmux -CC attach-session -t "$d"; else n=0; s=baseball; while tmux has-session -t "$s" 2>/dev/null; do n=$((n+1)); s="baseball-$n"; done; tmux -CC new-session -s "$s"; fi'
