...
2. **Optional `--note` flag**: If the user provided additional context about what to focus on, pass it via `--note "text"`:
   ```
   ./scripts/codex-spec-review.sh <epic-dir> --note "Focus on AC testability"
   ```
...
4. **Check for errors.** If the script exits with an error (codex not installed, epic directory invalid, rubric missing), report the error to the user and stop -- do not proceed to Phase 2.
