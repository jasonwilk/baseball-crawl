...
**Entry condition**: Phase 2 is complete (DRAFT epic with stories exists), or loop-back from Phase 4 (user requested re-review after refinement).

The main session automatically chains this phase after PM completes Phase 2. No separate user command is needed -- the spec review is a built-in step of the planning workflow.

Track the current spec review iteration: `spec_review_iteration` (starts at 1, increments on each loop-back from Phase 4).

### Step 1: Run the codex-spec-review script

Run the spec review script directly via Bash. Do NOT load the codex-spec-review skill -- run the underlying script to avoid skill-nesting complexity per TN-7:
...
- **Exit 0, output contains "No findings"** (or similar clean-review language): Report "Clean spec review -- no findings." Proceed directly to Phase 5 (READY gate). Skip Phase 4 entirely.

- **Exit 0, output contains findings**: Present the full codex findings to the user. Proceed to Step 3 (triage).

- **Exit 124 (timeout)**: Report to the user: "Spec review timed out after 10 minutes." Ask how to proceed:
  - (a) Generate a spec review prompt for async execution. Assemble the prompt using the prompt-generation format described in `.claude/skills/codex-spec-review/SKILL.md`. Present in a fenced code block. Pause and wait for the user to paste findings back -- if the pasted content contains "No findings" (case-insensitive), skip triage and proceed to Phase 5; otherwise enter Phase 3 Step 3 with the pasted content. The user may also say "skip" (proceed to Phase 5).
  - (b) Skip spec review and proceed to Phase 5
...
```
Spec review findings for triage (iteration [N] of 3):
...
3. Assign a disposition:
   - ACCEPT: Valid finding. You will incorporate the fix in Phase 4.
...
PM consults domain experts on the team as needed (e.g., data-engineer for schema findings, baseball-coach for stat-related findings). PM reports the triage summary.
...
- If any findings are ACCEPTED: Collect the accepted findings and proceed to Phase 4 (Refinement).

**Exit condition**: All findings triaged with ACCEPT or DISMISS disposition, or spec review was clean/skipped/errored.

---

## Phase 4: Refinement

**Entry condition**: Phase 3 completed with accepted findings to incorporate.

### Step 1: PM incorporates findings

Route to PM to incorporate all accepted findings into the epic and story files. Send accepted findings via `SendMessage`:

```
Incorporate these accepted spec review findings into the epic and story files:

[list of accepted findings with their descriptions]

After incorporating each finding, proceed to the consistency sweep (Step 2).
```

### Step 2: Post-incorporation consistency sweep (REQUIRED GATE)

After PM incorporates findings, the main session MUST route PM through the consistency sweep before offering the user the re-review/proceed choice (Step 3). The workflow does NOT advance past this step until PM confirms the sweep is complete and all drift is resolved.
...
**Gate enforcement**: If PM's report does not include the confirmation phrase or reports unresolved drift, the main session sends PM back to fix the remaining inconsistencies. The main session does NOT present Step 3 options to the user until PM confirms the sweep is clean.

**If PM finds no drift**: Record "Consistency sweep: clean" and proceed to Step 3.

**If PM finds and fixes drift**: Record what was found and fixed, then proceed to Step 3. The fixes are part of the refinement -- no separate review cycle is needed for consistency fixes.

### Step 3: User decides next step

Present the refinement summary to the user (what was changed, consistency sweep results). Then present the decision:

**If `spec_review_iteration < 3`** (circuit breaker not reached):

> Refinement complete. Spec review iteration [N] of 3.
> (a) Re-run spec review on the refined artifacts (loop to Phase 3)
> (b) Proceed to READY (Phase 5)

- If (a): Increment `spec_review_iteration` and loop back to Phase 3.
- If (b): Proceed to Phase 5.

**If `spec_review_iteration >= 3`** (circuit breaker fires):

> Circuit breaker: 3 spec review iterations completed. Remaining findings from the last review:
...
- If (b): Reset `spec_review_iteration = 1` and loop back to Phase 3.
- If (c): Report "Epic remains DRAFT. Planning workflow stopped." End the workflow.

**Exit condition**: User decides to proceed to Phase 5, loop back to Phase 3, or stop.

---

## Phase 5: READY Gate

**Entry condition**: Spec review is clean, or user chose to proceed, or circuit breaker resolved.

### Step 1: PM sets READY

Route to PM to set the epic status to `READY`.
...
- Spec review outcome (clean, N findings fixed, skipped)
...
4. **Code-reviewer**: Spawn fresh (code-reviewer is not on the planning team -- it is not needed during planning). Use the implement skill's code-reviewer spawn context.
...
Phase 3: Spec Review  <------------------------------+
  ...
  - Timeout/error? --> prompt fallback or skip        |
  |                                                   |
  v                                                   |
Phase 4: Refinement (if accepted findings exist)      |
  - PM incorporates accepted findings                 |
  - Consistency sweep (required gate)                 |
  - User decides:                                     |
    (a) Re-run spec review --------loop back----------+
    (b) Proceed to READY --> Phase 5
    (c) Leave as DRAFT --> STOP (circuit breaker only)
  |
  v
Phase 5: READY Gate
  ...
  +---> compound_dispatch = true?
          ...
          - Team transition (PM stays, experts transition, CR spawned fresh)
...
### Spec review timeout
If the script times out (exit 124) after 10 minutes, report the timeout and offer the same options as "codex not installed" above.

### No findings on spec review
If the spec review returns clean ("No findings"), skip Phase 4 entirely and proceed directly to Phase 5 (READY gate). Do not offer triage -- there is nothing to triage.

### Circuit breaker (max 3 spec review iterations)
If the 3rd spec review still has findings, present the remaining findings to the user and ask:
- (a) Fix remaining findings and mark READY anyway
- (b) Continue refining (resets circuit breaker)
- (c) Leave as DRAFT and stop
...
### User pastes findings from async spec review
When the user ran the spec review externally (via a generated prompt) and pastes findings back:
- If the pasted content contains "No findings" (case-insensitive), treat it as a clean review -- report "Clean spec review -- no findings." and proceed directly to Phase 5 (READY gate), skipping triage. This matches the headless clean path.
- Otherwise, treat the pasted findings identically to headless findings -- enter Phase 3 Step 3 (triage) with the pasted content.
...
4. **Do not nest skill loading.** The plan skill runs the codex-spec-review script directly (`./scripts/codex-spec-review.sh`) rather than loading the codex-spec-review skill. Skill nesting adds complexity and context overhead with no benefit.

5. **Do not skip the consistency sweep.** After PM incorporates spec review findings, the post-incorporation consistency sweep (Phase 4 Step 2) is a required gate. PM must confirm consistency before the workflow proceeds to Phase 5 or loops back to Phase 3.
...
7. **Do not tear down the team between planning and dispatch.** When the user uses a compound trigger, the planning team transitions to a dispatch team. PM stays, consultation-mode agents transition, code-reviewer is spawned fresh. The unified team lifecycle preserves expert context across phases.
