...
- credentials live (`bb creds check`);
...
**`verify-aggregates` is a HARD sub-check for loader/aggregate epics.** When the epic touched loaders or season aggregates, `bb report verify-aggregates` MUST report zero mismatches -- this guards the cache-staleness window E-256's own closure sits inside (the stored `player_season_*` tables still exist until E-259 drops them, so a stale aggregate is a real, detectable divergence). Honest limitation: whole-game plays idempotency means the scoreboard cannot see an ingestion-parser change until data is re-ingested, so Step 1d does not prove a parser change reconciles -- it proves the flow runs and the *current* data still reconciles.
