...
**Terminal fixture requirement (the `generate` target).** The `.smoke-fixture` `generate` target MUST be a **terminal GC team page** -- a completed season that will gain no further games -- with **high play-by-play coverage**. This requirement is what makes the generate -> reconcile-scoreboard order (below) safe: because a terminal team's corpus is **static**, a closure's `bb report generate` ingests no net-new plays, so the post-generate `reconcile-scoreboard` reading measures **only the epic's own derivation effect** -- never a self-caused ingestion delta that would false-trip the one-way ratchet against a baseline captured before the generate. The skill documents the requirement only; **the actual team identifier stays in the gitignored `.smoke-fixture` file** (operator-owned -- real GC identifiers never enter tracked skill text). **Bootstrap (one-time, operator-owned, precedented per E-257):** when the operator pins the fixture team, (a) verify its play-by-play coverage in the dev DB -- a count of games that actually carry play-by-play rows (data-bearing, not a bare games count, since scored-but-empty games are the modal case) -- to confirm the corpus is plays-rich, and (b) re-snapshot the `reconcile-scoreboard` baseline **ONCE** (`bb report reconcile-scoreboard --update-baseline`) at that bootstrap. There is **no per-closure re-snapshot** and **no reorder** -- a static terminal fixture kills the drift at the root, where reorder-only would just move the false-fail to the next epic and re-snapshot-every-closure would be a recurring operator burden.
...
1. **`bb report generate <generate public_id>`** -- runs BEFORE `reconcile-scoreboard`, so the ratchet measures the state the smoke just produced. This order is kept (not reordered) and is safe **because** the fixture is a terminal team (see *Terminal fixture requirement* above): the static corpus means generate ingests no net-new plays, so this reading reflects only the epic's own derivation effect, and the baseline is re-snapshotted once at fixture bootstrap, never per closure.
...
4. **`bb report reconcile-scoreboard`** -- fold in its axis counters: **`self_games` == 0** (a hard zero), and neither `dropped_pitch_events` nor `no_plays_units` has regressed against the committed baseline. The ratchet gate's own non-zero exit on a gated regression is authoritative; a gated regression is an epic-FAIL.
...
**No aggregate-integrity sub-check (E-259).** E-259 dropped the stored `player_season_*` tables and moved season totals to query-time derivation, so there is **no aggregate-integrity gate** to run here -- the aggregate IS the query, and `bb report verify-aggregates` no longer exists. This is a deliberate net shrinkage, not a hole to fill: `reconcile-scoreboard` (check 4 above) remains the surviving live fidelity gate, unchanged, and nothing moves into the vacated slot. Honest limitation of that surviving gate: whole-game plays idempotency means the scoreboard cannot see an ingestion-parser change until data is re-ingested, so Step 1d does not prove a parser change reconciles -- it proves the flow runs and the *current* data still reconciles.
