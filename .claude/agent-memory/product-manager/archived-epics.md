...
- E-121 (COMPLETED): Style Guide and Context-Layer Remediation -- 4 stories, 15 findings across 10 context-layer files. Deleted fabricated 50-line rule from CR memory, aligned python-style.md with CLAUDE.md core principle (dicts, print, type hints, docstrings), scoped http-discipline rate limits and testing.md paths, updated DE agent def (stub-row FK handling, two-mode indexing, aggregate tables), corrected DE memory (timestamp format, ip_outs scoping), added CR proportionality qualifier for convention violations, added consultation-mode exception to SE/DE agent defs. Bonus: SE agent def print restriction fixed, stale examples updated. CA only. Archived 2026-03-17.
