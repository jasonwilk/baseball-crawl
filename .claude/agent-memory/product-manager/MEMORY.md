...
- **Next available idea number: IDEA-088**
...
- **E-245** (READY, NOT dispatched): High-Fidelity Play Ingestion — recover annotated pitches (parser drops `(75 MPH Curveball)` suffix → pitch_count/FPS collapse; ~5,841 events / 29 games, ~5,328 on team-133), capture `pitch_type`+`pitch_speed_mph` (migration 007), data-bearing FPS%/P-PA denominator + pitch-charted badge, fix 23 self-games (home==away). 4 stories (01 migration DE, 02 parser+reload SE, 03 denominator+badge SE, 04 self-game DE; 04 blocked-by 02 — reuses its reload to re-derive `batting_team_id`). North-star: "Always Get Closer to Byte-Identical Play Ingestion". Baseline: `.project/research/E-245-plays-boxscore-reconciliation-baseline.md`. SE/DE/coach consultations DONE & incorporated: QAB% KEEPS all-PA denom (gate FPS/P-PA/P-BF only); badge "Pitch-charted: N of M games" + inline "(N charted games)" + never-suppress zero note; headline = pitch-detail fidelity (team-133 FPS 3.4%→~64%, P-PA→~2.7), Tier-1 already 98.4-100% (guardrail only); self-game ROOT CAUSE confirmed (DE): scouting hardcodes `opponent_id=""` (scouting_loader.py:393/426) → boxscore has no opp key → game_loader.py:580-584 sets `opp_team_id=own_team_id` → home==away; downstream `batting_team_id`-from-`half` collapses both halves. Fix = game_loader.py by-name opp resolution + home≠away invariant guard (sentinel). EXISTING-DATA correction = RE-INGEST w/ API re-fetch of games.json for 5 teams (opponent name unrecoverable from DB) — ASYMMETRIC vs story-02's offline reparse. Story-02 reload = reparse-from-`raw_template` (offline, no GC call); reload-path MUST recompute `is_qab` via OR-merge (NOT from-scratch `_compute_qab` — `final_details` not persisted → HHB-only QABs would drop) + RE-DERIVE `is_first_pitch`. +23 BF `e283438c` SCOPED OUT (cause-4 multi-pitcher-boundary, NOT self-game → IDEA-087). Internal review iter-1 incorporated (11 ACCEPT): in-place reparse MANDATED (no clear-and-re-ingest for offline reload), reload re-derives `batting_team_id` (TN-3b), zero-charted copy split (no-plays vs some-plays), Tier-1 guardrail reconciled (#P excluded — it's the headline). key-metrics.md FPS% wording stale (charted-PA gate) → NEW story E-245-05 (claude-architect, blocked-by 03) reconciles it; TN-10 also a closure backstop. 5 stories (01 mig DE / 02 parser+reload SE / 03 denom+badge SE / 04 self-game DE / 05 key-metrics CA); Dispatch Team = DE+SE+CA. (M2 routing flip-flopped 3× via crossed messages; FINAL = keep E-245-05.) All open-Qs CLOSED. NOT dispatched (stops at READY after codex review). IDEA-086 = pitch-mix/velocity follow-on; IDEA-087 = cause-4 attribution drift.
