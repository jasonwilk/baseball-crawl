...
Recently completed epics (E-218 — E-245) are one-line-indexed in [archived-epics.md](archived-epics.md). Most recent: **E-245** (high-fidelity play ingestion — recover annotated pitches + type/velocity capture, charted-PA denominator + pitch-charted badge, self-game fix; COMPLETED + archived 2026-06-29). **Operator follow-up owed**: run the live `bb data reload-annotated-pitches` pass + the `bb data fix-self-games` 23→0 run (need creds/live DB; verify team-133 FPS 3.4%→~64%, P-PA→~2.7).
