...
- **Next epic: E-280 · next migration: 013 · next idea: 233.** (Globbed 2026-07-28: archive max E-278, live max E-279; ideas max 232. No migration. One glob returned a false negative — a second channel gave the real max.) **This line said "227", "229", then "231" — falsified THREE times by its own author. Glob the live dir; never trust this counter.**
...
- **E-279 READY (2026-07-28; re-confirm by 2026-09-26)** — closure machinery: the write-guard wedge + the archive-rename deadlock. 5 stories (01 guard reads `git worktree list`, 02 telemetry deletion, 03 rename moves into the worktree, 04 sweep script + 2 call sites, 05 SE memory). **Story 01 runs FIRST — now an EDGE (01→02, 01→03), not prose; do not remove it as redundant.** Planning commit cited by SUBJECT not hash (an amend already invalidated one hash here): *"feat(E-279): plan closure machinery…(READY)"*, 2026-07-29. The `ACM`→`ACMR` PII fix shipped SEPARATELY (*"fix: PII pre-commit gate enumerates renamed files"*, 2026-07-28) and must not be reverted. Detail + the facts not derivable from the epic file: [e279-planning-state.md](e279-planning-state.md).
