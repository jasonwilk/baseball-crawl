...
- **Next epic: E-277 · next idea: IDEA-198 · next migration: 013.** (The idea counter was stale AGAIN on 2026-07-26 — it read 190 while 190 and 191 already existed from E-276's closure, and 192/193/194 were then created. That is the third recorded staleness of this line. **Glob, don't trust it.**)
- **⚠️ E-275 is RESERVED-BUT-UNCREATED — a disk glob shows it FREE and is WRONG.** It belongs to the classifier-hardening epic (P2), reserved across ~18 references in `baseball-coach` + `ux-designer` memory and `.project/research/`; cheaper to skip than renumber. **IDEA-184 is pre-approved scope for its story 01.** **Durable lesson: a number can be reserved in PROSE before any directory exists, so globbing the live dirs is necessary but NOT sufficient — also grep `.claude/agent-memory/` and `.project/research/` for `E-NNN` before claiming one.**
...
*(E-276 shipped and moved to Archived Epics — see below.)*
...
- **E-174 DRAFT (kept 2026-07-26)** — asset-chunk fallback for the GC client-key extractor. Facts recorded in the epic: `extract_client_key()` is also Step 1/3 of `_setup_web()` (`src/cli/creds.py:986`), not just the `bb creds extract-key` diagnostic (**3** docs / 15 refs cite that command for recovery + an in-code error message — an earlier triage message said "four"; it is three); code verified still unfixed. **⛔ NO PROBE DISPATCHED, NONE PENDING, and the epic must NOT be framed as awaiting one.** Operator steer 2026-07-26: *"We need to actually refine before we build. We shouldn't overengineer. I've been using the system since March with no real breakage."* Four months of operation is the operator's evidence base and it outweighs inferring fragility from code structure — I had written this up with latent-breakage urgency framing and was corrected. The live GC check is the FIRST REFINEMENT STEP if and when the operator takes this up through the normal plan/refine workflow; **no promotion path outside that workflow.** The spec may also be more than the problem needs — narrow it during refinement rather than building it as written.
- **The 2026-07-26 triage of the three oldest epics: E-072 + E-175 ABANDONED (see [archived-epics.md](archived-epics.md)), E-174 KEPT.** Two durable takeaways beyond the dispositions. **(1)** All three were already DRAFT — the E-255-06 gate had demoted them on 2026-07-08 — so a triage framed as "these have sat READY too long" was working from a false premise; **read the epic's own Status block before accepting any framing of its state**, including a teammate's. **(2)** E-175's demotion REASON was false while its verdict was right (blamed E-254, which never touched that subsystem), and it survived because a correct conclusion shields its premise from review. Both corrections are recorded in the artifacts, not just here. E-193 + E-073 were archived ABANDONED earlier.
...
- **IDEA-178 (LIVE DEFECT in E-272, most salient) — PROMOTABLE, coach ruling is IN (2026-07-25); only prioritisation remains.** `ngb=american_legion` shadows NRBL, so **NRBL never fires for the teams it was built for**; benign only because the two curves are byte-identical today. Full ruling spec is in the idea file. **Do not fold into E-274** — same function, different decision. Durable lesson: a green suite + passing smoke cannot detect an unreachable branch; only ground truth found it.
- **IDEA-196 (LIVE on the coach-facing surface, prod only):** a phantom `"Unknown"` roster stub at a real jersey + a transposed surname on a prod share link; **a full generation did NOT heal either.** Durable code fact worth not re-deriving: `ensure_player_row` overwrites only when the incoming name is **strictly longer**, so an **equal-length misspelling is permanently sticky by design** — regeneration can never fix it, and no existing command cleans an `Unknown` stub (`dedup-players` won't touch it). Promote on operator wanting the prod page fixed, or **a second stub anywhere** (that one means an active creating path).
- **Client/creds hygiene cluster — see these THREE together, never one at a time: [[IDEA-193]] + [[IDEA-194]]** (`credential_parser.py`) **and [[IDEA-197]]** (`client.py` / `morning_run.py`). Different files, one family: *the error path tells the operator something false or unhelpful*. Any epic touching the HTTP client, creds, or morning-run reporting is the carrier for whichever of them it reaches.
- **IDEA-193 (live, cheap, needs no epic of its own):** `bb creds import`'s access-token warning tells operators to copy a `POST /auth` curl — a workflow that cannot work, so it loops them. A handful of lines in `_resolve_web_token_key`. **Fold into the next epic touching `credential_parser.py`; do not plan around it.** Sibling [[IDEA-194]] holds the heavy execution design behind a deliberately high trigger — keep them separate, 193 must not wait on 194.
...
- [e276-health-gate-triage.md](e276-health-gate-triage.md) — **E-276 COMPLETED 2026-07-26.** Design + settled-do-not-relitigate list, AND the dispatch's two defect families with their detectors. **Read before any reconcile-at-load work.** Three shipped decisions that get re-litigated if unread: **(1)** roster ships with **NO floor at all** (less gating than it started with — operator ruling); **(2)** player-line is **DIAGNOSTIC ONLY, NO GATE** — every closing mechanism doubles the coach-facing season aggregate; **(3)** the headline fix is true for **ONE RUN** — a refusal still WRITES. Ideas out: [[IDEA-189]] (extended ×2, two open questions answered), [[IDEA-190]], [[IDEA-191]]. **Operator obligation still open: the context ratchet carries an inherited +2,675 that was deliberately NOT re-snapshotted** — a scheduled refinement pass owns it, and E-276's own ~196 is a signed exception. **Do not cite that exception as precedent for waiving the ratchet.**
