...
- **E-228** (READY): Defensive Positioning Pocket Cards -- v1 scope (B) Pre-generate; TN-9 mandatory branch implementation (no auto-merge); 2 Codex review iterations 10/10 incorporated; (A)→(B) flip 2026-05-15. 7 stories. New `batter_positioning` table (migration 002); Tier 1 deterministic engine (3 stages: per-batter optimal-point + deviation → direction/depth knobs via swappable geometry seam → quantize to 8 absolute `call_state` values) + optional Tier 2 LLM rationale; both delivery paths. Vocabulary is absolute field direction (LEFT/RIGHT), not handedness-relative pull/oppo (handedness unavailable on GC data paths E-228 uses). Runtime clustering is a Non-Goal / fast-follow (IDEA-072). Absorbs IDEA-048 + IDEA-049.
