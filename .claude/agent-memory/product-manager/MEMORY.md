...
| E-180 | Codify E-178 Planning Knowledge | ARCHIVED | Completed 2026-04-03. 1 story delivered. UXD agent memory updated: design principles topic file (4 principles), terminology refresh (11 E-178 replacements superseding E-088), reference implementation pointer. CA only. No context-layer impact (codified existing E-178 knowledge). |
...
| E-196 | Pitching Availability and Game Ordering | ARCHIVED | Completed 2026-04-03. 5 stories delivered. Migration 014 (start_time + timezone on games). Game ordering convention (start_time tiebreaker). `get_pitching_workload()` shared query. Rest/Last + P(7d) columns on all pitching surfaces. Context-layer: T1, T2, T4, T6 fire. |
...
| E-200 | Fix Stale season_id on Pre-Existing Games | ARCHIVED | Completed 2026-04-02. 2 stories delivered. Code fix (season_id in _upsert_game ON CONFLICT) + migration 013 (correct stale season_ids for ~200+ teams without program_id). 3 Codex findings (2 over-scoped migration UPDATEs + 1 missing dedup test). No context-layer impact. |
| E-201 | Close Reconciliation Accuracy Gaps | ARCHIVED | Completed 2026-04-03. 1 story delivered. 5 fixes in engine.py: boxscore #P/TS supplement, IBB in BB outcomes, games-table scores for game_runs, BF sum for game_pa_count. All cross-source signals 99%+. No context-layer impact. E-199 now unblocked. |
| E-202 | Fix Report Generator Team Dedup Bug | ARCHIVED | Completed 2026-04-03. 1 story delivered. Backfill public_id in generator.py after step-3 name match. Fixes duplicate team creation during report generation. No context-layer impact. |
| E-203 | Fix FPS% Formula to Match GameChanger | ARCHIVED | Completed 2026-04-03. 2 stories delivered. FPS% queries simplified to `FPS / BF` (no HBP/IBB exclusions), matching GameChanger. CLAUDE.md definition updated. No context-layer impact beyond CLAUDE.md (already handled by E-203-02). Docs impact: stale FPS% refs in docs/admin/. |
| E-204 | Starter vs. Relief Appearance Tracking | ARCHIVED | Completed 2026-04-03. 3 stories delivered. Migration 015 (`appearance_order INTEGER` on `player_game_pitching`). Game loader forward-fill. `bb data backfill-appearance-order` CLI. GS/GR column on all 5 pitching surfaces. Scouting aggregation computes GS from `appearance_order = 1`. Game boxscore sorted by appearance order. 5 code review findings (5 accepted, 0 dismissed). Context-layer: T1, T2, T3, T6 fire. |
| E-210 | Pitching Workload Display Redesign | DRAFT | 1 story. Replace P(7d) `N/Nd` format with `Np (Ng)` across all surfaces. Expose `appearances_7d` from shared query. Rename headers to `Pitches (7d)`. SE only. |
| E-211 | Fix Opponent-Perspective UUID Contamination | READY | 3 stories: stop UUID contamination at 3 pipeline sources (01), fix report generator gc_uuid resolution + plays scoping (02), document+execute data cleanup (03). SE only. 01+02 parallel, 03 depends on both. 24 review findings (14 accepted, 10 dismissed). |
| E-212 | Predicted Starter | READY | 5 stories: shared pitching history query (01), deterministic rotation engine (02), OpenRouter LLM client+analysis (03), report integration+template (04), dashboard opponent detail integration (05). Two-tier: Tier 1 deterministic always runs, Tier 2 LLM optional (reports only, not dashboard). SE only. 01→02→(03,04,05). First LLM integration in the project. Both scouting surfaces. |
...
| IDEA-064 | Dashboard-Report Feature Parity | CANDIDATE | 2026-07-03 | 7 gaps: FPS%/QAB% (HIGH), team-level plays stats, recent form, key players, throws/bats, roster (MEDIUM/LOW). Immediately promotable after E-212 ships. |
| IDEA-065 | LLM Starter Prediction Evaluation Harness | CANDIDATE | 2026-07-03 | Dev tool for prompt tuning + model comparison. Calls real enrich_prediction() with fixtures. After E-212-03. |
