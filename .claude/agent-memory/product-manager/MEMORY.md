...
- **E-229** (DRAFT): Plays in `bb data scout` parity -- shared helper `src/gamechanger/pipelines/plays_stage.py` invoked by both `_scout_live` (CLI) and `run_scouting_sync` (web) after post-spray dedup. Originally story E-228-02; promoted to its own epic during E-228 v1 refinement. **Independent of E-228 v1**; ships on its own value (scout-pipeline hygiene + dashboard parity). Discovery decisions locked. Immediately plannable.
