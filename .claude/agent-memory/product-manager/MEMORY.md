...
## Numbering State
- **Next available epic number: E-259.** E-258 (Review-System Hardening) COMPLETED+archived 2026-07-08. **E-257 (Plays→Boxscore Reconciliation Scoreboard productization, DE-owned) COMPLETED+archived 2026-07-09** — shipped `bb report reconcile-scoreboard` (+`--json`, `--update-baseline`) measurement + one-way ratchet gate (`self_games` hard-zero, baseline-absent exit 3, malformed exit 4) + report-time TEAM-level plausibility WARNING (FPS 30–75% / P-PA 3.0–4.5). Full-suite gate green (3849 passed). Operator baseline-snapshot step owed (see Operator follow-ups). Hard UPSTREAM of E-256. Detail in [archived-epics.md](archived-epics.md) + the archived epic file. **E-256 (CE-6) is the only remaining DRAFT stub** (query-time-aggregate cutover needs PM/user sign-off; its closure smoke consumes E-257's `--json` axis counters + committed baseline). CE-1..CE-5 = E-251..E-255 all done.
- **Next available idea number: IDEA-109.** IDEA-108 filed 2026-07-08 = persist E-257-03 plausibility signal as a run-record trust flag (deferred from the WARNING-log form). Full list: `/.project/ideas/README.md`.
