...
- **Next epic: E-278 · next migration: 013 · next idea: 222.** (E-277 closed 2026-07-27, commit `f32d3b9`. No migration.)
...
- **E-275 DRAFT (planning landed on main 2026-07-27)** — classifier hardening at `epics/E-275-classifier-hardening/`, **which is the durable record; prefer pointing over restating.** 2 stories, SE-only dispatch team: 01 narrows the Legion precedence fix (`legion`/`post N` ahead of `\bvarsity\b`), 02 (depends on 01) adds the constant tripwire + an append-only **ground-truth fixture pack**. **Value ordering is DESCENDING — pack first, reorder last — and it has been corrected THREE times; do not restore an earlier one or trust a summary that disagrees.** The reorder **changes 0 of 563 real names**, so it is *reachable but unobserved*, not live — it ships anyway because the rule-of-three bound on the co-naming rate is **~13.6%**, and that bound (never the bare `0 of 563`) is what the scope decision rests on. **Two OQs block READY: OQ-2** (SE's verbatim tripwire pin shape — the story file says unconfirmed and **the file wins over any handover note**) and **OQ-3** (a **fresh coach instance** certifying the CHANGE/GUARD matrix, held until the trim landed, which it has). Two named closure obligations (TN-10 A: the NRBL shadow sentence; B: the stale `pii-safety.md` coverage claim = [[IDEA-211]]). Ideas out: [[IDEA-201]]/[[IDEA-202]]/[[IDEA-209]]/[[IDEA-210]] (the four MINORs), [[IDEA-213]] (both flags, cut in full), [[IDEA-214]].
...
- **E-174 DRAFT (kept 2026-07-26)** — asset-chunk fallback for the GC client-key extractor. `extract_client_key()` is also Step 1/3 of `_setup_web()` (`src/cli/creds.py:986`), not just the `bb creds extract-key` diagnostic (**3** docs / 15 refs, not four); still unfixed. **⛔ NO PROBE DISPATCHED, NONE PENDING — do NOT frame it as awaiting one.** Operator steer: refine before building, four months of operation with no breakage outweighs structure-derived fragility (I used urgency framing and was corrected — [feedback_refine_before_building_no_urgency_framing.md](feedback_refine_before_building_no_urgency_framing.md)). The live GC check is the first refinement step **inside** the normal workflow; no promotion path outside it. The spec may exceed the problem.
