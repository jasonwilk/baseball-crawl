...
| E-165 | Fix Scouting Spray Chart Team Misattribution | COMPLETED | Scouting spray loader skips unresolvable players instead of guessing. Operator cleanup query documented. Ready to archive. |
...
| E-168 | Switch Opponent Resolution to POST /search | ARCHIVED | 3 stories (2 code + 1 docs). Admin search switched to POST /search, auto-resolver search fallback for ~14% null-progenitor opponents, flow doc updated. 30 ACs, 4 impl review findings (all accepted). |
...
| E-171 | Enrich Resolve Search Results | ABANDONED | Scope absorbed into E-173-03 (unified resolve page). Archived. |
...
| E-173 | Fix Opponent Scouting Workflow End-to-End | READY | 6 stories: resolution write-through (01), auto-scout trigger (02), unified resolve page (03), dashboard sort+status (04), terminology cleanup (05), data repair CLI (06). Fixes broken opponent→dashboard pipeline. Absorbs E-171 scope. SE only. 72 review findings (52 accepted, 20 dismissed). |
