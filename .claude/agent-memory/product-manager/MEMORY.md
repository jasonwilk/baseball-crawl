...
- **E-228** (READY): Reset Dev Env -- empty `bb db reset` (delete seed_dev.sql + dead code) + admin-sees-all dashboard (widen `_get_permitted_teams`; option A keeps `_assign_member_teams`). 2 SE stories. Internal+Codex reviewed (17 findings, 15 accepted).
