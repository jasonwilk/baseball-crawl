...
- **Next epic: E-277 · next idea: IDEA-190 · next migration: 013.** (Both counters were stale when last read — verify, don't trust.)
...
- **E-276 READY, NOT DISPATCHED** (dispatch is a separate operator gate). Reconcile-at-load health gate reads its "prior" AFTER the run's own writes — live uncapped loss on a routine `bb report generate`, all 3 grains. 5 stories, SE + CA. Amended twice on 2026-07-26 (R2–R5 repairs, then the R1 disposition); ~2,180 lines, story 01 → AC-15, story 03 gained AC-5b. **Three shipped decisions that get re-litigated if not read: (1) roster ships with NO floor at all — LESS gating than it started with, operator ruling; (2) R1 = DIAGNOSTIC ONLY, NO GATE on player-line, because every closing mechanism doubles the coach-facing season aggregate; (3) the headline fix is true for ONE RUN — a refusal still WRITES.** ⚠️ **Read [e276-health-gate-triage.md](e276-health-gate-triage.md) before touching it** — design, settled-do-not-relitigate list, the 2026-07-26 rounds, and the durable process lessons all live there.
