...
- **E-228** (COMPLETED on branch / pending merge): Defensive Positioning Pocket Cards v1. Committed at `2d6be06` on `epic/E-228-defensive-positioning-cards`. Held for dev validation per TN-9. User dev validation surfaced reference-frame bug + R2 tautology + categorical-vocabulary delivery failure → triggered E-229. E-228 will not merge to main on its own; merges as part of the combined E-228+E-229 stack after E-229 dev validation per E-229 TN-1 + user reaffirmation 2026-05-17.
- **E-229** (READY 2026-05-17): Team-Aggregate Defensive Positioning. **11 stories** (E-229-01 migration v2 / 02 engine rewrite / 2b feasibility prototype + locked-constants artifact / 03 card SVG / 04 outlier pills + coord-regression test / 05 compact card template / 06 prep page / 07 call sheet with NOTE column / 08 bundle + LLM render-time threading / 09 LLM input contract / 10 pipeline wiring). Builds on E-228 branch per TN-1 mandatory branch implementation (NOT main); user-locked "MIGHT merge to main only after validation" 2026-05-17. Reframe = team-aggregate centroid as reference frame (retires textbook BASE_POSITIONS bug), visual A-H compass zones with SVG y-axis convention `y=0 at deep CF` (TN-15), single-source-of-truth locked-constants artifact at `/.project/research/E-229-locked-layout-constants.md`. Migration 002 rewritten in place (v2 schema with TEXT season_id/player_id, FK REFERENCES, PK `(player_id, team_id, season_id, perspective_team_id, position)`, new `team_position_aggregate` table). Handedness = structural data-availability choice (not v1 deferral). LLM rationale ephemeral (no DB persistence; rendered render-time on prep + call sheet, NOT cards). Print = quarter-letter 4-up, 4-page mixed-orientation PDF (call sheet landscape, prep page landscape, 2 card sheets portrait with sheet-2 fill = compass-key + opponent-context-card). Review cycles: 68 distinct findings, 67 ACCEPTED, 0 DISMISSED across 4 cycles. PM-owned lessons: schema regression (feedback_no_fabricated_expert_confirmation.md), concept-word + artifact-vs-stories sweep gap (feedback_concept_word_consistency_sweep.md). Absorbs IDEA-073. Captures IDEA-077 (boxscore batting-order backfill, future epic if promoted per DE).
...
- **IDEA-072**: Clustering-Derived Empirical Fielding Zones — E-228 fast-follow; blocked on E-228 + its calibration pass confirming per-batter centroid tightness (signal-to-noise gate). E-228-02 builds the swappable geometry seam this plugs into.
- **IDEA-073**: Team-Wide Base Defensive Alignment — blocked on E-228; deferred low-priority (at most a call-sheet header note, doesn't change per-batter cards).
...
- [feedback_relay_dropouts_no_silent_fail.md](feedback_relay_dropouts_no_silent_fail.md) -- Peer DM drops; sanity-check inbox vs. task list, ack receipt, never silent-fail
- [feedback_no_fabricated_expert_confirmation.md](feedback_no_fabricated_expert_confirmation.md) -- Never cite expert input from memory without re-reading the actual relay; fabricated confirmations are anti-fabrication violations (E-229 lesson)
- [feedback_concept_word_consistency_sweep.md](feedback_concept_word_consistency_sweep.md) -- Consistency sweep must grep concept words AND code syntax; syntax-only grep misses prose drift (E-229 iteration-1 sweep gap)
