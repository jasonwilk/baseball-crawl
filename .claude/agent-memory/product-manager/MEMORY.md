...
- E-055 (READY): Unified Operator CLI -- 7 stories. Single `bb` entry point via Typer. Command groups: creds, data, proxy, db, status. Wraps existing scripts as library code. `bb status` = operator health dashboard. Dep chain: 01 first, then 02+03+04+05 parallel, then 06 (needs 02+04), then 07 (needs all). UX+SE consulted. Epic-level deps: E-042 (--source flag on crawl/load), E-052, E-053, E-054. Dispatch blocked until all deps COMPLETED.
## Archived Epics
