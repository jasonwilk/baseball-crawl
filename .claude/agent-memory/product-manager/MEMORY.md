...
- E-060 (READY): Stabilize Header-Capture Aggregation -- 1 story. Replace "latest request wins" overwrite in HeaderCapture.request() with first-seen-wins per-key aggregation + conflict logging. Eliminates non-deterministic header-report.json output. SE only. No deps.
