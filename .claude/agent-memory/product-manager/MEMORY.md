...
## Numbering State
- **Next available epic number: E-259.** E-258 (Review-System Hardening) COMPLETED+archived 2026-07-08. **E-257 (Plays→Boxscore Reconciliation Scoreboard productization, DE-owned) planned to READY 2026-07-09** — NOT yet dispatched (separate user auth); sequenced STRICTLY BEFORE E-256; 3 stories (01 scoreboard core+CLI+`--json` DE; 02 committed-JSON-baseline + one-way ratchet gate + `self_games` hard-zero + baseline-absent bootstrap, DE — real baseline `.project/baselines/reconciliation-scoreboard.json` is an OPERATOR/closure artifact NOT an implementer create; 03 report-time plausibility WARNING for TEAM-level FPS 30–75% / P-PA 3.0–4.5, SE); review scorecard 24 findings all accepted/0 dismissed; full detail in the epic file + [archived-epics.md](archived-epics.md). **E-256 (CE-6) is the only remaining DRAFT stub** (query-time-aggregate cutover needs PM/user sign-off). CE-1..CE-5 = E-251..E-255 all done.
