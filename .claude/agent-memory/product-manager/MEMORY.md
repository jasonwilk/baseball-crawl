...
- **E-228** (READY): Matchup Strategy Report (v1) -- refined down from 11 stories to 4 on 2026-04-28 per 5-reviewer consensus + user steering. Codex passes 1/2/4/5/7/8 on 2026-04-28 resolved 18 distinct findings (loss-recipe engine output narrowed; suppress trigger restated as in-engine condition; section order pinned; PullTendencyNote citations from raw fields; E-229 removed as blocker for v1; `pull_tendency_prose` removed from LLM schema; multi-field LLM output schema codified; `data_notes.subsection` field added for per-sub-section rendering). Set READY 2026-04-29 (Path B light-touch flip closing the 2026-04-28 "returned to DRAFT pending re-spec-review" bookkeeping gap; two stale CUT-stub assertions in E-228-02 + E-228-07 also corrected for current-truth alignment, no scope change). **Independent of E-229.** Awaiting user dispatch authorization.
