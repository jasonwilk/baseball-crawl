...
- **E-247** (READY): Twin-Method & Duplicated-Block Extractions — wave 2 (H2, H4, H3-consolidate, M4, M5, M6, M3; 7 stories). Medium risk; byte-identical verification + parity gate. NOTE: E-246-02 made ZERO changes to `crawlers/opponents.py` (the M2 import json/data_root/disk-docstring residue did not exist — opponents.py is the clean E-240 crawler), so the prior E-246↔E-247-03 opponents.py overlap is NIL; E-247's only remaining shared-file serialization is `generator.py`. api-scout consult recorded on E-247-03; CA review-time security pass on E-247-04/07.
...
Recently completed epics (E-218 — E-246) are one-line-indexed in [archived-epics.md](archived-epics.md). Most recent: **E-246** (Dead-Code Removal & Low-Risk Consolidation — wave 1 of the whole-project /simplify sweep; 7 stories: deleted dead gc_uuid resolver+test, dead/vestigial crawler/API/signing/parser residue, dead GC client verbs post()/delete()+tests; consolidated the 5× DB-path cascade into canonical `resolve_db_path()` with `bb data` newly honoring `DATABASE_PATH`, the aggregate-parity SUM projections+compared-column list into shared builders/keys (HARD GATE, byte-identical), the spray play_type→marker vocabulary, and CLI/safety dups. Zero stat-definition changes. 1 Codex finding (aggregate_parity second-edit-site) remediated in-dispatch. Closure gates passed: full-suite 3441 passed/0 failed; `verify-aggregates` clean (10208 cells, 0 mismatches). Context-layer codified: CLAUDE.md `resolve_db_path` canonical bullet + data-model.md two-axis single-source parity footgun. COMPLETED + archived 2026-06-30). Also: **E-242** (align dispatch/plan/implement vocabulary to subagent framing — context-layer vocab/breakage fix after CC v2.1.178 removed TeamCreate/TeamDelete; Option B named-subagent framing, keep `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`, surgical scope, no behavioral change except multi-agent-patterns L55 parallel→serial; CA-led 3 file-disjoint stories; COMPLETED + archived 2026-06-29). Also: **E-245** (high-fidelity play ingestion — recover annotated pitches + type/velocity capture, charted-PA denominator + pitch-charted badge, self-game fix; COMPLETED + archived 2026-06-29). **Operator follow-up owed (E-245)**: run the live `bb data reload-annotated-pitches` pass + the `bb data fix-self-games` 23→0 run (need creds/live DB; verify team-133 FPS 3.4%→~64%, P-PA→~2.7).
