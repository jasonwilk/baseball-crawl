...
| E-188 | Eliminate Orphan Team Stubs from Report Generation | COMPLETED | Archived 2026-03-30. Snapshot-and-diff orphan cleanup in generator (01 DONE). CLI cleanup command (02) ABANDONED -- one-off tasks are documented SQL, not permanent CLI commands. Shared cleanup module at `src/db/cleanup.py`. |
| E-189 | Opponent Flow Pipeline and Display Parity | READY | 5 stories: spray stages + gc_uuid resolution in web pipeline (01), auto-scout after discovery (02), public_id filtering in gc_uuid resolver (03), PA/IP badges + heat-map on dashboard opponent detail (04), context-layer pipeline parity (05). SE + CA. Promoted from IDEA-059. Serial: 01->02->05; 03 and 04 parallel. 23 review findings (19 accepted, 4 dismissed). |
| E-190 | Declare Dependencies in pyproject.toml | READY | 2 stories: add runtime deps to pyproject.toml (01), update dependency-management rule (02). SE + CA. Promoted from IDEA-058. Serial: 01->02. 13 review findings (12 accepted, 1 dismissed). |
...
- Data model (revised 2026-03-27, E-167): Fresh-start schema rewrite in single migration `001_initial_schema.sql` (old 001-008 archived). Key changes: programs table (hs/usssa/legion umbrella entity); teams use INTEGER PK AUTOINCREMENT with gc_uuid/public_id as UNIQUE external identifier columns; `membership_type` (member/tracked) replaces is_owned; `classification` replaces level (varsity/jv/freshman/reserve/8U-14U/legion); team_opponents junction table for opponent relationships; TeamRef dataclass pattern (id/gc_uuid/public_id) for pipeline code; enriched stat columns -- game_stream_id + all counting stats now populated by E-117 (game loader: 12 stats + game_stream_id; season stats: 47 batting + 47 pitching; scouting aggregates: 5 batting + 6 pitching cascade). E-158 added: migration 006 (spray_charts indexes + columns: event_gc_id, created_at_ms, season_id). spray_charts now populated by SprayChartCrawler + SprayChartLoader. E-167 added: migration 007 (idx_teams_name_season_year COLLATE NOCASE index); `src/db/teams.py` with `ensure_team_row()` -- canonical team creation function used by all 8 pipeline INSERT paths. Still unpopulated: bats/throws, splits, batting_order. Auth tables simplified (users/sessions/magic_link_tokens/passkey_credentials/coaching_assignments). E-143 added: migration 002 (users.role column), migration 003 (crawl_jobs table). Next migration: 008.
