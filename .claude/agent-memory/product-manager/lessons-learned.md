...
**8. The instrument can be wrong, not just the result.** Three separate times: stale bytecode silently invalidated mutation evidence (every source-of-truth agreed and all were wrong about what executed); a garbled-but-COHERENT tool Read fabricated exactly the defect the context made most plausible, nearly making me accuse an implementer of shipping a mutation; and the trigger-7 ratchet turned out unable to see CLAUDE.md at all. Careful reading defeats none of these — only an independent channel does.
