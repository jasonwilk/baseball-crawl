...
- **Top-level keys, classified by IDENTITY not shape** (corrected 2026-08-02): the response is a JSON object with exactly 2 keys (one per team). Each key is that team's `public_id` slug or its UUID, decided by whether that TEAM has a public GC presence -- so both keys are slugs when the opponent was also scored on GC. The parser matches a key against our own `public_id`, then our `gc_uuid`; the remaining key is the opponent's. A dashes/no-dashes regex must NOT decide the split -- doing so reported "no opponent key" on all-slug payloads and discarded the opponent's whole envelope.
