...
- [season_aggregate_writers.md](season_aggregate_writers.md) -- HISTORY: the entire season-aggregate WRITER architecture (three writers → `canonical_recompute` → `aggregate_parity` → `player_season_*` tables) was RETIRED by E-259 (query-time derivation). Live residual: canonical column set = ScoutingLoader's, now the surviving SUM projection in `season_aggregates.py`; PA/XBH are renderer-derived, never stored.
