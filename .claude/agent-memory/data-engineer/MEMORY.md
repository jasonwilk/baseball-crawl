...
- [season_tables_are_a_pure_cache.md](season_tables_are_a_pure_cache.md) -- `player_season_*` are a pure derived cache post-E-239 (only INSERT sites write `boxscore_only`; live DB 100% boxscore_only) → DROP is data-lossless IFF zero member rows; the provenance guard is inert; query-time read costs 0.046 ms/team but MUST add the `perspective_team_id` filter the stored rows implied. (E-259 design basis.)
