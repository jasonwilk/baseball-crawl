...
**CORRECTION 2026-07-25 (IDEA-178, coach ruling, not yet implemented) — "a recognized `ngb` still wins outright" is FALSE for `american_legion` specifically.** api-scout found live teams tagged `ngb=["american_legion"]` (accurate — NRBL follows ALB regulations, so NRBL coaches have no better `ngb` value to pick) whose OTHER signals (a 15U-16U bracket, or a "Reserve" name word) clearly indicate NRBL, not Legion — e.g. "Ortho Nebraska Reserve" (`age_group=16U`, resolves `legion`, should be `nrbl`) and "PrimeTime Westview Reserve" (`age_group="Between 13 - 18"`, same defect). This DEFEATS the bracket floor described above: Ortho's `16U` bracket would correctly route to `nrbl` if the `ngb` check didn't short-circuit first.

**Why the original Priority-2 dispositive-ngb reasoning does not transfer here.** That reasoning (TN-2) is sound for `usssa`/`perfect_game` — those are genuinely different RULE SYSTEMS (innings/outs-based, not pitch-count), so a recognized `ngb` must win to avoid applying the wrong kind of rule entirely. **Legion and NRBL are NOT different rule systems — they are byte-identical curves today**, so `ngb=american_legion` winning outright over a strong NRBL signal only produces a mislabel, not a wrong rest-day number, which is exactly why nothing looked broken until api-scout went looking. The moment the two bodies' published tables diverge (the whole reason NRBL is kept as its own constant rather than aliased to `LEGION`), this becomes a live wrong-table bug — the distinct-constant design existed for precisely this case and had, until now, never been allowed to fire.

**Ruling: `american_legion` — and ONLY `american_legion` — is REFINABLE, not fully dispositive, by a strong summer sub-varsity signal.** `usssa`/`perfect_game` remain fully dispositive with no refinement (they ARE a different rule system; TN-2's original protection stays intact there). Refinement precedence, extending the existing bracket-over-name-word pattern: (1) a parseable 15U-16U bracket refines `american_legion` → `nrbl`; a 17U+ bracket confirms `legion`, no change. (2) Absent a parseable bracket — including the free-text range form ("Between 13-18"), which does NOT parse as a `\d+U` bracket — a summer sub-varsity name word (Reserve/Reserves/JV/Junior Varsity/Freshman/Frosh/Sophomore) refines to `nrbl` too. This is what fixes PrimeTime: its range-form `age_group` carries no bracket, so it needs the name-word fallback, not just the bracket check, to resolve correctly. (3) Absent both, `american_legion` stays dispositive at `legion` (the safe default absent contrary evidence — Legion is the top summer tier).

**This also revises IDEA-126's range-form default for this narrow overlap case.** IDEA-126 treats a bare free-text age range as uncertain enough to fall to the `youth_travel` guideline estimate (since "13-18" dips below the confirmed 15-18 curve). That default should still hold for a range-form team with NO other corroborating signal. But when `ngb=american_legion` AND a summer sub-varsity name word are BOTH present (PrimeTime's exact case), the name evidence is more SPECIFIC to this team than the org's broad/generic range text, and should override the range-form default to resolve binding `nrbl` rather than the youth estimate — WARN-logged, since it's an inference stacked on two signals rather than a direct parse.

**Stakes note, applies to both bullets above:** this refinement affects OPPONENT-scouting predictions only (which of the opponent's arms is likely to start), not LSB's own athlete safety/compliance — the rest-day NUMBERS stay identical either way since `LEGION` and `NRBL` are byte-identical today (only the label and future-divergence protection are at stake). See [[e274-age-group-level-signal-consultation]] for the fuller stakes-recalibration reasoning, developed in the same 2026-07-25 consultation round.

**ADDITIONAL GAP FOUND 2026-07-25 — the recreational family's `Under 13` / `Over 18` free-text values, and the travel `NNO` ("and over") bracket suffix, are also unparsed and fall through unprotected to the name-keyword path.** Same mechanism as the `high_freshman` discovery, on a family nobody had examined. Ruled: `Under 13` hard-suppresses identically to `middle_12U`/`elementary`/`college` (must veto ahead of the name-keyword path — a young rec team must never resolve to a binding Legion/NRBL table via a "Reserve"-style name match); `Over 18` stays a GUIDELINE fallback (the under-rest hazard is age-directional and doesn't transfer to an adult population, but free-text "Over 18" spans too wide an adult age range to bind confidently); the travel `NNO` "and over" bracket form (e.g. `18O`) BINDS, folded into the existing 17U+-maps-to-Legion-age rule rather than treated as a new case. Full reasoning in [[e274-age-group-level-signal-consultation]].
