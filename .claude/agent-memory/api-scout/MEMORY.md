...
- [operational-notes.md](operational-notes.md) -- High-priority unexplored areas (POST /search schema, LSB HS credentials, import-summary), boxscore critical facts (game_stream.id, team keys classified by IDENTITY not shape, groups; see [boxscore-key-form.md](boxscore-key-form.md)), JWT decode tips (exp-iat thresholds), security rules and PII hotspots.
...
- [boxscore-key-form.md](boxscore-key-form.md) -- what decides a boxscore top-level key's slug-vs-UUID form is UNKNOWN; "public GC presence" and "linked vs hand-typed" are BOTH refuted live (n=140 games/28 teams, slug+slug 10.0%). A UUID-form opponent key is a local `root_team_id` (=`pregame_data.opponent_id`), never a canonical `gc_uuid` -- the E-211 contamination path. `GET /teams/{id}/opponent/{opp_id}` works for NON-MANAGED teams (2026-08-03)
- [opponent-registry-access.md](opponent-registry-access.md) -- `/teams/{id}/opponents` needs NO association (21/21 + 24/24 strangers; 403 control) so resolution is BULK; **the BARE call is SILENTLY TRUNCATED at 100 (my "returns everything" was REFUTED -- send `x-pagination: true`)**; **`POST /search` returns ORGANIZATIONS as well as teams (15.5% of hits) -- filter on the ENVELOPE `type`, never `result.type` (absent on teams, inverts) and never `public_id` (orgs carry one); an org id 404s on `GET /teams/{id}` (58/58) and a STORED `gc_uuid` can BE one**; per-team median missing ~30-38% (the "distributions disagree" lesson was org contamination, WITHDRAWN); chain 18/18/18 + 20/20; search-coincidence = observed outcome, **mechanism UNTESTED** (2026-08-04)
