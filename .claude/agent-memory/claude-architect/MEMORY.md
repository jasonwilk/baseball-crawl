...
- Per-epic codification records: **see [epic-codifications.md](epic-codifications.md)** -- E-077 through E-278, one section each: what was codified, where it was placed, and why. That file is the record; this index deliberately does not restate it. (Collapsed 2026-07-26, layer pass D6: this bullet had reached 8,553 characters -- 42% of MEMORY.md -- duplicating its own topic file and tripping the read-limit hook. Before removal, its project-fact content was verified still present, in fuller form, in `.claude/rules/data-model.md` (the scored-but-empty MODAL case and the data-bearing-EXISTS rule), `canonical-seams.md` (the canonical helpers, and the same-population-is-necessary-but-not-sufficient invariant with its load-bearing temporal clause), `architecture-subsystems.md`, and CLAUDE.md (report generation and deletion are destructive).)
  - Two habits from that record that generalize past any single epic, kept here because they change how a codification pass is RUN: **a defect belongs where it BINDS** -- the rule an agent already has loaded at the moment it would make the mistake, not the rule nearest to where the defect was found; and **re-homing beats duplicating** -- at E-276 a lesson moved into a shared rule let software-engineer delete its longer local copy, so that promotion cost negative lines. E-277 added a third and a bound: **a rule in the wrong place is not a weaker rule, it is an ABSENT one** -- story 01 learned that a sweep must run after the story's own edits land and wrote it into story 01's AC-9a; story 05's AC-8 did not inherit it and reproduced the identical defect one story over. The rule was not weak, disputed or forgotten; it was in a file its next author had no reason to open. **The test, and run it on your own output: name the next person who needs this rule and ask whether they will open the file it is in.** Run it FORWARD as well as backward -- E-277's two-coordinator findings looked dead until PM noted their audience arrives if the operator ever grants the `Agent` tool to named agents, which made the experiment post-mortem their home rather than the bin. **The bound: `.claude/rules/tool-output-integrity.md` loads on EVERY interaction for EVERY agent (21KB before E-277), so a rule binding only at AC-authoring time belongs in `product-manager.md`, not there.** Placement is by binding site, and prefer extending an existing sentence to adding a section. E-278 added two more, both about what a codification pass ACCEPTS rather than where it places things. **(4) Codify the OPERATION, never one command's spelling** -- my E-267 prohibition named `git checkout -- <file>`; `git checkout-index` and `git restore` are the same index-restore hazard, and an SE who had QUOTED that rule mid-dispatch lost work to a spelling it did not name. Whenever a prohibition names a literal, ask what CLASS it belongs to and declare the list open. **(5) The diagnosis you are handed is itself an inherited claim -- enumerate before accepting a finding's SCOPE.** At E-278 the A1 brief said one new query had falsified a `data-model.md` absolute; enumerating `FROM games` showed the absolute had never held and three sibling queries in the same file already violated it, so "state the exception" would have been the wrong fix. Same pass, same file: a PM instruction that a docstring claim was "still TRUE" was refuted by that enumeration -- **the correct VERDICT it rode on is exactly what stops the premise being checked**, so a codification pass must re-derive a finding's extent, not just its direction. **And apply this to instructions FROM ABOVE, not only to material inherited sideways** -- the rule reads as governing what upstream artifacts hand you, but its highest-yield use at E-278 was on a direct instruction from the agent coordinating the work (a "leave the reasoning" that had itself gone stale, given in the same message that ruled on two other instances of the shape). An instruction is an artifact written at one moment and executed at another, which is the whole basis of the relay rule; coming from the coordinator exempts nothing.
