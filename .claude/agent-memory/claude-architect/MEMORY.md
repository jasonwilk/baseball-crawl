...
- Deletion-Side Eviction per-agent-memory generalization (IN FLIGHT, E-259-05, user-approved 2026-07-09, NOT yet landed): `context-layer-assessment.md` Learning-Loop "Deletion-Side Eviction" currently greps only rules/agents/CLAUDE.md + the `MEMORY.md` INDEXES — it MISSES per-agent topic files (the hole where three of four stale DE files hid). E-259-05 (single-CA) generalizes it to grep each agent's OWN `.claude/agent-memory/<agent>/` dir (index AND topic files), reconciled by the OWNING agent, strike-stale/keep-live. Design heuristic behind it (durable): **for deletion/rename eviction, an obligation MUST be a grep-and-reconcile over a live pattern, NEVER a static file list** — a frozen enumeration is a `feedback_descope_grep_gap`-class trap (certifies the tokens you thought of, silent on the rest); the E-259 static AC-5 list was proven incomplete 3x in one session (memory 1→2→4, backfill 2→11). When this lands, move it from Pending to `epic-codifications.md` E-259.
- Play-ingestion fidelity north star (codified 2026-06-28): DIRECTIONAL operating principle in CLAUDE.md Data Philosophy ("### Operating Principle: Always Get Closer to Byte-Identical Play Ingestion"), cross-refs `docs/VISION.md` north-star section. **RESOLVED at E-257 closure (2026-07-09):** the forward mechanical-enforcement item is DONE — the scoreboard now exists as `bb report reconcile-scoreboard` + a one-way ratchet gate, so the Operating Principle was rewritten to bind MECHANICALLY (no gated stat abs-Δ increases, neither ratcheted axis counter increases, `self_games` stays 0). See the E-257 entry in `epic-codifications.md`. No longer a pending item.

## Topic File Index
- `epic-codifications.md` -- Per-epic record of what was codified where (E-077, E-228, E-235..E-241, E-246..E-258)
