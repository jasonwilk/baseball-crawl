...
## Reviewing fold-in / housekeeping epics under the E-260 meta-layer freeze
When reviewing a context-layer story that "folds in" a previously-filed idea (or an audit residual) as a defect-cited correction, **re-verify each cited defect against the LIVE file before trusting the citation** — an intervening epic may have already fixed it, which turns a defect-cited correction into freeze-barred enrichment. Concrete case (E-262-05 review, iter 1, 2026-07-12): IDEA-092 (filed 2026-07-03) cited data-engineer.md's Core Entities table as naming non-existent tables (`PlayerTeamSeason`/`Lineup`/`PlateAppearance`); E-250-04 had already stripped those anchors, leaving a table that names only real tables plus a self-aware "fuller refresh tracked as IDEA-092" note. The story's AC passed with zero edits and the residual work was pure "could be richer" enrichment — which the E-260 freeze bars. Outcome: IDEA-092 DROPPED from the story and flipped to DISCARDED. Review heuristic for freeze-era fold-in epics: (1) a citation is a claim about a PAST file state — read the current file and confirm the falsehood/staleness STILL EXISTS; (2) if the cited defect is gone, the item is done — do not let leftover "make it fuller/richer" scope ride in as defect-cited; (3) watch for citation drift generally (line refs go stale — e.g. IDEA-114's `report.py:581/:666` had moved to `:530`; IDEA-118's "drop post-E-213 date" was already done). This is the freeze's teeth in practice: the citation must name a defect that is real NOW, not one that was real when the idea was filed.
