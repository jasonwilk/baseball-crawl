...
**`GameSummaryEntry.last_scoring_update` does NOT carry a last-scoring-update instant on any live path.** `ScoutingLoader._build_games_index_from_data` (`scouting_loader.py:774`) is the ONLY construction site in `src/` and sets `last_scoring_update=str(start_ts)` where `start_ts = game.get("start_ts") or game.get("end_ts") or ""`. The real field belongs to the authenticated `game-summaries` endpoint, whose file-reading loader entry points died in E-256. The name is vestigial and actively misleading -- it misled a live-data audit into filing "a scorekeeper edited the book the next morning" as a candidate cause. `_derive_game_date`'s docstring inherits the same error. **The basis is already the start instant.** Do not "fix" it by re-basing.

**An unknown timezone yields the UTC date through the SUCCESS return, not the `or` fallback.** `derive_local_date`'s `except (ZoneInfoNotFoundError, ValueError)` (`src/util/timezone.py:114`) logs a WARNING and falls through to `return dt.date().isoformat()` with `dt` still UTC -- it does NOT return `None`. For a `...Z` instant that string is byte-identical to the unparseable-instant fallback `instant[:10]`, so the two paths are indistinguishable from the stored date; only the log discriminates. An ABSENT timezone is different again: `tz_name = summary.timezone or get_operating_timezone().key` routes it to the operating zone (default `America/Chicago`), never to UTC.

**Two writers of `games.game_date`, on different bases.** Forward: `_derive_game_date` from `last_scoring_update` (= `start_ts or end_ts`). Backfill: `src/db/backfill_game_dates.py` from **`start_time`** + stored `games.timezone`, wired to a live operator command (`src/cli/data.py:570`), and it SKIPS rows with NULL `start_time`. Since `start_time = game.get("start_ts")` has no `end_ts` fallback while `last_scoring_update` does, a row whose payload lacked `start_ts` is skipped by the backfill while a sibling row is re-derived -- splitting their dates with the API payloads in perfect agreement. Its `dry_run=True` mode is a ready-made read-only stored-vs-re-derived audit across all rows.
