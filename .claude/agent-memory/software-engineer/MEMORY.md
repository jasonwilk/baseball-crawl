...
- [testing-gotchas.md](testing-gotchas.md) -- Never trust a `pytest | tail` exit code (capture real RC); the `db` fixture is `:memory:` in test_report_plays.py but disk-backed in test_report_generator.py, so `db.backup()` onto the same path deadlocks
