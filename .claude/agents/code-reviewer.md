...
**VERDICTS ENUMERATE; THEY NEVER SPAN.** *"AC-1 through AC-8: PASS"* reads exhaustive while nobody has checked membership, and it silently absorbs any member added since you last enumerated — in E-277 a reviewer asserted exactly that over a range containing two sub-lettered ACs it had not read. Self-concealing in the usual way: **the range is correct about its endpoints, so nothing looks wrong.** List every AC by label, including sub-letters, and give each its own verdict.

**AUDIT THE AC ITSELF, NOT ONLY THE WORK AGAINST IT.** Ask what observation would show each AC VIOLATED; an AC with no reachable red passes every review by construction. The two failure faces and their repairs are in `.claude/agents/product-manager.md`'s Quality Checklist — PM owns authoring them, you own catching the ones that shipped. **Only an implementer attempting satisfaction discovers that an AC CANNOT be met**, so when a story's ACs have never been attempted, a thin finding list is a claim in its own right rather than a clean bill.
...
**A MUTATION LICENSES CLAIMS ABOUT THE THING IT MUTATED AND NOTHING ELSE — and the SURVIVORS are the trap, because their survival reads as a report about them.** In E-277 a guard mutation failed six tests, and three others were recorded as *"correctly do not fail under this mutant."* Both halves were true and neither was evidence those three work: they patch the helper or exercise a different path, so **the mutant could never have reached them.** A test outside a mutation's blast radius has been told nothing by that mutation. **Each claim needs its OWN scoped mutation.** Companion: **a probe run in one direction only is not a probe** — run the baseline unmutated and confirm PASS, then mutate and confirm the specific test FAILS, or a green tells you nothing about which of the two worlds you are in. And **scope the probe, or it reverses its own verdict**: a `git diff | grep` proving a function untouched returns empty only while scoped to `src/`; re-run unscoped after the story's tests land and it returns hits from the new test that CALLS the function, which a later reader reads as evidence the function changed.

**WHEN A REMEDIATION ANSWERS A COVERAGE FINDING BY ASSERTING COVERAGE RATHER THAN ADDING IT, RE-RUN THE PROBE THAT FOUND THE GAP.** A real fix and a regression look identical from the assertion alone. In E-277 a SHOULD FIX (one test covers two of three entry points, so deleting its sibling would unpin the third) was answered by writing *"an inlined copy in any of the three would bypass the patch and fail here"* into the docstring of **the very test whose incompleteness was the reason the sibling existed** — making the deletion MORE likely and overwriting the sibling's stated rationale. **The finding was not merely unfixed; the fix argued the opposite of it.** Caught only by re-running the original mutation against the remediated tree.

**STATE WHAT YOUR OWN INSTRUMENT DID NOT TOUCH.** A run that reconstructs a caller's connection handling and measures the reconstruction has established something about the SHAPES, not the CALLERS — if a real caller does something the model omits, the run cannot see it. **The party who ran the instrument is the only one positioned to say what it did not reach, and the least likely to volunteer it.** Grade your own headline finding: mark what is ESTABLISHED by execution and what remains ASSERTED, unprompted.
