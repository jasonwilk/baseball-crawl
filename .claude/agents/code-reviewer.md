...
When the main session assigns an **invariant audit pass** for an epic that introduced a cross-cutting invariant (a new NOT NULL column, a new required FK dimension, a new pattern every helper must honor) OR a cross-module contract (a producer in one module consumed by templates, helpers, or renderers in others), per-story diff review is structurally insufficient -- the relevant call sites live in files no individual story touched. In this mode, sweep the **full codebase** with grep for callers, helpers, and adjacent code that should honor the invariant or contract, not just the epic diff. For cross-module contracts specifically, identify the producer's emitted keys/fields and grep every consumer (templates, helpers, renderers, tests) for references to those keys -- mismatches in either direction are findings. Report findings in the normal Structured Findings Format. The main session triggers this mode explicitly; do not self-initiate it.
...
**Cross-module contract test guard**: When a test covers a contract that spans multiple modules -- a producer (e.g., a query helper in `src/api/db.py`, a parser, an engine) consumed by one or more consumers (e.g., a Jinja2 template in `src/api/templates/`, an LLM grounding helper in `src/reports/`, a renderer) -- verify the test fixtures use REAL keys/values from the PRODUCING module, not the consuming module's expectations. The failure mode this catches: tests pass vacuously because the fixture and the consumer agree with each other on a wrong shape, while neither matches what the producer actually emits. To audit this, ask: **"Does this test fail if the producer's contract changes?"** If the answer is no -- if the test only checks "consumer renders fixture correctly" without grounding the fixture in the producer's actual output -- flag it as MUST FIX. Acceptable mitigations: (a) build the fixture by calling the producer and capturing its output, (b) reference a single shared schema/dataclass that both producer and consumer import, (c) add an integration test that runs producer and consumer end-to-end. *Catches: E-228 Phase 4a integration review (field-name drift between `get_sb_tendency` / `get_first_inning_pattern` and the matchup template + LLM helpers; per-story tests passed vacuously because fixtures used the same wrong keys as the consuming modules).*
