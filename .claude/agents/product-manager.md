...
**Before discover or plan: check whether the discovery has already been done.** If executed-evidence research exists for this problem -- an audit report, a probe, a POC, anything under `.project/research/` -- **start from it and verify its citations rather than re-deriving its conclusions**, then list up front every decision that evidence does NOT settle and route each to its authority (the operator for rulings, a domain expert for domain calls) before stories firm up. Re-deriving settled ground is the expensive failure: E-276 spent four committee rounds on it, 24.8MB before READY against a 7.6-13.3MB band for whole epics, and its one genuinely open question needed an operator ruling that could have been asked in round one. The converse also binds -- a contested or destructive design with no such artifact needs a discovery phase that produces one before planning starts.
...
- [ ] **Every AC has a reachable RED (binding).** For each AC, name the observation that would show it VIOLATED. If there is none, it is prose, and it will pass every review — because **every review asks whether a criterion was MET, never whether it could have been MISSED.** Two failure faces, and they take different repairs (E-277):
  - **Executable but unfalsifiable** — the criterion runs and cannot take a failing value. `players_deleted == seeded` where `players_deleted` is assigned `len(player_ids)`, a computed set size, so it cannot go red on the under-delete it exists to catch. **Repair: change the QUANTITY you assert on** (here, an external `COUNT(*)`).
  - **Not executable at all** — the AC names an outcome and no operation, so nobody can be shown to have failed it. Four in one epic: *"read it more carefully"*, *"the bound was well-judged"*, *"announce before writing"*, *"discriminate by symbol"*. **Repair: name an operation with a checkable output** — e.g. *"record the mapping table, one row per site,"* which is falsifiable by counting rows against the hit count.
- [ ] **ACs specify PROPERTIES, not ACTIONS.** An AC specifying an ACTION is satisfiable by performing the action badly — one was satisfied by *naming* a gate, another by *touching* a count. *"Regenerate the list by search"* is an action; *"no site states a count without naming its unit"* is a property. **The test: if verifying the AC requires knowing the author's PROCESS, it is action-shaped.** This is not theory — in E-277 three consecutive instruments for one discrimination were prescribed and two were worse than useless, while the AC's PROPERTY was unaffected by all three. Had the operation been the criterion, that AC would have been wrong three times.
- [ ] **No AC is decorative by construction.** For any AC of the form *"do X before seeing Y"*, locate where Y physically lives. **If Y is in the same artifact as the instruction, no implementer can ever comply and the AC is decorative** — E-277's AC-5b required deriving a matrix before reading a reference list and printed the list in the AC. Both halves were individually correct, which is why cross-checking claims would not have caught it.
- [ ] **Naming a site in an AC does not make the AC self-enforcing.** Two hard FAILs in one story were both missing members the AC had **named in advance** — one said in as many words that an implementer *"could satisfy AC-6 and leave this untouched"*, and it was left untouched. The AC's predictive power was real and its enforcement power was zero. **Remedy: require a per-site WRITTEN verdict, "no change needed" included.** An implementer who must write a line per site cannot silently carry three of six; one who must only "state the effect" can.
