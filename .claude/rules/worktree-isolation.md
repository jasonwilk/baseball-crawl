...
- **NEVER run `git checkout -- <file>` in a dispatch worktree.** With prior stories staged by the staging-boundary protocol (`git add -A` after each story passes review), `checkout --` restores the STAGED version — the PREVIOUS story's state — not HEAD, silently destroying the current story's uncommitted work. It bites hardest on a file that several stories in the epic all edit, and the mutation-verification practice is what invites it: an agent that just mutated a file to prove a test discriminates is exactly the agent reaching for a quick revert. Instead, `cp` the file to the scratchpad BEFORE mutating and restore from that copy. In E-267 this destroyed an entire story's changes to `src/db/reconcile_at_load.py`; the recovery was a scratchpad backup plus a full re-run of the verification battery, because a restore you did not verify is not a restore.
