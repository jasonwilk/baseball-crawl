...
**The second question is harder and matters more: is this a GOTCHA or is it REFERENCE?** A gotcha is something an agent would get wrong by reasonable default -- report generation being destructive, the public roster URL being inverted, `root_team_id` not being `gc_uuid`. Reference is everything an agent can look up once it knows where to look: flag lists, exit codes, per-seam rationale, endpoint schemas. Gotchas belong here, stated in a sentence or two. Reference belongs behind a pointer. The 2026-07-26 restructure cut CLAUDE.md from ~59KB to ~21KB almost entirely by moving reference out -- the canonical-seam bullets to `.claude/rules/canonical-seams.md`, per-command detail to `docs/admin/operations.md`, endpoint detail to `docs/api/` -- while keeping every gotcha those blocks contained. Density is not thoroughness: a long block reads as a mandate to trace every clause before acting, which is a cost paid on every interaction by every agent. There is no line target here, and what disciplines CLAUDE.md's size instead is stated rather than assumed: the gotcha-versus-reference test above, applied to every addition; the operator-reviewed closure diff, where growth is visible; and the periodic refinement pass, where it is acted on (`.claude/rules/context-layer-assessment.md`, Cadence). `.claude/hooks/context-ratchet.sh` measures the four subtrees on demand -- it reports a reading and gates nothing.

## MEMORY.md Scope

MEMORY.md is an index, not a memory store: the platform silently truncates content beyond line 200, so extract detailed content to topic files in the same directory and link from MEMORY.md. What disciplines an index's length is that truncation itself -- a harder bound than any target, because exceeding it costs recall silently rather than failing a review -- together with the periodic refinement pass (`.claude/rules/context-layer-assessment.md`, Cadence), which is where an index that has outgrown line 200 gets re-homed into topic files.
