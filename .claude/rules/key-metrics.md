...
- **Pitching workload**: 4-field model: `last_outing_date`, `last_outing_days_ago`, `pitches_7d` (rolling 7-day pitch count), `span_days_7d` (days covered in window). `pitches_7d` has 3 states: 0 (pitched but zero pitches recorded), NULL (no outings in window), SUM (normal). Shared query function `get_pitching_workload()` in `src/api/db.py` is KEPT protected core (it serves the reports flow) and currently also feeds the dashboard. The dashboard CONSUMER is quarantined (see `.claude/rules/quarantine.md`), so the prior two-surface parity requirement is FROZEN -- the function stays, but new work need not keep the dashboard rendering in parity with reports. Display formats vary by surface: dashboard uses relative ("Xd ago"), report PDF uses absolute dates ("Mar 28"), report web uses JS-upgraded relative with absolute tooltip.
