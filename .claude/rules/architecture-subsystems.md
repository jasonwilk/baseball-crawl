...
**Cross-perspective dedup redirect map (E-244)**: `GameLoader` collapses cross-perspective duplicate games to a single canonical `game_id` (natural key: `game_date` + unordered team pair), so the deduped game's SOURCE event id no longer has a `games` row. `LoadResult.redirect_map` (`{source_event_id: canonical_game_id}`, populated at the dedup redirect site, surfaced via `ScoutingLoader.load_team`/`_load_team_from_disk`) carries this mapping forward. **Footgun**: any per-game generator stage that keys a DB write, idempotency precheck, reconcile call, or query-scope off the in-memory `crawl_result`/`boxscores` source event ids MUST remap those ids through `redirect_map` (identity via `.get(src, src)` for non-deduped games) before filing/reading — otherwise its FK guard silently SKIPS every deduped game (the bug E-244 fixed: plays/spray/reconciliation re-derived ids from source keys and no-opped on deduped games, computing plays-derived rate stats over a subset). Fetch stays keyed by the SOURCE event id (the API returns the scouted perspective by source id); only the DB-facing key becomes canonical. Live consumers: `_crawl_and_load_plays` and `_crawl_and_load_spray` in `generator.py`. See `.claude/rules/perspective-provenance.md` (Plays Pipeline) for the underlying collapse mechanism.
