...
| GET | [/game-stream-processing/{event_id}/boxscore](endpoints/get-game-stream-processing-event_id-boxscore.md) | CONFIRMED | req | Per-player batting and pitching lines for both teams |
...
| GET | [/game-streams/{game_stream_id}/events](endpoints/get-game-streams-game_stream_id-events.md) | CONFIRMED | req | Raw game event stream (event_data is JSON-encoded string); STRUCTURED pitch-level source (pitch speed/type/result + per-pitch createdAt timing) that plays flattens |
