...
*Last updated: 2026-07-26 | Source: E-267 (reconcile-at-load against the fresh crawl), E-276 (corrected the game and player-line gate to a pre-upsert snapshot; removed the roster grain's floor gate outright; docs-writer disclosed the player-line grain's one-run-window and `bb data dedup-players`-reliance residual, per the 2026-07-26 layer-pass handoff)*
...
*Last updated: 2026-07-24 | Source: E-273 (E-273-05: one-time backlog reclamation one-shot + operator sequence)*

Deleting reports (via cascade-delete or the report-only path above) can leave `teams`, `players`, and `team_rosters` rows behind with no surviving report pointing at them -- orphaned reference data. E-273 added a reachability-based reclamation pass that hard-deletes exactly these rows: any team (and its players and roster rows) no longer reachable from any surviving report.
...
| `1` | Operational error (e.g. the database path could not be resolved). |

**Idempotent**: a second run against an already-clean database deletes nothing and reports zero orphans.

**Scale is not a precondition.** The sequence above works regardless of the current backlog's size -- it keys on the reachability invariant (is this team/player/roster row reachable from any surviving report?), not on a specific count. Run it against the database's current live state rather than any previously-reported figure.
