...
### Commit blocked by `[archive-refs: BLOCKED]`

A pre-commit hook (`.githooks/pre-commit`, delegating to `scripts/check_archive_refs.sh`) guards epic archiving. It fires only when a single commit stages **both** a deletion under `epics/E-NNN-...` **and** an addition under `.project/archive/E-NNN-...` for the same epic ID -- i.e. an archive-rename commit. On every other commit it is silent.

When it fires, it sweeps the repo for surviving literal references to the epic's pre-archive path (`epics/E-NNN-`) outside `.project/archive/`, and refuses the commit if any remain, printing `[archive-refs: BLOCKED]` followed by the offending lines. It fails closed: if `scripts/check_archive_refs.sh` is missing or not executable, the commit is refused with the same message.

**What to do**: reconcile the surviving references -- do not try to make the message go away without addressing them. The script's own output routes each hit by the directory it lives in: `epics/`, `.project/` -> product-manager; `.claude/**` -> claude-architect; `src/`, `tests/`, `scripts/` -> software-engineer; `docs/` -> docs-writer. Each surviving reference is one of two kinds, and the fix differs:

- **Criterion reference** -- something a reader is meant to act on or navigate to (a link, a "see epics/E-NNN-..." pointer). Repoint it to the new `.project/archive/E-NNN-...` path.
- **Evidence reference** -- a record of what was actually observed (a decision log, a completion note citing where something happened). Reword it so it no longer spells the pre-archive path, preserving the record rather than deleting it.

**There is no override flag for this gate.** The only escape is `git commit --no-verify`, and that is a deliberate operator decision to make case by case, not a routine bypass -- it also disables the PII scan for that commit.
