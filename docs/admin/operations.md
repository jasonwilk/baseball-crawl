...
**Before and after**: run [`bb report reconcile-scoreboard`](#reconciliation-scoreboard-bb-report-reconcile-scoreboard) on either side of the run, and compare the two readings -- merging duplicate games should hold steady or improve `no_plays_units` as perspectives consolidate onto a single game row, and should not worsen any stat figure.

### Deduplicating Player Entries (`bb data dedup-players`)

*Last updated: 2026-08-12 | Source: 2026-08-10-opponent-roster-dedup-gap (placeholder-name guard + content-aware refusal, and the load-path sweep widened to every team a load writes roster rows for), E-249 (connected-component dedup + fork refusal)*

`bb data dedup-players` detects and merges same-team duplicate player entries caused by cross-perspective UUID mismatch. It groups candidates into connected components and collapses unambiguous ones, but **refuses** any component it cannot merge without guessing -- leaving it unmerged for manual review rather than silently mis-merging. There are **two refusal classes** and they mean different things; both are listed below. Use `--dry-run` (default) to preview, then `--execute` to apply.
...
**Refused for conflicting content (the second class, review the same way).** A component whose members hold stat rows for the *same* game under the *same* perspective can only be merged by DELETING one of the two rows. When those rows are byte-identical that costs nothing and the merge proceeds normally. When they **disagree** on any stat column, merging would destroy a real observation, so the whole component is refused. The preview and the `--execute` WARN name the table, `game_id`, `perspective_team_id` and the differing columns so you can go look at the two rows. This command **refuses; it does not adjudicate** -- deciding which row is right is a manual judgment call, and there is no flag to override it.

**Placeholder names are not duplicates at all.** Two players both recorded as `Unknown Unknown` (the stub written when a boxscore omits a name) will report as *no duplicates found*, not as a refusal -- the guard is in detection, so the pair never reaches the plan. This is deliberate: those two ids fold to identical names and would otherwise collapse as a single-terminal-name component, and on the live corpus such a pair was **two different pitchers in one game**, whose merge would have deleted one of the two real appearances. Note the consequence for reading logs: fork refusal fires on *distinct* names, so **"zero refused forks" was exactly what this hazard looked like** -- it was never evidence of safety.

**Non-zero exit on failure.** If any component merge fails during `--execute`, the command prints the error and exits non-zero. A zero exit on execute means every collapsible component succeeded (neither refusal class is a failure -- both are intentional deferrals). Check the exit code in any scripted context.
...
One figure is not a judgement call. **`self_games` must be 0** -- a game with `home_team_id == away_team_id` is always an opponent-resolution bug, never an acceptable floor. The [Runtime Smoke Check](production-deployment.md#runtime-smoke-check) asserts exactly this and nothing else.
...
**Vestige, until a follow-up chunk removes it**: the command's gate code is still in place. It still accepts `--update-baseline`, still diffs against the now-frozen baseline JSON, still describes a live gate in its `--help` text, and can still exit non-zero (`1` regression or precondition failure, `3` baseline absent, `4` baseline malformed). **Ignore that exit code and read the numbers.** If the CLI's own help text and this page disagree, this page is current.
...
**⚠ The check is not passive -- a refused submission still changes the database.** Before counting, it runs the stale-generation reaper: any generation stuck in progress for more than an hour (its process died mid-run) is marked failed and its half-written HTML file is deleted, and that result is committed whether or not anything was reaped. This is deliberate, but **not because reaping first shortens the hour** -- it does not. The reaper only touches generations that are ALREADY past the hour, so a younger one blocks this page under any order, and that is correct: it is probably still alive. What the order buys is that once a crashed generation IS past the hour, this click clears it and lets your submission through in the SAME request, rather than clearing it for some later attempt. The cost is that submitting here is never a read-only act, even when you are told no.
...
**Bias to refuse -- game and player-line grains.** On these two grains, a retire happens only when the fresh crawl is corroborated healthy -- it fetched successfully, returned a non-empty payload, and did not shrink catastrophically against a snapshot of what was already loaded, captured *before* this run's own writes (E-276; a snapshot read after the run's own writes would count rows the run itself just wrote as corroboration, which is not a health check). Any doubt keeps the existing data untouched. On the game grain, this now logs exactly one WARNING per refusal cause -- naming the affected game-id set and count -- rather than one WARNING per absent game (E-277); the player-line grain's refusal WARNs remain per refused entry, unaffected by this change. A transient failure, a postponed game, or a partial/degraded crawl never causes data loss on these two grains. A rescheduled game (moved date, same real game) is matched and updated in place rather than treated as a removal.

**The player-line grain's protection has a one-run window, not a standing one (E-276).** A refusal does not roll back that run's own writes -- the fresh rows land regardless -- so a refused churn still grows the prior population the *next* run's gate measures against. Sustained id churn (the same players repeatedly re-issued fresh ids across successive re-scouts) can eventually push the comparable ratio to the permit floor and hard-delete the entire prior generation in one run -- unlike roster, this grain has no departure ceiling beneath it. When a permitted retire looks like this shape, the load logs a WARN naming the matched players and pointing at `bb data dedup-players`; run it to merge the surviving pair before the next re-scout rather than waiting for the WARN to recur. That command is not a complete closer, though. **One of its three gaps is now partly closed, and the qualification matters**: as of 2026-08-12 the load-path sweep runs for EVERY team whose `team_rosters` rows a load wrote -- both sides of every boxscore -- so an opponent block now has a closer where it previously had none *in any shape* (and because league play is cyclic, no generation order converged). It closes the window only where the merge is SAFE, and there are two shapes it still leaves open:

- **The re-issued line disagrees with the prior one.** If the scorekeeper edited the line between re-scouts, the two rows collide on `(game_id, perspective_team_id)` with differing content, the component is refused (see the second refusal class above), and this grain reverts to exactly the pre-2026-08-12 sequence -- both generations accumulate, and the prior one is hard-deleted on the run after. The deletion is deferred to this grain rather than prevented; once it happens the component carries no conflict and the next run merges it. Pinned by `test_the_content_refusal_leaves_regime_B_open_on_a_DIFFERING_opponent_line`.
- **Both ids are placeholder-named.** Detection excludes the `Unknown Unknown` stub outright, so a re-issue where GameChanger supplied no name in either generation has no closer here at all. That exclusion is deliberate and is the safer error (see the placeholder note above); it is a real coverage boundary, not an oversight.

The two other gaps are unchanged: it matches only on name *prefix* (a rename such as `Mike` to `Michael` is invisible to it), and a refused component does not currently surface as an error (IDEA-189) -- now in either of the two refusal classes described under `bb data dedup-players` above. Partial id churn on this grain is therefore an accepted, documented residual, not a defect to chase down (IDEA-185).
...
### Reclaiming Orphaned Reference Data

*Last updated: 2026-07-27 | Source: E-273 (E-273-05: one-time backlog reclamation one-shot + operator sequence), E-277 (E-277-01: audit-row keep-root; E-277-02: refuses a dirty borrowed connection)*

Deleting reports (via cascade-delete or the report-only path above) can leave `teams`, `players`, and `team_rosters` rows behind with no surviving report pointing at them -- orphaned reference data. E-273 added a reachability-based reclamation pass that hard-deletes a team (and its players and roster rows) once it is unreachable through every root the pass recognizes -- report-linkage is the common one, but not the only one.

**A team carrying any `scheduled_report_runs` row is kept regardless of report reachability (E-277).** That table records the operator's morning-run scheduling history for the team, including a placeholder slot from a bracket that hadn't resolved yet -- a shape that persists no other pinning row. Losing it is not regenerable, so it is a reachability root in its own right: any own team with even one morning-run slot survives the sweep, whether or not a report currently references it. A team with zero slots -- one that has never had a morning run attempt against it -- is unaffected by this and is still swept and recreated harmlessly if orphaned.

**Ambient reclamation (no operator action needed).** The pass is wired into the deletion paths and also fires opportunistically -- alongside expired-file cleanup -- at the start of every `bb report generate` and every `bb report cleanup` run. It is best-effort: a failure in the sweep is isolated and never blocks or fails the caller it rides along with. It also **defers** rather than deletes anything while a report is mid-generation (`status = 'generating'`) -- a live generation's in-progress data is never at risk. This is a liveness delay, not a leak; the sweep simply catches up on its next opportunistic run.

**Guard against a dirty borrowed connection (E-277).** The pass, the stuck-`generating` reaper it calls, and the expired-file cleanup sweep all COMMIT on whatever connection they are handed. If a caller ever passed one of them a connection that already had uncommitted work in flight, that work would become visible, get classified as orphaned, and be deleted -- with the caller's later rollback recovering nothing. All three entry points now refuse a connection in that state, raising before any commit occurs, rather than silently destroying it. This is a defensive guard, not a response to an observed incident: every live call site was measured clean before the guard shipped, and it is expected to stay inert. If it ever does fire, the effect an operator sees depends on the caller -- inside `bb report generate` or the admin report-delete action it is caught and logged (a WARNING or ERROR in the application log; the command itself still completes), and in the one-shot script below it is caught, logged, and surfaced as a non-zero exit (see the exit-code table).

**One-time backlog reclamation (`scripts/reclaim_orphan_reference_data.py`).** A throwaway operator one-shot for clearing an existing backlog in one controlled pass against a quiescent database. It is deliberately **not** a `bb data` subcommand -- it imports and calls the same `reclaim_orphan_reference_data` pass the ambient sweep above uses, so there is no duplicated reclamation logic and no new permanent CLI surface:

```bash
python scripts/reclaim_orphan_reference_data.py
```

Optional `--db-path PATH` overrides `DATABASE_PATH`.
...
| Code | Meaning |
|---|---|
| `0` | Success -- reclamation ran and no orphaned reference data remains. |
| `2` | Deferred -- a report generation was in flight when the one-shot ran; the same reap-then-gate guard as the ambient sweep refused and deleted nothing. Re-run against a quiescent database. |
| `3` | Residual -- reclamation ran but orphans remain afterward. Investigate (possible overreach or an unhandled reachability edge) rather than re-running blindly. |
| `1` | Operational error -- e.g. the database path could not be resolved, or (rarely, E-277) the guard against a dirty borrowed connection refused to proceed. |

**The audit keep-root does not produce a false residual (verified, E-277).** Two synthetic-DB shapes were run through the real one-shot: an audit-pinned team alongside a separate unpinned orphan, and an audit-pinned team alone -- the shape that isolates a false residual, since with only one team in the database there is no other count movement to mask a divergence. Both exited `0`, with the pinned team and its audit row surviving. This holds structurally, not just in the two fixtures: `count_orphan_reference_data` and the delete set are drawn from the same predicate, so they cannot diverge on the audit root without the predicate itself changing, and the pass's own fixed-point self-assert aborts the sweep -- rather than exiting quietly -- if a divergence is ever detected. **This check covers the audit root only** -- the other three reachability roots were not exercised by this verification.

**Idempotent**: a second run against an already-clean database deletes nothing and reports zero orphans.

**Scale is not a precondition.** The sequence above works regardless of the current backlog's size -- it keys on the reachability invariant (is this team/player/roster row reachable through any root the pass recognizes?), not on a specific count. Run it against the database's current live state rather than any previously-reported figure.
...
*Last updated: 2026-07-25 | Source: E-267 (E-267-06: purge scouting/report data, preserve identity + auth), E-270 (E-270-02: purge guard hardening)*
...
**Sequence: guard -> preview -> confirm -> backup -> purge.** Each step runs to completion before the next begins, and no row is deleted until the last one:

1. **Guard.** `APP_ENV` is validated before a single row is read or counted. An unrecognized value (a typo such as `APP_ENV=prod`) refuses outright, regardless of any flag -- see below.
2. **Preview.** The command prints the resolved database path and a per-table row count of what would be deleted, so the operator confirms against the actual target rather than an assumption about which database is in play. This read never modifies the database file and fails with a clear error if the database cannot be found.
3. **Confirm.** Off production, a yes/no prompt. On production, the operator must TYPE the resolved database's filename or the literal `purge` -- a reflex `y` is exactly how the wrong database gets emptied. `--yes` skips whichever of these two prompt forms applies -- the typed production confirmation included -- which is why a scripted production purge needs `--force --yes` together, not `--force` alone.
4. **Backup.** A fail-closed backup of the resolved database is taken immediately before the purge (same mechanism as [`bb db backup`](#database-backup-and-restore) above). If the backup fails for any reason, the command aborts with nothing deleted.
5. **Purge.** The 20 tables are deleted inside one transaction, as before.
...
**Production is refused by a typo-proof guard that no flag overrides.** `APP_ENV` is normalized (case and whitespace) before it is compared to `production`, so a variant like `" Production "` is still caught, not just the exact string `production`. An unrecognized value -- an abbreviation typo such as `APP_ENV=prod` -- is refused outright rather than silently treated as non-production: the command cannot tell what environment it is running in, and `--force` cannot resolve an ambiguous posture. This check runs first, before the preview and before either `--force` or `--yes` is read.

**`--force` and `--yes` are separate flags with separate jobs.** This is a breaking change from the pre-E-270 command, which had a single `--force` flag that did both.

- `--force` overrides the PRODUCTION REFUSAL only. It does **not** skip the confirmation prompt.
- `--yes` skips the interactive confirmation prompt only -- whichever form applies (the yes/no prompt off production, or the typed filename/`purge` prompt on production). It does **not** override the production refusal.
- A scripted or cron production purge needs **both**. An existing script or cron entry that passed `--force` alone will now stop at the confirmation prompt and abort on EOF (a non-zero exit, not a hang) -- update any such invocation to `--force --yes` before relying on it.

```bash
bb db purge-scouting --force --yes
```

Without `--force` against a production database, the command refuses and exits non-zero.
...
**Single transaction.** All 20 tables are deleted inside one transaction -- a mid-purge database failure rolls back cleanly and the database is left unchanged (only the post-commit file-unlink step carries the orphan risk described above). The pre-purge backup taken in step 4 above gives a recovery point independent of this transaction.
