...
| `REFRESH_TOKEN_WEB` | JWT for obtaining access tokens | 14 days (self-renewing) | `bb creds refresh` or browser capture |
| `ACCESS_TOKEN_WEB` | JWT for API calls | ~60 minutes | Generated automatically |
| `DEVICE_ID_WEB` | Stable hex device fingerprint | Permanent | Browser capture (one-time) |
