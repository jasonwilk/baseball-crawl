...
- [ ] **AC-3**: Given two games on the same date between the same teams where `start_time` is NULL on one or both, but the scores differ, when `GameLoader.load_file()` processes the second game, then score matching prevents false-positive dedup. Per Technical Notes "Natural Key for Game Dedup" section.
...
- [ ] **AC-5**: All existing tests in `tests/test_game_loader.py` continue to pass (no regressions).
...
- `tests/test_game_loader.py` or `tests/test_game_dedup.py` -- new dedup-specific tests
