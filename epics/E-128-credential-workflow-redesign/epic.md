...
- **Test file merge note**: Stories 01, 04, and 05 all add tests to `tests/test_cli_creds.py`. If dispatched in parallel worktrees, expect merge conflicts in the test file. The router should merge E-128-01 first (it's a dependency for 02 and 05), then the remaining stories' test additions will be additive.
