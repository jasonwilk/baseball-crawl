...
- [ ] **AC-4**: CLAUDE.md's "Key Architectural Decisions" auth paragraph is slimmed to a pointer to the rule file (e.g., "Auth module constraints are in `.claude/rules/auth-module.md`"), removing duplicated detail. The "GameChanger API" section's auth bullet is consistent with the rule file content.
