...
## Acceptance Criteria
- [ ] **AC-1**: Given the mitmproxy addon captures a POST /auth request with a user-auth body containing the operator's address, when the request is logged to `endpoint-log.jsonl`, then the address field value is replaced with `"[REDACTED]"` and the `type` field is preserved.
