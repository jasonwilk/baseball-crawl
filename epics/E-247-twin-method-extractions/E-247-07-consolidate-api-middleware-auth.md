...
- `APP_URL` is read in **three** live places with two divergent default values: `src/reports/generator.py:226` (default `http://localhost:8001`, const at `:65`), `src/api/routes/reports_admin.py:543` (inline default `http://localhost:8001`), and `src/api/routes/auth.py:92`/`:569` (default `http://baseball.localhost:8001`, const at `:73`). Codex spec review surfaced the generator.py site as the third helper missed in the original scoping. Unifying to one default changes the link default only at the site(s) that currently default to `baseball.localhost:8001` (the auth.py helper) — generator.py and reports_admin.py already default to `localhost:8001`.

This is security-relevant (auth/session paths); behavior must stay identical except the sanctioned `APP_URL`-default unification. **The single default is decided: `localhost:8001`** (epic Open Questions, resolved 2026-06-29) — matches the documented local-dev stack URL in CLAUDE.md; production sets `APP_URL` explicitly so only dev links are affected. In practice this changes only the auth.py helper's unset-default (from `baseball.localhost:8001` to `localhost:8001`).
...
- [ ] **AC-4**: Given `APP_URL` is read in three live places with two divergent default values, when the story completes, then one `_get_app_url` helper with the single decided default **`localhost:8001`** is the source, and **all three former sites** (`src/reports/generator.py:226`, `src/api/routes/reports_admin.py:543`, `src/api/routes/auth.py`) use it — no inline `APP_URL` read or local default constant survives at any of the three. When `APP_URL` is unset, every link resolves to `http://localhost:8001` (dev only — production sets `APP_URL` explicitly).
...
- [ ] APP_URL unified to the decided default `localhost:8001` (AC-4)
...
## Notes
The `APP_URL` default is decided: `localhost:8001` (affects dev report-links only when the env var is unset; production sets `APP_URL` explicitly). Runs after E-247-05 (shared `reports_admin.py`). Security-sensitive — claude-architect performs a review-time security pass on this diff (user decision, epic Open Questions).
