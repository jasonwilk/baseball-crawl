...
## Acceptance Criteria
- [ ] **AC-1**: A project document explains the checked-in Codex layer (`AGENTS.md`, `.codex/config.toml`, repo skills) versus the gitignored runtime layer (`CODEX_HOME`, auth, sessions, history, caches).
...
- [ ] **AC-6**: The document names the repo skill added by E-081-03 and explains when to use it.
- [ ] **AC-7**: The document explicitly states that the baseline Codex lane does not require spawned agents, `multi_agent`, or other experimental Codex features.
