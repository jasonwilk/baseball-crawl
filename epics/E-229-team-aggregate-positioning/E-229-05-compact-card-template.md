...
- [ ] **AC-8**: Coach design review of the layout: a documented baseball-coach review record exists in this story's Notes section (verdict, date, failure points on FAIL or no-failure-modes confirmation on PASS). The AC is satisfied by the presence of the record; revision on FAIL is a PM-gated process step, not part of the AC. Coach reviews against quarter-letter print sample. **Coach legibility minimums** (per coach MN-2): pill jersey numbers ≥7–8pt, compass letters ≥7–8pt, simple field outline (no texture). Fold-line consideration: if folded landscape mid-card, the fold runs through the field-diagram body — coach's review verdict captures whether this is workable. Coach reviews the typography minimums in context.
