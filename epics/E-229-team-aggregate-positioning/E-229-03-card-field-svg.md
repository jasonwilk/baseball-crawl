...
- [ ] **AC-6**: Card legend rendered at the bottom of the diagram from the `COMPASS_LEGEND_SHORT` module-level constant (per UXD M-1 single-source legend wording, also referenced in epic TN-3). Format: `★ team default · ○ textbook · A–H = outlier zones (see right)`. One line, ~7pt.
...
```python
COMPASS_LEGEND_SHORT = "★ team default · ○ textbook · A–H = outlier zones (see right)"
