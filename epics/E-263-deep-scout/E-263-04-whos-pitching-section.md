...
- [ ] **AC-8** (eligibility-basis accuracy sentence, per Technical Notes TN-11): the section renders the coach-facing availability-basis sentence from the fact E-263-02c produced — SET → "Availability is calculated from the official [level] pitch-count rules — these calls are exact."; UNSET → "Level not set — availability uses a general youth-arm guideline (USA Baseball Pitch Smart) and may be off by a day at the margins." — so the coach knows whether the eligibility calls are exact or a guideline. A test covers the SET and UNSET copy paths.
...
## Dependencies
- **Blocked by**: E-263-01 (layout spec), E-263-02a (fact-sheet framework + pitching stub + partial), E-263-02b (SIG-001 fact — the join target), E-263-02c (the availability-basis fact for the AC-8 accuracy sentence)
...
- `tests/test_deep_scout_pitching.py` (new — innings-weighting, SIG-001 join, whiff% derivation, FPS% rate + coverage badge, perspective-scoping via the shared twin-game fixture, and the SET/UNSET availability-basis accuracy sentence per AC-8)
...
## SHOULD-HAVE refinement (non-blocking — NOT part of the DoD gate)
**Per-start strike% volatility read (methodology correction #3, 2026-07-18 reviews; baseball-coach `.claude/agent-memory/baseball-coach/deep-scout-signal-catalog.md` §3).** A pitcher-consistency read that sharpens the Game Plan's live cue (E-263-07 AC-8): a VOLATILE arm means the live cue governs (don't trust the season average), a STEADY arm means the season line is reliable. It improves the plan but is NOT MUST-tier and MUST NOT block this story; land it opportunistically if within a session, else it stays captured here + in coach memory for a follow-on. Definition (authoritative, baseball-coach):
...
- **Start-count floor: <4 starts → "Consistency: unknown — only N starts." Do NOT guess** (committee arms make only 2–4 starts a summer).
- Labels: range ≤ ~10 pts → **Steady** ("trust the season line"); ≥ ~15 pts → **Volatile** ("read him tonight — see the early cue"); middle → moderately consistent.
- **RETRACTION note:** this REPLACES the earlier-floated "both tails for volatile arms" rule keyed on moderate strike% + high walk rate — baseball-coach retracted that as WRONG (it describes a command LEVEL, not volatility, and would tag nearly every HS arm). That wrong version was never written into an E-263 story; the correct dispersion definition above is the only one that binds.

## Handoff Context
- **Produces for E-263-07**: SIG-004 (per-arm control, for the two-branch plan's "wild" vs "locates" branches) and **SIG-006 FPS%** (the season baseline the Game Plan's live identification cue prints, E-263-07 AC-8). If the SHOULD-HAVE per-start volatility read above lands, it also feeds the cue (volatile → cue governs; steady → season line reliable). (SIG-016 first-inning wobble is surfaced for the opponent's starter in this section; its self-scout reuse on our own starter is a v2 item — the `--vs` context is out of v1.)
