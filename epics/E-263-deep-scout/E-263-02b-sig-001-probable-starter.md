...
- [ ] **AC-6** (eligibility level gate, per Technical Notes TN-11): SIG-001's eligibility computation consumes the OPERATOR-SELECTED competition level resolved by E-263-02c (authoritative, overriding `detect_league_level`), so a Legion opponent is rested on the Legion table and an NSAA opponent on the NSAA table — the level is NEVER re-derived from the opponent's team name here. The rest table is chosen via the engine's existing `get_rules_for_league`; season phase stays date-derived. A test confirms SIG-001 eligibility uses the operator-selected level (e.g. a Legion-set opponent whose arm sits at a Legion/NSAA divergence margin — 46–50, 61–70, or 81–90 pitches — resolves per the Legion table, not the NSAA table).

## Technical Approach
Fill the SIG-001 computation into the pitching group stub module from E-263-02a (the one the assembler already calls), so it is composed into the fact sheet. Reuse the existing starter-prediction engine and its reference-date mechanism rather than re-deriving eligibility, and consume the operator-selected `league` established by E-263-02c (do NOT re-call `detect_league_level`, which would reintroduce team-name inference — per AC-6 / Technical Notes TN-11). Do not render a partial here — E-263-04 owns the Who's Pitching partial that surfaces SIG-001. Per AC-5 (operator-decided), honor `FEATURE_PREDICTED_STARTER` (default-OFF, not promoted): when off, surface a loud operator signal + a visible degraded state on the starter-conditioned sections (E-263-04/07), never a silent empty section — reuse the existing report-run degraded/honesty mechanism rather than inventing a new one.

## SHOULD-HAVE refinements (non-blocking — NOT part of the DoD gate)
Two probable-starter MODEL refinements the 2026-07-18 live runs exposed (baseball-coach recorded them in `.claude/agent-memory/baseball-coach/probable-starter-model`). They improve rank quality but are NOT MUST-tier and MUST NOT block this story's completion; address opportunistically here if within a session, else they remain captured in the model memory for a follow-on:
1. **Rank on days-since-last-START, not days-since-last-appearance.** A long total-layoff arm is a WILDCARD, not "due" — in both live runs a long-idle arm was mis-ranked #1 (Norfolk floated demoted swing arms; Columbus would pick a 23-day-idle arm over the held ace). Rank should key on rest since the last START.
2. **Detect opener + bulk-reliever.** `appearance_order = 1` mislabels a 28-pitch opener as the starter; detect the opener/bulk pattern so the bulk arm is not hidden behind a token opener.

## Dependencies
- **Blocked by**: E-263-02a (the fact-sheet framework + assembler skeleton), E-263-02c (the operator-selected competition level SIG-001's eligibility consumes)
