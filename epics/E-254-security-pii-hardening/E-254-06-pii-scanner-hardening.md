...
- [ ] **AC-8**: Given `git show :<path>` fails for a staged path (mocked/absent blob), when `pii_scanner --staged` runs, then the scanner warns and skips that path while the exit code still reflects any other findings (error-path honesty, per TN-6).
