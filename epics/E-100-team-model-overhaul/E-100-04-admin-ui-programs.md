...
### Team List and Edit
- [x] **AC-1**: `GET /admin/teams` displays all teams in a flat table. Columns: team name, program (from programs join, or blank), division (classification), membership badge (filled green "Member" / hollow gray "Tracked"), active/inactive, opponent count (from opponent_links WHERE our_team_id = teams.id), edit link.
- [x] **AC-2**: All admin team routes use INTEGER `{id}` path parameters: `GET /admin/teams/{id}/edit`, `POST /admin/teams/{id}/edit`, `POST /admin/teams/{id}/toggle-active`, `POST /admin/teams/{id}/discover-opponents`.
- [x] **AC-3**: `GET /admin/teams/{id}/edit` includes: name field, program dropdown (existing programs + empty option), division dropdown (optgroup: HS with Varsity/JV/Freshman/Reserve, USSSA with 8U-14U, Other with legion), active toggle. Membership type is editable (radio button) — correction path for misclassification.
- [x] **AC-4**: `POST /admin/teams/{id}/edit` saves name, program_id, classification, and membership_type using INTEGER `id` WHERE clause.
- [x] **AC-5**: All `is_owned` references in admin routes replaced with `membership_type`. All `level` references replaced with `classification` (displayed as "Division").
- [x] **AC-6**: Opponent count per team (from opponent_links) links to the opponents page filtered by that team.
- [x] **AC-7**: Template links use INTEGER `id` (e.g., `url_for('edit_team', id=team.id)`).

### Two-Phase Add-Team Flow
- [x] **AC-8**: Add-team form on `GET /admin/teams` is a single field (GC URL or public_id) and a submit button. No "team type" radio buttons.
- [x] **AC-9**: `POST /admin/teams` resolves the team from the URL/public_id, discovers gc_uuid via reverse bridge (success = store UUID, 403 = NULL), and redirects to a confirm page. Errors shown on Phase 1 form.
- [x] **AC-10**: `GET /admin/teams/confirm` displays resolved team info: team name, gc_uuid status (informational), membership radio (member/tracked, default: tracked), program dropdown (pre-selected if name matches), division dropdown (pre-selected if keywords match).
- [x] **AC-11**: `POST /admin/teams/confirm` creates the team row with INTEGER PK auto-assigned, operator-selected `membership_type`, `gc_uuid` (UUID if bridge succeeded, NULL otherwise), `public_id`, optional `program_id` and `classification`. Redirects to team list.
- [x] **AC-12**: gc_uuid discovery: reverse bridge success = gc_uuid stored; `BridgeForbiddenError` = gc_uuid NULL. Membership_type is operator-selected (not bridge-derived). Default is 'tracked'.
- [x] **AC-13**: Division inference from team name: keywords like "Varsity", "JV", "Freshman", "Reserve", age-group patterns (e.g., "14U") pre-select the division dropdown.
- [x] **AC-14**: Program pre-selection: existing program name as substring of GC team name (case-insensitive, longest match wins). Defaults to no program if no match.
- [x] **AC-15**: Duplicate detection: if team already exists (duplicate `public_id` or `gc_uuid`), confirm page shows an error instead of creating a duplicate.

### User-Team Assignment
- [x] **AC-16**: User management functions in admin.py (`_get_available_teams`, `_get_user_team_ids`, `_create_user`, `_update_user`, create/edit user route handlers) use INTEGER team references (`teams.id`) instead of TEXT `team_id`. Team checkbox values in `edit_user.html` and `users.html` use `team.id` (INTEGER). `user_team_access.team_id` column references are updated to work with INTEGER FK to `teams(id)`.
...
- [x] **AC-18**: All admin test suites pass: `tests/test_admin.py`, `tests/test_admin_teams.py`, `tests/test_admin_opponents.py`.
