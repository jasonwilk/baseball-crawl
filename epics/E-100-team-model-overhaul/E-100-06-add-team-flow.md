...
## Context
The current add-team form at `/admin/teams` requires the operator to manually declare "owned" or "tracked" before seeing what team was found — UXD identified this as the core UX problem. The two-phase flow eliminates manual declaration: the system resolves the team, auto-detects membership via the reverse bridge, and presents a confirm page with smart defaults. Program assignment is optional — the team-first model treats programs as organizational metadata, not a required field. This story depends on E-100-03 (membership_type in pipeline code) and E-100-04 (division dropdown and membership display patterns in admin UI) because the confirm page reuses those UI patterns.
...
- [ ] **AC-3**: `GET /admin/teams/confirm` (new route) displays the resolved team information: team name (from GC), auto-detected membership status (displayed, not editable), program dropdown (pre-selected if team name matches an existing program, with empty/none default), and division dropdown (pre-selected if inferred from team name keywords).
...
- [ ] **AC-7**: Program pre-selection: if an existing program's `name` appears as a substring of the GC team name (case-insensitive), that program is pre-selected. Longest match wins. If no match, defaults to "-- no program --" (program assignment is optional, not required).
- [ ] **AC-8**: If the team already exists in the database (duplicate `public_id` or `gc_uuid`), the confirm page shows an informative error instead of creating a duplicate row.
- [ ] **AC-9**: Tests verify: (a) URL parsing to public_id, (b) bridge success → member detection, (c) bridge 403 → tracked detection, (d) confirm page renders with correct pre-selections, (e) POST confirm creates team row with correct values, (f) duplicate team detection, (g) division inference from team name keywords, (h) team created without program when no match (program_id=NULL).
...
- The "＋ Create new program" sub-page flow from the confirm page was removed during the vision pivot (team-first simplification). Program creation is deferred to the edit page or a future admin programs page. Teams can be created without a program.
