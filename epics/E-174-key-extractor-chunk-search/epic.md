...
**2026-07-26 triage addendum -- KEPT, stays DRAFT. No promotion path outside the normal plan/refine workflow.**

Reviewed in a PM triage of the three oldest epics in `/epics/` (E-072 and E-175 were abandoned in the same pass; this one was retained). Recorded as fact, deliberately without urgency framing:

**1. Scope of the dependency.** The Overview frames this as fixing `bb creds extract-key`, which reads as a troubleshooting utility. `extract_client_key()` is also **Step 1/3 of `_setup_web()`** (`src/cli/creds.py:986`), the credential bootstrap documented in `docs/admin/bootstrap-guide.md`, where a `KeyExtractionError` aborts setup with exit 1. Three docs cite `bb creds extract-key` as the recovery procedure for a stale client key -- `docs/api/auth.md`, `docs/admin/credential-refresh.md`, `docs/admin/post-reset-guide.md` (15 references total) -- and `src/gamechanger/credentials.py:328` raises an error message pointing at it. (**Count note**: an earlier triage message said "four runbooks." It is three; the figure was re-derived and corrected before being written here. Do not reintroduce four.)

**2. Code state verified unfixed (2026-07-26).** `src/gamechanger/key_extractor.py` still searches only the index bundle -- `_BUNDLE_SRC_PATTERN` matches `static/js/index.*.js`, and the module docstring still documents index-bundle-only extraction. Every internal E-174-01 cites (`_find_bundle_url`, `_fetch_bundle`, `_find_composites`, `_parse_composite`) is present and unchanged in shape, so its Technical Approach still applies as written.

**3. The operator's evidence base, which governs how the two facts above should be read.** The system has been in continuous use since March 2026 with no real breakage (operator, 2026-07-26). That is four months of direct operating evidence, and it outweighs a reading of the code that infers fragility from structure. `_setup_web()` short-circuits at `src/cli/creds.py:962` when credentials are still valid, so the extractor is reached only when they are stale -- which means the code path has plausibly not been exercised, **and equally means nothing has demanded it**. The honest statement is that we do not know the current live behavior, not that a break is waiting. Facts 1 and 2 describe what the code does; they are not an argument for pulling this epic forward.

**Refinement, not a probe-gated promotion.** This epic's premise is LIVE-GC-dependent and cannot be settled from the repository (no credentials, no live GC access, and PM has no Bash tool). The live check -- **is `EDEN_AUTH_CLIENT_KEY` still absent from the index bundle and present in a code-split `/assets/*.js` chunk on the current GC build?** -- is the natural **first refinement step**, to be taken **if and when the operator picks this epic up through the normal plan/refine workflow**. No probe is dispatched, and none is pending.

**There is no promotion path outside that workflow.** Do not promote this epic on a probe result arriving out of band, on inference from the code, or on the age of the finding. It stays DRAFT until refined deliberately. Refining before building is the point: the spec as written may also be more than the problem needs -- E-174-01 covers multi-chunk discovery, per-chunk error tolerance, and early exit, and a narrower fix may suffice once the live behavior is actually known.

**Superseded stale-READY triage note (2026-07-08, retained as history)**: A real bug fix (`bb creds extract-key` broke ~2026-03-25 when GC moved `EDEN_AUTH_CLIENT_KEY` into code-split asset chunks), but it sat READY ~99–122 days. Its premise is LIVE-GC-dependent and cannot be re-confirmed from the worktree (no creds/live GC): GC may have restructured their build again, and the operator may have routed around it via `bb creds import`. Demoted to DRAFT per the stale-READY rule pending operator/api-scout re-confirmation of TWO questions: (1) is `bb creds extract-key` still broken against the current live GC site? (2) is it still needed, or does `bb creds import` (paste-curl) suffice as the credential path? If still broken AND needed, re-promote to READY and prioritize (it is live-infra). Content preserved.
...
## History
- **2026-07-26: KEPT as DRAFT** by operator ruling on a PM triage of the three oldest epics in `/epics/`. Two facts recorded in the Status section without urgency framing: `extract_client_key()` is also Step 1/3 of `_setup_web()`, not only the `bb creds extract-key` diagnostic; and the code is verified still unfixed. Set against the operator's stated evidence base -- **the system has run since March 2026 with no real breakage** -- which is why this is recorded rather than escalated. **No probe dispatched and none pending.** The live GC check is the first refinement step if and when the operator takes this epic up through the normal plan/refine workflow; there is no promotion path outside it. Not archived and not abandoned.
- 2026-07-08: Demoted READY→DRAFT by the E-255-06 stale-READY triage (sat READY ~99–122 days).
