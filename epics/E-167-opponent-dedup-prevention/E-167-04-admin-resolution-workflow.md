...
- [ ] **AC-3**: `GET /admin/opponents/{link_id}/resolve` renders a suggestion page per TN-6. The page auto-searches the GC endpoint with the opponent's name + `sport=baseball` + `year={current_season_year}`. Results are displayed as cards showing team name, location (city/state), age group, and record (or whatever fields the actual response provides -- see TN-7). Each card has a "Select" button.
...
- [ ] **AC-11**: Hidden opponents (`is_hidden = 1`) show an "Unhide" button in the opponents list that POSTs to set `is_hidden = 0`, making the opponent visible in the unresolved banner again. This provides reversibility for the skip action.
...
- Hidden opponents (`is_hidden = 1`) remain visible in the opponents list with a "Hidden" badge and an "Unhide" button for reversibility. They are excluded from the unresolved banner count.
