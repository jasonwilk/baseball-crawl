...
- [ ] **AC-4**: `find_duplicate_teams()` is enhanced to also return groups where one team has `season_year=NULL` and the other has a non-NULL season_year (same name, case-insensitive, both tracked). These are returned as separate groups from the exact-season_year matches.
...
- [ ] **AC-7**: Tests cover the enhanced detection (NULL-vs-non-NULL season_year), the canonical selection heuristic, the safe-merge predicate (especially the games-between exclusion), and the dry-run output format.
