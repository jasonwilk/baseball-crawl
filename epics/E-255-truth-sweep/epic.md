...
**baseball-coach (own memory)** — MEMORY.md cross-team/multi-season "from day one" section → one-line non-goal note + dashboard reference; coaching-decisions.md season-over-season framing + ghost entities; scouting-pipeline.md SUPERSEDED banner on the 403 season-stats recipes.

**data-engineer (own memory)** — etl-patterns.md token-refresh-impossible rewrite; MEMORY.md Core Entity Model (ghost tables, cross-team identity, nonexistent Litestream); endpoint-schema-notes `PlayerTeamSeason` mapping; season_aggregate_writers deleted-caller list.
