...
- **READY gate in refinement rounds**: The refinement-rounds variant ends with the main session offering to mark the epic READY. This connects the skill's completion to the existing READY gate in workflow-discipline.md ("PM sets READY after refinement is complete"). During review-rounds, the main session orchestrates without PM -- so the main session performs the status update directly after user confirmation. This is an offer, not automatic: the user must confirm, preserving the Dispatch Authorization Gate (marking READY and dispatching remain separate actions).
- **Review rounds operate on the main checkout**: Implementers spawned during review rounds are NOT isolated in worktrees. Review rounds operate on already-merged code in the main checkout -- worktree isolation is a dispatch-time concern, not a review-time concern.
