...
- [ ] **AC-9**: The skill includes an Anti-Patterns section that prohibits: modifying wrapped skills, skipping respawn between rounds, continuing rounds after a clean review, and the main session performing review or triage alone without spawning the required participant agents (implementer for review rounds, PM + domain experts for refinement rounds).
