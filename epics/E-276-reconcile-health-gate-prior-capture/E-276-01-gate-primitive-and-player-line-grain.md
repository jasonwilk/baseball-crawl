...
- [ ] **AC-14 (MULTI-RUN — the class every planning probe missed, at production scale)**: A test drives **N ≥ 4 sequential `ScoutingLoader.load_team` invocations** and asserts the **exact surviving line count after each one**, not merely at the end. Per-invocation assertions are the deliverable; an endpoint-only assertion does not satisfy this AC.
      ...
      **The property to pin, which is the player-line analogue of the roster lock: THIS GRAIN MUST NOT RATCHET.** Run the AC-1 churn payload (zero overlap) repeatedly against the same team-season:

      - after **every** invocation, the **9 original lines survive** — not 9 then 9 then fewer;
      - the gate's prior count, asserted on the keyed record entry per AC-2, **stays at the pre-run population on every run** and never grows across runs;
      - a final **recovery** invocation carrying the original ids retires nothing and leaves the 9 intact.

      **The mechanism that makes this non-trivial rather than obvious**: a refusal writes nothing, so run 2's snapshot should equal run 1's. **That is a prediction, not a given** — on the roster grain the equivalent prediction is FALSE (a refusal strands jersey-backfill rows into the next run's snapshot, which is the lock's whole mechanism, epic TN-3 / [[IDEA-186]]). This grain is claimed clean because `W ⊆ fresh` holds here and the backfill has no player-line analogue. **The AC is what converts that claim from a premise into an assertion**, and it is the same premise story 02 AC-8 guards at runtime.
      ...
      **Scoped honestly, per this story's Definition of Done**: the *recovery* and *no-ratchet* assertions hold under pre-fix code too (a polluted gate also writes nothing when it refuses). **The discriminating half is the per-run prior count** — pre-fix it reads the post-upsert population (18) on the churn run, post-fix the pre-run one (9). Assert that per invocation and the test fails pre-fix for the right reason.
...
- [ ] Tests written and passing. **The DISCRIMINATING tests — those in AC-1, AC-2, AC-4 and AC-14 — demonstrably FAIL against pre-fix code and PASS after.** Scoped deliberately: several required tests here cannot fail pre-fix and it is not a defect that they cannot. AC-3's `9/8` case refuses under both regimes (it is a floor, and the parametrization is discriminating as a set, not case-by-case); AC-5 asserts behaviour identical to today; AC-6, AC-7, AC-8, AC-9a and AC-9b test code that does not exist pre-fix, so "fails before" is not even well-defined for them. **A blanket fail-before/pass-after line would make this story's own Definition of Done unsatisfiable**, and an implementer meeting it literally would have to manufacture discrimination where none is available — the failure this epic exists to stop.
      ...
      **AC-14 is listed as discriminating on ONE of its assertions only, and the split must be kept.** Its **per-run prior count** discriminates (18 pre-fix, 9 post-fix, on the churn invocation). Its **no-ratchet and recovery** assertions hold under both regimes and are regression guards, not discrimination — the same shape as story 03 AC-8(a). Reporting AC-14 as wholly fail-before/pass-after would be an overclaim; reporting it as wholly non-discriminating would drop the assertion that earns it.
