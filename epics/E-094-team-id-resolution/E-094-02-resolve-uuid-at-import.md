...
- [ ] **AC-2**: Given an owned team added via UUID input, the UUID is stored as `team_id`. The public_id is resolved via the forward bridge (`GET /teams/{uuid}/public-team-profile-id`) or from the `resolve_team()` response. The team profile is still fetched via public API for name/metadata.
...
- [ ] **AC-4**: Given an owned team where the reverse bridge returns 403, the import fails with a clear error message explaining that the team cannot be found on the user's account (the user may have selected "owned" for a team they are not a member of).
...
- [ ] **AC-8**: The `GameChangerClient` usage for the reverse bridge call handles auth errors gracefully (expired tokens should produce a user-friendly error, not a crash).
...
The duplicate check currently tests `_team_id_exists(public_id)`. After the fix, owned teams will have UUID as `team_id`, so the duplicate check also needs to test `public_id` column matches (a team with UUID `team_id` and matching `public_id` should be considered a duplicate when the same public_id is submitted again).
